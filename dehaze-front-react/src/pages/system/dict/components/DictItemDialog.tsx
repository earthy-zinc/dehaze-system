...
const DictItemDialog = forwardRef<DictItemDialogRef>((_props, ref) => {
  ...
  const openForm = useCallback(
    (t: "add" | "edit", record?: DictPageVO) => {
      ...
      if (t === "add") {
        ...
        dictForm.resetFields();
        DictAPI.getDictFormData(record.id)
          .then((d) => {
  ...
  const handleDelete = useCallback(
    (record: DictPageVO) => {
      DictAPI.deleteDictByIds(String(record.id))
        .then(() => {
  ...
  const handleBatchDelete = useCallback(() => {
    Modal.confirm({
      ...
      okType: "danger",
      onOk: () =>
        DictAPI.deleteDictByIds(selectedRowKeys.join(","))
          .then(() => {
      ...
      },
      {
        ...
        render: (_: unknown, record: DictPageVO) => (
          <Space size="small">
            ...
            <Popconfirm
              title={`确认删除字典数据「${record.name}」吗？删除后不可恢复。`}
