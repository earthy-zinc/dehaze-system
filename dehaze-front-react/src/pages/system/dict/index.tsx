...
import {
  Button, Card, Form, Input, message, Modal, Popconfirm, Space, Table, Tag, type TableColumnsType,
} from "antd";
...
const DictManagement: React.FC = () => {
  ...
  const refreshList = useCallback(() => { setRefreshFlag((prev) => prev + 1); setSelectedRowKeys([]); }, []);
  ...
  const handleReset = useCallback(() => { searchForm.resetFields(); setSelectedRowKeys([]); setQuery({ pageNum: 1, pageSize: 10 }); }, [searchForm]);
  ...
  const handleBatchDelete = useCallback(() => {
    Modal.confirm({
      ...
      onOk: () => DictAPI.deleteDictTypes(selectedRowKeys.join(",")).then(() => {
        message.success(`成功删除 ${selectedRowKeys.length} 个字典类型`); refreshList();
      }).catch((e) => { message.error(e?.message || "删除失败"); return Promise.reject(e); }),
    });
    ...
    { title: "创建时间", dataIndex: "createTime" as any, key: "createTime", width: 180, align: "center" },
    {
      ...
      render: (_: unknown, record: DictTypePageVO) => (
        <Space size="small">
          ...
          <Popconfirm title={`确认删除字典类型「${record.name}」吗？删除后不可恢复。`} onConfirm={() => handleDelete(record)} okText="确定" cancelText="取消" okType="danger">
