...
def define_GH(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal',
...
def define_D(input_nc, config, init_type='normal', gpu_ids=[]):
    ...
    if which_model_netD == 'basic':
        ...
        netD = UnetGenerator(input_nc, 1, 8, ndf, norm_layer=norm_layer, use_dropout=False,
...
class ResnetGenerator_K(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d,
                 use_dropout=False, n_blocks=6, padding_type: str = 'reflect'):
        assert (n_blocks >= 0)
        ...
        model_K = [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                               use_bias=use_bias)]
        model_de = []
...
class UnetGenerator_new(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
        ...
        self.unet_block3 = UnetSkipBlocks(output_nc, ngf, input_nc=input_nc, outermost=True, norm_layer=norm_layer,
                                          final_activation=final_activation)
...
class GenerativeModel(nn.Module):
    ...
    def estimate_airlight(_I, method):
        ...
        if method == 'he':
            ...
            dist = nn.CosineSimilarity(dim=1)(input_pixels, norm_vec) * torch.norm(input_pixels, 2,
                                                                                   dim=1)  # projecte distance [bs, -1]
    ...
    def __init__(self, input_nc, output_nc, ngf=32, n_downsample_global=3, n_blocks_global=6,
        ...
        for n in range(1, n_local_enhancers + 1):
            ...
            for i in range(n_blocks_local):
                model_upsample += [
                    ResnetBlock(ngf_global * 2, padding_type=padding_type, norm_layer=norm_layer, use_dropout=False,
                                use_bias=use_bias)]
            ### upsample
            model_upsample += [
...
if __name__ == '__main__':
    ResnetBlock(64, padding_type='reflect', norm_layer=nn.BatchNorm2d, use_dropout=False,
                use_bias=False)
