...
/** LLM 调用状态展示 */
export const CALL_STATUS_META: Record<number, { label: string; tag: TagType }> =
  {
    1: { label: "成功", tag: "success" },
    2: { label: "失败", tag: "danger" },
...
export const TRACE_TYPE_META: Record<string, { label: string; tag: TagType }> =
  {
...
export function traceTypeMeta(traceType?: string) {
  ...
  return (
    TRACE_TYPE_META[traceType] ?? { label: traceType, tag: "info" as const }
  );
}
