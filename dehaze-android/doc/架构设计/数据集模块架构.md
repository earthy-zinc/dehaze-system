...
1. `DatasetDetailFragment` 通过 DataBinding 与 `DatasetViewModel` 进行数据绑定
2. `DatasetViewModel` 处理业务逻辑，调用 `DatasetAPI` 获取数据
3. `DatasetAPI` 通过 Retrofit 发送网络请求
4. 请求结果通过回调返回给 `DatasetViewModel`
5. `DatasetViewModel` 更新 LiveData 状态
6. `DatasetDetailFragment` 观察 LiveData 状态变化并更新UI
