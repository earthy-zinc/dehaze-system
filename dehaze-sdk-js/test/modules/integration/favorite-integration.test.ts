 ...
 * 覆盖 algorithm/result/dataset 三种 targetType 的完整收藏生命周期。
...
describe("收藏跨模块集成测试", () => {
  ...
  /** dataset 由本套件创建才在 afterAll 清理，避免误删并行套件的数据 */
  let datasetCreated = false;

  /** 收藏目标 ID 守卫：beforeAll 未能预取目标时直接失败 */
  function requireTargetId(targetId: number | undefined, label: string): number {
    if (!targetId) {
      throw new Error(`无可用${label}作为收藏目标`);
    }
    return targetId;
  }

  /** 记录待清理的收藏 ID，便于 afterAll 统一清理 */
  function trackFavorite(id: number) {
    favoriteIds.push(id);
  }

  /** 取消收藏后从待清理列表移除，避免 afterAll 重复删除 */
  function untrackFavorite(id: number) {
    const idx = favoriteIds.indexOf(id);
    if (idx >= 0) favoriteIds.splice(idx, 1);
  }

  /** 在指定 targetType 的收藏列表中按收藏 ID 查找记录 */
  async function findFavoriteInPage(targetType: FavoriteTargetType, favoriteId: number) {
    const page = await FavoriteAPI.getPage({ pageNum: 1, pageSize: 100, targetType });
    return page.list.find((item) => item.id === favoriteId);
  }

  /** 清理指定 targetType+targetId 的残留收藏（上次异常中断可能留下未清理数据） */
  ...
  beforeAll(async () => {
    ...
    // 始终创建专用数据集（复用列表中的可能被并行套件误删）；创建需 admin 权限
  ...
  describe("场景1：算法收藏完整流程", () => {
    test("正向流程：收藏算法→验证状态→验证列表→取消收藏", async () => {
      const targetId = requireTargetId(algorithmTargetId, "算法");
  ...
  describe("场景2：处理结果收藏完整流程", () => {
    test("正向流程：去雾处理→收藏结果→验证列表→取消收藏", async () => {
      ...
      const found = await findFavoriteInPage("result", favoriteId);
      expect(found).toBeDefined();
  ...
  describe("场景3：数据集收藏完整流程", () => {
    test("正向流程：获取数据集→收藏→验证列表→取消收藏", async () => {
      const targetId = requireTargetId(datasetTargetId, "数据集");
      ...
      const found = await findFavoriteInPage("dataset", favoriteId);
      expect(found).toBeDefined();
