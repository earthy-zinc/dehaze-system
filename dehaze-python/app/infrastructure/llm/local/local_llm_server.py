...
async def chat_completions(req: ChatCompletionRequest):
    ...
    async def event_stream():
        ...
        # llama.cpp 流式 chunk 不携带 usage：prompt 侧按 tokenize 计数（与推理
        # 输入一致），completion 侧流式每 chunk 恰为一个 token，按 chunk 数累计
        prompt_tokens = len(llm.tokenize(prompt.encode("utf-8"), add_bos=True, special=True))
        ...
        def generate():
            ...
            try:
                ...
                _offer(loop, queue, error if error is not None else _StreamEnd(usage), stop)
        ...
        try:
            ...
            # OpenAI 协议：usage 随独立 chunk（choices=[]）在 [DONE] 前下发，
            # 供客户端计量 token（对应 stream_options.include_usage 请求）
            if final_usage:
                usage_chunk = {
                    "id": completion_id,
                    "object": "chat.completion.chunk",
