...
class A2AServer:
    ...
    def __init__(self, agent_service=None, deep_agent_builder=None):
        """构造注入外部依赖。

        依赖未显式注入时延迟解析模块级单例：基础设施层不反向依赖 service 层，
        避免应用启动期导入 a2a_server 即拉起服务依赖链。
        """
        self._agent_service = agent_service
        self._deep_agent_builder_cls = deep_agent_builder
    ...
    def _deep_builder_cls(self):
        if self._deep_agent_builder_cls is None:
            from app.service.ai.builders.deep_agent_builder import DeepAgentBuilder

            self._deep_agent_builder_cls = DeepAgentBuilder
        return self._deep_agent_builder_cls
    ...
    @staticmethod
    async def resolve_agent(db: AsyncSession, agent_id: int | None, agent_code: str | None) -> int:
        """按 agent_id 或 agent_code 定位可对外服务的 Agent，返回其主键。

        全局 A2A 入口（POST /a2a）不携带路径级 agent_id，由请求体定位目标 Agent。
        """
        from app.repository.ai_agent_repository import ai_agent_repository

        if agent_id is not None:
            return (await A2AServer._get_exposed_agent(db, agent_id)).id
        if agent_code:
    ...
    @staticmethod
    async def resolve_default_exposed_agent(db: AsyncSession) -> int:
        """返回平台默认（首个可对外服务）Agent 的主键，供全局发现端点使用。"""
        from app.repository.ai_agent_repository import ai_agent_repository

        agents = await ai_agent_repository.list_exposed(db)
        if not agents:
            raise ValueError("暂无可对外服务的 Agent")
        return agents[0].id
    ...
    async def _message_send(self, db: AsyncSession, redis, agent_id: int, params: dict) -> dict:
        ...
        try:
            ...
            await get_task_tracker().register(
                task_id=task_id,
                task=runner,
                task_type="a2a_inference",
                metadata={"agent_id": agent_id, "context_id": context_id},
            )
        except Exception as e:
            logger.warning("A2A 任务追踪注册失败（不影响执行）: %s", e)
    ...
    async def _run_inference(
        ...
        try:
            graph = await self._deep_builder_cls().build_from_snapshot(db, redis, snapshot)
            initial_state = {
                "messages": [
