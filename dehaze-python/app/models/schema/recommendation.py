...
class ImageFeatureAnalysisVO(BaseModel):
    imageMd5: Optional[str] = Field(default=None, description="图像MD5（用于后续查询推荐）")
    hazeLevel: str = Field(..., description="雾霾浓度(light/moderate/heavy)")
    hazeConfidence: float = Field(..., description="雾霾浓度置信度(0-1)")
    sceneType: str = Field(..., description="场景类型(urban/landscape/building/night/backlight/indoor)")
...
class AnalyzeForm(BaseModel):
    imageId: Optional[int] = Field(default=None, description="图片ID(imageId/imageUrl二选一)")
    imageUrl: Optional[str] = Field(default=None, description="图片URL(imageId/imageUrl二选一)")


# ========== 算法推荐 ==========

class RecommendedAlgorithmVO(BaseModel):
    recommendationId: Optional[int] = Field(default=None, description="推荐记录ID（用于提交反馈）")
...
class RecommendationRuleForm(BaseModel):
    id: Optional[int] = Field(default=0, description="规则ID(0表示新增)")
    ruleName: str = Field(..., description="规则名称")
    sceneType: str = Field(..., description="场景类型")
    algorithmIds: list[int] = Field(..., description="候选算法ID列表")
