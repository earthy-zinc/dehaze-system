...
if TYPE_CHECKING:
    from app.models.schema.menu import MenuVO
...
class MenuQuery(BaseModel):
    ...
    status: Optional[int] = Field(default=None, ge=0, le=1, description="状态(1-显示；0-隐藏)")
...
class MenuForm(BaseModel):
    ...
    component: Optional[str] = Field(default=None, max_length=200, description="组件路径(vue页面完整路径，省略.vue后缀)")
    perm: Optional[str] = Field(default=None, max_length=100, description="权限标识")
    visible: Optional[int] = Field(default=1, ge=0, le=1, description="显示状态(1:显示;0:隐藏)")
    ...
    redirect: Optional[str] = Field(default=None, max_length=200, description="跳转路径")
    keepAlive: Optional[int] = Field(default=None, ge=0, le=1, description="【菜单】是否开启页面缓存")
    alwaysShow: Optional[int] = Field(default=None, ge=0, le=1, description="【目录】只有一个子路由是否始终显示")
...
class RouteVO(BaseModel):
    ...
    redirect: Optional[str] = Field(default=None, description="跳转路径")
    perm: Optional[str] = Field(default=None, description="按钮权限标识")
    children: Optional[List["RouteVO"]] = Field(default=None, description="子菜单")
