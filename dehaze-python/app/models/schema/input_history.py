...
class InputHistoryForm(BaseModel):
    ...
    status: Optional[int] = Field(default=3, description="处理状态（1=成功，2=失败，3=处理中），创建时确定")
...
class InputHistoryVO(BaseModel):
    ...
    status: Optional[int] = Field(default=None, description="处理状态（1=成功，2=失败，3=处理中），创建时确定，不随处理进度更新")
