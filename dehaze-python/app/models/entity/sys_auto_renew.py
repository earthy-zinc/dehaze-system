...
class SysAutoRenew(BaseModel):
    ...
    status: Mapped[int] = mapped_column(mysql_types.TINYINT, nullable=False, default=1, comment='状态(1:启用;0:已关闭)')
    next_renew_time: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True, comment='下次扣款时间')
    fail_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment='连续失败次数')
    last_renew_order_id: Mapped[Optional[int]] = mapped_column(BigInteger, nullable=True, comment='上次续费订单ID')
