...
class SysMenu(BaseModel, SoftDeleteMixin):
    ...
    name: Mapped[str] = mapped_column(String(64), nullable=False, default="", comment="菜单名称")
    type: Mapped[int] = mapped_column(
        mysql_types.TINYINT, nullable=False, comment="菜单类型(1:目录 2:菜单 3:外链 4:按钮)"
    )
    path: Mapped[str] = mapped_column(String(128), default="", comment="路由路径(浏览器地址栏路径)")
    ...
    visible: Mapped[int] = mapped_column(
        mysql_types.TINYINT, nullable=False, default=1, comment="显示状态(1:显示 0:隐藏)"
    )
    ...
    always_show: Mapped[int] = mapped_column(
        mysql_types.TINYINT, default=0, comment="【目录】只有一个子路由是否始终显示(1:是 0:否)"
    )
...
class SysRoleMenu(Base):
    ...
    menu_id: Mapped[int] = mapped_column(
        BigInteger, primary_key=True, nullable=False, comment="菜单ID"
    )
