...
class SysCoupon(BaseModel):
    ...
    threshold: Mapped[Optional[int]] = mapped_column(BigInteger, nullable=True, comment='使用门槛(分)')
    valid_type: Mapped[str] = mapped_column(String(16), nullable=False, comment='有效期类型')
    valid_start: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True, comment='有效期开始时间')
    valid_end: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True, comment='有效期结束时间')
    valid_days: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, comment='领取后有效天数')
    total_qty: Mapped[int] = mapped_column(Integer, nullable=False, default=-1, comment='发放总量(-1为不限)')
    issued_qty: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment='已发放数量')
    used_qty: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment='已使用数量')
    per_user_limit: Mapped[int] = mapped_column(Integer, nullable=False, default=1, comment='每人限领数量')
