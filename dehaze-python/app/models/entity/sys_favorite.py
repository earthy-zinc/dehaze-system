...
class SysFavorite(BaseModel):
    ...
    target_type: Mapped[str] = mapped_column(String(32), nullable=False, comment="收藏对象类型(algorithm:算法;result:处理结果;dataset:数据集;image:图片;preset:参数方案)")
    target_id: Mapped[int] = mapped_column(BigInteger, nullable=False, comment="收藏对象ID")
    is_invalid: Mapped[int] = mapped_column(mysql_types.TINYINT, nullable=False, default=0, comment="收藏对象是否已失效(0:正常;1:已失效)")
