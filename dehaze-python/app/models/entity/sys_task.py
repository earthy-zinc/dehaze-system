...
class SysTask(BaseModel):
    ...
    status: Mapped[int] = mapped_column(
        ...
        nullable=False,
        default=1,
        comment="任务状态(1:待处理;2:处理中;3:已完成;4:失败;5:已取消)",
    )
    progress: Mapped[int] = mapped_column(Integer, default=0, comment="任务进度")
    total_files: Mapped[int | None] = mapped_column(Integer, comment="总文件数")
    processed_files: Mapped[int] = mapped_column(Integer, default=0, comment="已处理文件数")
    params: Mapped[str | None] = mapped_column(Text, comment="任务参数（JSON）")
    result: Mapped[Any | None] = mapped_column(
        JSON,
        comment=(
            "任务结果（JSON）：导出任务存储对象键（object_name），"
            "下载 URL 由响应层运行时拼接，不落库"
        ),
    )
    ...
    worker_id: Mapped[str | None] = mapped_column(String(64), comment="执行 Worker 标识")
