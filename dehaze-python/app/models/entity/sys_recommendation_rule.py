...
class SysRecommendationRule(BaseModel):
    ...
    algorithm_ids: Mapped[list] = mapped_column(JSON, nullable=False, comment="候选算法ID列表（JSON数组）")
    weight: Mapped[int] = mapped_column(mysql_types.INTEGER, nullable=False, default=0, comment="规则权重（数值越大越优先）")
    enabled: Mapped[int] = mapped_column(mysql_types.TINYINT, nullable=False, default=1, comment="是否启用(0:禁用;1:启用)")
