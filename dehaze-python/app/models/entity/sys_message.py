...
class SysMessage(BaseModel, SoftDeleteMixin):
    ...
    title: Mapped[str] = mapped_column(String(255), nullable=False, default="", comment="消息标题")
    ...
    biz_module: Mapped[str | None] = mapped_column(String(32), nullable=True, comment="业务模块")
    biz_id: Mapped[str | None] = mapped_column(String(64), nullable=True, comment="业务ID")
    priority: Mapped[int] = mapped_column(
        mysql_types.TINYINT, nullable=False, default=2, comment="优先级(1:低;2:中;3:高;4:紧急)"
    )
    jump_url: Mapped[str | None] = mapped_column(String(255), nullable=True, comment="跳转链接")
    extra: Mapped[Any | None] = mapped_column(JSON, nullable=True, comment="扩展数据")
    read_status: Mapped[int] = mapped_column(
        mysql_types.TINYINT, nullable=False, default=0, comment="已读状态(0:未读;1:已读)"
    )
    read_time: Mapped[datetime | None] = mapped_column(DateTime, nullable=True, comment="已读时间")
