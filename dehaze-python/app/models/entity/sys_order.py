...
class SysOrder(BaseModel, SoftDeleteMixin):
    ...
    coupon_id: Mapped[int | None] = mapped_column(
        BigInteger, nullable=True, comment="用户优惠券实例ID"
    )
    ...
    pay_method: Mapped[str | None] = mapped_column(String(16), nullable=True, comment="支付方式")
    status: Mapped[int] = mapped_column(
        mysql_types.TINYINT,
        nullable=False,
        default=1,
        comment="订单状态(1:待支付;2:已支付;3:已完成;4:已取消;5:退款中;6:已退款)",
    )
    ...
    effective_time: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="权益生效时间"
    )
    package_expire_time: Mapped[datetime | None] = mapped_column(
    ...
    is_auto_renew: Mapped[int] = mapped_column(
        mysql_types.TINYINT, nullable=False, default=0, comment="是否自动续费订单"
    )
