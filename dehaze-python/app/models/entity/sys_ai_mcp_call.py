...
class SysAiMcpCall(AppendOnlyModel):
    ...
    tool_name: Mapped[str] = mapped_column(String(128), nullable=False, comment="被调用的工具名")
    request: Mapped[Any | None] = mapped_column(JSON, nullable=True, comment="调用载荷(JSON)")
    response: Mapped[str | None] = mapped_column(Text, nullable=True, comment="响应结果(JSON文本)")
