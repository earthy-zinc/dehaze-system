...
与 AI 对话计费（BillingService 预扣-结算两阶段）不同，语音用量在调用完成前
...
class VoiceBillingService:
    ...
    @staticmethod
    async def ensure_balance(db: AsyncSession, user_id: int, estimated_credits: int) -> None:
        ...
        if not await QuotaService.check_quota(db, user_id, estimated_credits):
            raise BusinessException(
                ResultCode.QUOTA_INSUFFICIENT, "今日或本月 AI 积分配额不足，请升级会员或明日再试"
            )
        if not await BalanceService.check_balance(db, user_id, estimated_credits):
            raise BusinessException(ResultCode.QUOTA_INSUFFICIENT, "积分余额不足，请充值后继续使用")

    @staticmethod
    async def charge_asr(db: AsyncSession, user_id: int, audio_seconds: float) -> int:
        """ASR 按音频时长（秒）实扣，返回消耗积分数"""
        credits = math.ceil(audio_seconds * settings.VOICE_ASR_CREDITS_PER_SECOND)
        return await VoiceBillingService._charge(
            db,
            user_id,
            bill_type="asr",
            model="funasr",
    ...
    @staticmethod
    async def charge_tts(db: AsyncSession, user_id: int, text_chars: int) -> int:
    ...
    async def _charge(
        ...
        *,
        ...
        if credits > 0:
            await QuotaService.deduct(user_id, credits)
            await BalanceService.deduct(db, user_id, credits)
