...
class ApiKeyService:
    ...
    async def _validate_whitelist(self, db: AsyncSession, whitelist: list[str] | None) -> None:
        """校验白名单模型 ID 均存在于启用模型表，非法模型 ID 抛参数异常。"""
        if not whitelist:
            return
        for model_id in whitelist:
            enabled = await self.ai_model_repository.list_enabled_by_model_id(db, model_id)
            if not enabled:
    ...
    async def create_api_key(
        ...
        await self._validate_whitelist(db, model_whitelist)
        ...
        entity = await self.api_key_repository.create(db, entity)
    ...
    async def list_api_keys(self, db: AsyncSession, user_id: int) -> list[dict]:
        ...
        items = await self.api_key_repository.list_active_by_user(db, user_id)
    ...
    async def delete_api_key(self, db: AsyncSession, user_id: int, key_id: int) -> bool:
        ...
        entity = await self.api_key_repository.get_active_by_id_and_user(db, key_id, user_id)
        if not entity:
            return False
        await self.api_key_repository.revoke(db, entity)
        return True
