...
class ScheduledTaskService:
    def __init__(
        self,
        ai_schedule_repository=ai_schedule_repository,
        ai_schedule_run_repository=ai_schedule_run_repository,
        member_repository=member_repository,
    ):
        self.ai_schedule_repository = ai_schedule_repository
        self.ai_schedule_run_repository = ai_schedule_run_repository
    ...
    async def create(self, db: AsyncSession, user_id: int, form: ScheduleCreate) -> ScheduleDetail:
        ...
        count = await self.ai_schedule_repository.count_by_user(db, user_id)
        ...
        await self.ai_schedule_repository.create(db, task)
    ...
    async def list_page(
        ...
        items, total = await self.ai_schedule_repository.paginate(
            db, stmt, query.pageNum, query.pageSize
        )

        latest_map = await self.ai_schedule_run_repository.get_latest_by_schedule_ids(
    ...
    async def set_enabled(self, db: AsyncSession, user_id: int, schedule_id: int, enabled: int) -> None:
        ...
        if enabled == 1:
            if task.status == 2:
                await self.ai_schedule_repository.reset_circuit(db, schedule_id)
            await self.ai_schedule_repository.update_next_trigger(
                db,
                schedule_id,
                self._compute_next_trigger(task.cron, task.timezone),
            )
        await self.ai_schedule_repository.set_enabled(db, schedule_id, enabled)

    async def delete(self, db: AsyncSession, user_id: int, schedule_id: int) -> None:
        """删除任务（软删除，删除后不可恢复）。"""
        await self._get_owned(db, user_id, schedule_id)
        await self.ai_schedule_repository.soft_delete(db, schedule_id)
    ...
    async def list_history(
        ...
        await self._get_owned(db, user_id, schedule_id)
        items, total = await self.ai_schedule_run_repository.page_by_schedule(
    ...
    async def _get_owned(self, db: AsyncSession, user_id: int, schedule_id: int) -> SysAiSchedule:
        ...
        task = await self.ai_schedule_repository.get_by_id(db, schedule_id)
