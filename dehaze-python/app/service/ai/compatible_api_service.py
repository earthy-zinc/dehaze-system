...
async def _get_api_key(db, key_id: int | None) -> SysApiKey | None:
    """按 key_id 查询 SysApiKey（治理预检用），不存在返回 None"""
    if not key_id:
        return None
    return await api_key_repository.get_by_id(db, key_id)
...
class CompatibleApiService:
    ...
    async def _enforce_model_whitelist(
        db, user_id: int, api_key, model: str | None, conv_id: int | None
    ) -> None:
    ...
    async def handle_openai_chat(
        db, user_id: int, body: dict, api_key=None, audit: dict | None = None
    ):
        ...
        conv_id = await CompatibleApiService._resolve_conversation(
            db,
            user_id,
...
async def _run_compatible_call(
    request,
    db,
    user,
    *,
    ...
    if api_key_info:
        ...
        api_key = await _get_api_key(db, key_id)
    audit = {
...
async def _run_models_call(
    ...
    *,
    ...
    api_key = None
    if api_key_info:
        api_key = await _get_api_key(db, api_key_info.get("key_id"))
    audit = {
        "user_id": user.id if user else None,
