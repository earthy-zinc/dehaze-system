...
async def test_update(db):
    ...
    await promotion_service.update(
        db,
        promo["id"],
        PromotionForm(
...
async def test_bind_packages_replace_old(db):
    ...
    await promotion_service.bind_packages(
        db, promo["id"], PromotionPackageForm(packageIds=[101, 102])
    )
    # 重新绑定，应替换旧关联
...
async def test_delete_logical(db):
    ...
    detail = await promotion_repository.get_by_id(db, promo["id"], with_deleted=True)
...
async def test_get_page_filter(db):
    ...
    await _make_promo(
        db, "test_promo_pg2", discount_type="fixed", discount_value=500, promo_type="discount"
    )
