...
async def _create_pkg(db, data: dict) -> SysPackage:
    ...
    pkg = await package_repository.get_by_name(db, data["name"])
...
def _promo_payload(name: str, start, end, extra: dict | None = None) -> dict:
    base = {
        ...
        "startTime": start,
        "endTime": end,
        "activityRules": extra.pop(
            "activityRules", {"tiers": [{"threshold": 5000, "faceValue": 500}]}
        )
        if extra
        else {"tiers": [{"threshold": 5000, "faceValue": 500}]},
        "newUserOnly": 0,
        "status": 1,
    }
    if extra:
...
async def _bind_promo(
    db, pkg_id: int, discount_type: str, discount_value: int, tiers: list | None = None
):
    from datetime import datetime, timedelta
    ...
    now = datetime.now()
    promo = await promotion_service.create(
        db,
        PromotionForm(
            **_promo_payload(
...
async def test_create_duplicate_with_deleted(db):
    ...
    recreated = await package_repository.get_by_name(db, "test_pkg_dup_soft")
    assert recreated is not None
    assert recreated.id != dup.id
...
async def test_get_page_level_name_filled(db):
    vip = await _create_pkg(
        db,
        {
            "name": "test_pkg_vip_gpl",
            "packageType": "vip",
            "levelCode": "level_1",
            "period": "monthly",
            "periodDays": 30,
    ...
    result = await package_service.get_page(db, {"pageNum": 1, "pageSize": 100})
    rows = {r["id"]: r for r in result["list"]}
    assert rows[vip.id]["levelName"] == "VIP1"
    credit_row = next(r for r in result["list"] if r["packageType"] == "credit")
    assert credit_row["levelName"] == ""


async def test_create_invalid_type_rejected_before_name_dupe(db):
    """非法 packageType 应报参数错误，而非被名称查重抢先误报占用"""
    with pytest.raises(BusinessException) as exc:
        await package_service.create(
            db,
            {
                "name": "test_pkg_vip_ok",  # 与已有套餐同名
                "packageType": "unknown",
                "originalPrice": 5000,
                "salePrice": 4000,
            },
...
async def test_off_shelf_after_promotion_ended(db):
    ...
    await package_service.update_status(db, vip.id, 0)
    reloaded_pkg = await package_repository.get_by_id(db, vip.id)
    assert reloaded_pkg is not None
    assert reloaded_pkg.status == 0
...
async def test_delete_no_orders(db):
    ...
    await package_service.delete_by_ids(db, [vip.id])
    deleted_pkg = await package_repository.get_by_id(db, vip.id, with_deleted=True)
    assert deleted_pkg is not None
    assert deleted_pkg.deleted != 0
...
async def test_calc_full_reduction_below_threshold(db):
    ...
    await _bind_promo(
        db, vip.id, "full_reduction", 0, tiers=[{"threshold": 10000, "faceValue": 500}]
    )
    await package_service.update_status(db, vip.id, 1)
...
async def test_calc_coupon_not_applicable(db):
    ...
    await package_service.update_status(db, vip.id, 1)
    with pytest.raises(BusinessException) as exc:
        await package_service.calculate_price(db, vip.id, received["userCouponId"], user_id=USER_ID)
...
async def test_calc_coupon_template_disabled(db):
    ...
    await package_service.update_status(db, vip.id, 1)
    with pytest.raises(BusinessException) as exc:
        await package_service.calculate_price(db, vip.id, user_coupon.id, user_id=USER_ID)
...
async def test_get_sales_stats_type_group(db):
    ...
    db.add(
        SysOrder(
            order_no="test_stat_c1",
            user_id=USER_ID,
            package_id=credit.id,
            package_level="",
            package_name=credit.name,
            package_type="credit",
            original_price=4000,
            payable_amount=4000,
            paid_amount=4000,
...
# ===================== 权益覆盖 benefit_overrides =====================

# level_1 种子权益：monthly_dehaze_quota=100 / hd_export=1 / priority=2
_LEVEL1_BASE_QUOTA = 100
_LEVEL1_PRIORITY = 2


async def test_benefit_overrides_effective_in_detail(db):
    # T-PM-010/014：配额类覆盖取 max(等级权益, 覆盖值)，功能开关类以覆盖值为准，未覆盖项继承
    vip = await _create_pkg(
        db,
        {
            "name": "test_pkg_vip_bo",
            "packageType": "vip",
            "levelCode": "level_1",
            "period": "monthly",
            "periodDays": 30,
            "originalPrice": 10000,
            "salePrice": 8000,
            "benefitOverrides": {"monthlyDehazeQuota": 50, "hdExport": 0},
        },
    )
    await package_service.update_status(db, vip.id, 1)
    detail = await package_service.get_detail(db, vip.id)
    # 覆盖值低于等级权益时取等级权益（max 语义）
    assert detail["benefits"]["monthlyDehazeQuota"] == _LEVEL1_BASE_QUOTA
    assert detail["benefits"]["hdExport"] == 0
    assert detail["benefits"]["priority"] == _LEVEL1_PRIORITY

    higher = await _create_pkg(
        db,
        {
            "name": "test_pkg_vip_bo_high",
            "packageType": "vip",
            "levelCode": "level_1",
            "period": "monthly",
            "periodDays": 30,
            "originalPrice": 10000,
            "salePrice": 8000,
            "benefitOverrides": {"monthlyDehazeQuota": 150},
        },
    )
    await package_service.update_status(db, higher.id, 1)
    detail_high = await package_service.get_detail(db, higher.id)
    # 覆盖值高于等级权益时取覆盖值
    assert detail_high["benefits"]["monthlyDehazeQuota"] == 150


async def test_benefit_overrides_returned_in_form(db):
    overrides = {"monthlyDehazeQuota": 66, "batchLimit": 99}
    vip = await _create_pkg(
        db,
        {
            "name": "test_pkg_vip_bf",
            "packageType": "vip",
            "levelCode": "level_1",
            "period": "monthly",
            "periodDays": 30,
            "originalPrice": 10000,
            "salePrice": 8000,
            "benefitOverrides": overrides,
        },
    )
    form = await package_service.get_form(db, vip.id)
    assert form["benefitOverrides"] == overrides


async def test_update_benefit_overrides(db):
    vip = await _create_pkg(
        db,
        {
            "name": "test_pkg_vip_bu",
            "packageType": "vip",
            "levelCode": "level_1",
            "period": "monthly",
            "periodDays": 30,
            "originalPrice": 10000,
            "salePrice": 8000,
            "benefitOverrides": {"monthlyDehazeQuota": 50},
        },
    )
    await package_service.update(
        db,
        vip.id,
        {
            ...
            "originalPrice": 10000,
            "salePrice": 8000,
            "benefitOverrides": {"monthlyDehazeQuota": 88},
        },
    )
    form = await package_service.get_form(db, vip.id)
    assert form["benefitOverrides"]["monthlyDehazeQuota"] == 88


async def test_credit_package_has_no_benefits(db):
    # 积分卡无等级/权益，benefits 为空 dict
    credit = await _create_pkg(
        db,
        {
            "name": "test_pkg_credit_nb",
            "packageType": "credit",
            "creditAmount": 1000,
            "originalPrice": 5000,
            "salePrice": 4000,
    ...
    await package_service.update_status(db, credit.id, 1)
    detail = await package_service.get_detail(db, credit.id)
    assert detail["benefits"] == {}


async def test_create_price_stored_in_cents(db):
    # T-PM-022：价格以分（整数）存储，非浮点
    vip = await _create_pkg(
        db,
        {
            "name": "test_pkg_vip_cents",
            "packageType": "vip",
            "levelCode": "level_1",
            "period": "monthly",
            "periodDays": 30,
    ...
    assert vip.original_price == 9600
    assert isinstance(vip.original_price, int)
    assert isinstance(vip.sale_price, int)


# ===================== 上下架状态流转不变量 =====================


async def test_status_cycle_onsale_visibility(db):
    # T-PM-037/038：上架后用户端可见、下架后不可见，双向流转
    vip = await _create_pkg(
        db,
        {
            "name": "test_pkg_vip_sc",
            "packageType": "vip",
            "levelCode": "level_1",
            "period": "monthly",
            "periodDays": 30,
    ...
    assert vip.id not in [p["id"] for p in await package_service.list_on_sale(db, "vip")]
    await package_service.update_status(db, vip.id, 1)
    assert vip.id in [p["id"] for p in await package_service.list_on_sale(db, "vip")]
    await package_service.update_status(db, vip.id, 0)
    assert vip.id not in [p["id"] for p in await package_service.list_on_sale(db, "vip")]
...
async def test_update_status_not_found(db):
    with pytest.raises(BusinessException) as exc:
        await package_service.update_status(db, 999999999, 1)
    assert exc.value.code == ResultCode.PACKAGE_NOT_FOUND
...
async def test_calc_fixed_discount_exceeds_price(db):
    # 折扣金额超过售价时应付金额钳制为 0，不允许为负
    vip = await _create_pkg(
        db,
        {
            "name": "test_pkg_vip_fneg",
            "packageType": "vip",
            "levelCode": "level_1",
            "period": "monthly",
            "periodDays": 30,
    ...
    await _bind_promo(db, vip.id, "fixed", 20000)
    await package_service.update_status(db, vip.id, 1)
    result = await package_service.calculate_price(db, vip.id, None, user_id=USER_ID)
    assert result["payableAmount"] == 0


async def test_calc_coupon_amount_capped_at_zero(db):
    # 无门槛券面值超过折后价 → 应付 0
    from app.models.entity.sys_coupon import SysCoupon
    from app.repository.coupon_repository import coupon_repository
    from app.service.coupon_service import coupon_service
...
async def test_calc_full_reduction_coupon_below_threshold(db):
    # T-PM-052 边界：订单金额未达满减券门槛 → 券不抵扣
    from app.models.entity.sys_coupon import SysCoupon
    from app.repository.coupon_repository import coupon_repository
    from app.service.coupon_service import coupon_service
    ...
    assert result["couponAmount"] == 0
    assert result["payableAmount"] == 8000


async def test_calc_coupon_expired(db):
    from app.models.entity.sys_coupon import SysCoupon
    from app.repository.coupon_repository import coupon_repository, user_coupon_repository
    from app.service.coupon_service import coupon_service
    ...
    uc = await user_coupon_repository.get_by_id(db, received["userCouponId"])
    assert uc is not None
    uc.expire_time = datetime.now() - timedelta(days=1)
    await db.flush()
    await package_service.update_status(db, vip.id, 1)
    with pytest.raises(BusinessException) as exc:
        await package_service.calculate_price(db, vip.id, received["userCouponId"], user_id=USER_ID)
...
async def test_calc_coupon_of_other_user_rejected(db):
    # T-PM-060：他人优惠券拒绝，且不暴露归属（返回"优惠券不存在"）
    from app.models.entity.sys_coupon import SysCoupon
    from app.repository.coupon_repository import coupon_repository
    from app.service.coupon_service import coupon_service
    ...
    await coupon_repository.create(db, coupon)
    other_received = await coupon_service.receive(db, coupon.id, USER_ID + 1)
    await package_service.update_status(db, vip.id, 1)
    with pytest.raises(BusinessException) as exc:
        await package_service.calculate_price(
            db, vip.id, other_received["userCouponId"], user_id=USER_ID
        )
    assert exc.value.code == ResultCode.COUPON_NOT_FOUND


async def test_calc_new_user_only_passes_for_new_user(db):
    # T-PM-084：无历史付费订单的用户可购买新用户专享
    vip = await _create_pkg(
        db,
        {
            "name": "test_pkg_vip_nu",
            "packageType": "vip",
            "levelCode": "level_1",
            "period": "monthly",
            "periodDays": 30,
    ...
    await package_service.update_status(db, vip.id, 1)
    result = await package_service.calculate_price(db, vip.id, None, user_id=USER_ID + 7)
    assert result["payableAmount"] == 7200
