...
class TestSnapshotContract:
    async def test_snapshot_contains_config_and_resolved_config(self, env):
        db, redis, _ = env
        snap = await agent_version_service._build_snapshot(db, redis, _agent())
...
class TestVersionFlow:
    async def test_save_draft_writes_status_1(self, env):
        db, redis, _ = env
        result = await agent_version_service.save_draft(db, redis, 1, 1, "draft note")
        assert result.status == 1
        assert db.added

    async def test_publish_creates_published_and_demotes_old(self, env):
        db, redis, calls = env
        vno = await agent_version_service.publish(db, redis, 1, 1, "release")
    ...
    async def test_publish_gate_failed_raises(self, env, monkeypatch):
        ...
        with pytest.raises(BusinessException) as exc:
            await agent_version_service.publish(db, redis, 1, 1, "x")
    ...
    async def test_rollback_restores_and_creates_new_published(self, env, monkeypatch):
        ...
        vno = await agent_version_service.rollback(db, redis, 1, 2, 1)
    ...
    async def test_rollback_restores_config_and_prompt(self, env, monkeypatch):
        ...
        await agent_version_service.rollback(db, redis, 1, 2, 1)
    ...
    async def test_publish_only_one_published(self, env):
        db, redis, calls = env
        await agent_version_service.publish(db, redis, 1, 1, "v1")
        await agent_version_service.publish(db, redis, 1, 1, "v2")
