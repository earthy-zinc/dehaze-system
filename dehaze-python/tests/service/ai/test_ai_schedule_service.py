...
from app.service.ai import ai_schedule_service as m
...
def env(monkeypatch):
    ...
    run = _RunRepo()
    members = {}
    monkeypatch.setattr(m, "ai_schedule_repository", sched)
    monkeypatch.setattr(m, "ai_schedule_run_repository", run)
    monkeypatch.setattr(m, "member_repository", _member_repo(members))
    return SimpleNamespace(db=StubAsyncSession(), sched=sched, run=run, members=members)
...
@pytest.fixture
def freq_env(monkeypatch):
    sched = _ScheduleRepo()
    monkeypatch.setattr(m, "ai_schedule_repository", sched)
    monkeypatch.setattr(
        m, "member_repository", _member_repo({1: make_member("level_2")})
    )
    return SimpleNamespace(db=StubAsyncSession(), sched=sched)
