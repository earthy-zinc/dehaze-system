...
from app.service.ai import async_resume
...
async def test_notify_completed_resumes(mock_redis, monkeypatch):
    ...
    monkeypatch.setattr("app.service.ai.reasoning_service.reasoning_service", rs)
    monkeypatch.setattr("app.service.ai.async_resume.sse_emitter_manager", emitter)
...
async def test_notify_releases_lock_on_failure(mock_redis, monkeypatch):
    ...
    emitter = _Emitter(acquire=True)
    monkeypatch.setattr("app.service.ai.reasoning_service.reasoning_service", _FailingRS())
    monkeypatch.setattr("app.service.ai.async_resume.sse_emitter_manager", emitter)
