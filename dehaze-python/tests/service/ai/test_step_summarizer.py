...
from app.service.ai.step_summarizer import _generate_summaries, summarize_steps
...
async def test_generate_summaries_batch(mock_redis):
    with patch(
        "app.service.ai.step_summarizer.llm_client.stream_chat",
...
async def test_summarize_steps_updates_thoughts(mock_redis):
    thoughts = [
    ...
    with (
        patch("app.service.ai.step_summarizer.ai_agent_thought_repository", _repo(thoughts, updated)),
        patch(
            "app.service.ai.step_summarizer.llm_client.stream_chat",
            side_effect=_llm_stream(LLMChunk("text_delta", json.dumps(["s1", "s2"])), LLMChunk("done")),
        ),
        patch("app.database.get_db_session", Mock(return_value=NullDBSession())),
    ):
        await summarize_steps(1, "model-x")
    assert sorted(u[1] for u in updated) == ["s1", "s2"]


async def test_summarize_steps_llm_failure_silent(mock_redis):
...
async def test_summarize_steps_no_thoughts_no_call(mock_redis):
    with (
        patch(
            "app.service.ai.step_summarizer.ai_agent_thought_repository",
            SimpleNamespace(list_by_message=AsyncMock(return_value=[])),
        ),
        patch("app.service.ai.step_summarizer.llm_client.stream_chat") as stream,
        patch("app.database.get_db_session", Mock(return_value=NullDBSession())),
    ):
        await summarize_steps(1, "model-x")
