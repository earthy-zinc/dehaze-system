...
from app.service.ai.service import memory_es_service, memory_extraction
...
def _patch_extraction(monkeypatch, payload: str):
    ...
    async def fake_score(db, model_id, content, user_id):
        return 50
    ...
    monkeypatch.setattr(memory_extraction.ai_memory_repository, "get_active_by_user", no_existing)
...
class TestEmbeddingConfig:
    ...
    async def test_embedding_config_fallback_on_error(self, monkeypatch):
        ...
        monkeypatch.setattr(memory_es_service.dict_repository, "list_enabled_by_type_code", _boom)
        config = await memory_es_service._load_embedding_config()
...
class TestExtractMemoriesParsing:
    async def test_semantic_metadata_and_type_mapping(self, db, monkeypatch):
        payload = (
            '[{"type":"semantic","content":"用户偏好简洁回复",'
    ...
    async def test_procedural_metadata(self, db, monkeypatch):
        payload = (
            '[{"type":"procedural","content":"先去雾再评估",'
    ...
    async def test_pii_content_masked_in_output(self, db, monkeypatch):
        payload = '[{"type":"semantic","content":"手机：13800138000，请尽快联系我","metadata":{}}]'
        out = await _extract(db, monkeypatch, payload)
...
class TestSaveMetadata:
    def test_metadata_passed_to_entity(self):
        ...
        assert "metadata" in SysAiMemory.__table__.columns
        assert "delete_time" in SysAiMemory.__table__.columns
