"""本地 TTS（Piper 引擎 + TtsService）单元测试
...
def test_validate_params_rejects_bad_input():
    ...
    for text, voice, fmt, rate in cases:
        with pytest.raises(BusinessException):
            TtsService._validate_params(text, voice, fmt, rate)
...
def test_validate_params_rejects_overlong_text(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(BusinessException):
        TtsService._validate_params("超过五个字的文本", "huayan", "mp3", 16000)
    # 边界内通过
    TtsService._validate_params("五个字内", "huayan", "mp3", 16000)
...
def test_validate_params_accepts_catalog_voices():
    ...
    for v in VOICE_CATALOG:
        TtsService._validate_params("文本", v["id"], "wav", 8000)
...
def _install_service_stubs(monkeypatch: pytest.MonkeyPatch, synth_calls: list[str]):
    """桩掉文件存储与计费，仅考察 TtsService 缓存编排；合成走真实引擎"""
    ...
    monkeypatch.setattr(m.FileService, "upload_file", staticmethod(_upload_file))
    monkeypatch.setattr(m.VoiceBillingService, "ensure_balance", staticmethod(_ensure_balance))
    monkeypatch.setattr(m.VoiceBillingService, "charge_tts", staticmethod(_charge_tts))
...
async def test_synthesize_caches_second_call(redis: FakeAsyncRedis, monkeypatch):
    ...
    first = await TtsService.synthesize(
    ...
    calls.clear()
    second = await TtsService.synthesize(
        None, redis, 1, "缓存命中测试", "huayan", 1.0, "wav", 16000
    )
    ...
    # 语速不同 → 缓存 Key 不同 → 重新合成
    third = await TtsService.synthesize(
...
async def test_synthesize_default_voice_applied(redis: FakeAsyncRedis, monkeypatch):
    ...
    _install_service_stubs(monkeypatch, calls)
    # voice=None → 使用默认音色配置
    result = await TtsService.synthesize(None, redis, 1, "默认音色", None, 1.0, "mp3", 16000)
...
async def test_synthesize_engine_error_wrapped(redis: FakeAsyncRedis, monkeypatch):
    ...
    with pytest.raises(BusinessException):
        await TtsService.synthesize(None, redis, 1, "触发故障", "huayan", 1.0, "wav", 16000)
    assert calls == ["check"]  # 失败不存储不扣费


async def test_synthesize_rejects_unknown_voice_without_synth(redis: FakeAsyncRedis):
    with pytest.raises(BusinessException):
        await TtsService.synthesize(None, redis, 1, "文本", "aixia", 1.0, "wav", 16000)
