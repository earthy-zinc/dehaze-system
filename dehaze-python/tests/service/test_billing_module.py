...
class TestRateCalculate:
    ...
    async def test_calculate_delegates_to_price_service(self):
        svc = self._rate_svc({"credits": 500, "credits_saved": 100, "configured": True})
        calc = await svc.calculate(None, "gpt-4o", 1, 1000, 500, 200)
        assert calc["credits"] == 500
        assert calc["credits_saved"] == 100

    async def test_free_zero_price_not_raises(self):
        svc = self._rate_svc({"credits": 0, "credits_saved": 0, "configured": True})
        calc = await svc.calculate(None, "qwen3-0.6b", 77, 1000, 500, 200)
        assert calc["credits"] == 0

    async def test_model_without_price_raises(self):
        svc = self._rate_svc({"credits": 0, "credits_saved": 0, "configured": False})
        with pytest.raises(BusinessException) as exc:
            await svc.calculate(None, "unknown", None, 1000, 500, 200)
        assert exc.value.code == ResultCode.AI_MODEL_NOT_AVAILABLE

    async def test_rate_cache_hit(self, mock_redis):
        await mock_redis.mset(
            {
...
class TestQuotaService:
    async def test_check_quota_daily_exceeded(self, mock_redis):
...
class TestBalanceService:
    ...
    async def test_deduct_overdraw_marks_arrears(self, monkeypatch, mock_redis):
        await mock_redis.mset({"ai:balance:1": "100"})
        cas = {"amount": None}
    ...
    async def test_increase_clears_arrears(self, monkeypatch, mock_redis):
        await mock_redis.mset({"ai:balance:1": "0", "ai:arrears:1": "1"})
        logs = []

        async def _create_log(db, **kwargs):
            logs.append(kwargs)

        monkeypatch.setattr(bm.balance_service, "_increase_cas", AsyncMock(return_value=None))
        from app.repository.ai_credit_log_repository import ai_credit_log_repository as log_repo
...
class TestPreCharge:
    ...
    async def test_quota_fail_records_anomaly(self, monkeypatch):
        self._patch_base(monkeypatch)
        monkeypatch.setattr(bs.quota_service, "check_quota", AsyncMock(return_value=False))
        fail_calls = []

        async def _fail(db, uid):
    ...
    async def test_balance_prededuct_fail_rolls_back_quota(self, monkeypatch):
        self._patch_base(monkeypatch)
        monkeypatch.setattr(bs.balance_service, "pre_deduct", AsyncMock(return_value=False))
        refunds = []
...
class TestSettleDiff:
    ...
    def _patch_settle(monkeypatch, actual_credits, billing=None):
        ...
        class _Anomaly:
            async def check(self, db, uid, record, monthly_limit=0, daily_limit=0):
                return None
...
class TestAnomalyRules:
    ...
    async def test_single_high_triggers(self, db, mock_redis):
        await am.billing_anomaly_service.check(
            db, 1, self._record(credits=15000), monthly_limit=100000
        )
    ...
    async def test_burst_peak_triggers(self, db, mock_redis):
        ...
        rows = await self._all_anomalies(db)
        assert len(rows) == 1
        assert rows[0].anomaly_type == "burst"

    async def test_empty_reply_high_cost_triggers(self, db, mock_redis):
        await am.billing_anomaly_service.check(
            db, 1, self._record(input_tokens=15000, output_tokens=0)
        )
        assert int(await mock_redis.get("ai:anomaly:count:empty_high_output:1")) == 1
        rows = await self._all_anomalies(db)
        assert len(rows) == 1
        assert rows[0].anomaly_type == "empty_high_output"
    ...
    async def test_normal_usage_no_alert(self, db, mock_redis):
        ...
        assert not keys
        assert await self._all_anomalies(db) == []

    async def test_consecutive_quota_fail_alert(self, db, mock_redis):
        for _ in range(10):
            await am.billing_anomaly_service.record_quota_fail(db, 1)
        assert int(await mock_redis.get("ai:anomaly:count:consecutive_quota_fail:1")) == 1
        rows = await self._all_anomalies(db)
        assert len(rows) == 1
        assert rows[0].anomaly_type == "consecutive_quota_fail"
        assert rows[0].billing_id is None

    async def test_redis_unavailable_safe(self, db, monkeypatch):
        async def _broken():
...
class TestRefundService:
    ...
    async def test_audit_refund_approve_increases_balance(self, db, monkeypatch):
        ...
        result = await rf.refund_service.audit_refund(db, 9, True, "同意", 2)
        assert result.status == 2
        assert result.auditor_id == 2
        assert result.audit_remark == "同意"
...
class TestBillService:
    ...
    def _patch_bill(monkeypatch, by_type, by_source, balances):
        monkeypatch.setattr(
            ...
            AsyncMock(return_value=by_type),
        )
        monkeypatch.setattr(
            blm.ai_credit_log_repository,
            "sum_amount_by_user_and_source",
            AsyncMock(return_value=by_source),
        )
        monkeypatch.setattr(
            blm.ai_credit_log_repository,
...
class TestRegisterTrialCredits:
    async def test_register_grants_trial_credits(self, monkeypatch, mock_redis):
        import app.repository.member_repository as mem_mod
        ...
        monkeypatch.setattr(user_repo, "check_username_exists", AsyncMock(return_value=False))
        monkeypatch.setattr(
            role_repo,
            "get_enabled_by_code",
            AsyncMock(return_value=SimpleNamespace(id=7, data_scope=0)),
        )

        await mock_redis.set(f"{settings.CAPTCHA_KEY_PREFIX}test-key", "abcd")
