...
def _form(name: str) -> McpServerCreate:
    return McpServerCreate(name=name, endpoint="https://example.com/mcp")


class TestCredentialConfigured:
    async def test_new_server_not_configured(self, db):
        result = await AiMcpServerService().create_server(db, _form("cred-new"))
    ...
    async def test_credentials_update_marks_configured(self, db):
        ...
        created = await svc.create_server(db, _form("cred-set"))
        await svc.update_credentials(db, created.id, McpCredentialForm(api_key="sk-test"))
    ...
    async def test_extra_only_credentials_mark_configured(self, db):
        svc = AiMcpServerService()
        created = await svc.create_server(db, _form("cred-extra"))
        await svc.update_credentials(db, created.id, McpCredentialForm(extra={"token": "abc"}))
        result = await svc.get_server(db, created.id)
        assert result.credential_configured is True
