...
async def test_list_traces_filters(db):
    ...
    await ai_trace_repository.insert_idempotent(db, _trace_values("tr-a", conv.id, status=1))
    ...
    items, total = await ai_trace_repository.list_traces(db, end_time=datetime(2021, 1, 1))
...
async def test_list_abnormal_conversation_ids(db):
    ...
    await ai_trace_repository.insert_idempotent(db, _trace_values("tr-ok", conv_ok.id, status=1))
    await ai_trace_repository.insert_idempotent(db, _trace_values("tr-bad1", conv_bad.id, status=2))
    await ai_trace_repository.insert_idempotent(db, _trace_values("tr-bad2", conv_bad.id, status=4))
