...
class TestRoleService:
    ...
    def test_delete_roles(self, db_session):
        ...
        # 验证角色已被标记为删除（直接从数据库查询，不通过Service方法）
        deleted_role = db_session.get(SysRole, role.id)
        assert deleted_role is not None  # 对象仍存在
        assert deleted_role.deleted == 1  # 但标记为已删除
