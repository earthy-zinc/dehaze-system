...
3.  现有基于深度学习的图像去雾方法泛化能力差，在真实雾霾图像上的表现远低于合成雾霾图像的问题。设计和之间存在的
4.
...
基于ViT-VQGAN模型从高清无雾图像中预训练一个高质量码本作为先验知识，用于替换雾霾带来的不利影响。

### 优化模型结构

使用TaylorFormer中多尺度注意力细化模块构建去雾模型的编码器结构

### 设计匹配编码器输出特征和码本编码的距离

针对高质量码本中编码与编码器输出的离散编码数据分布不一致的情况，通过一个可控的匹配操作来计算编码器输出特征和码本编码的距离。
...
2.  设计了基于TaylorFormer的多尺度注意力细化模型用于图像去雾
