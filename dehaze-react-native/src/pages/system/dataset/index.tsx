...
type Props = NativeStackScreenProps<ProfileStackParamList, 'SystemDataset'>;

const PAGE_SIZE = 15;

const SystemDatasetScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const fetchList = useCallback(async (pn: number) => {
    try {
      const res = await DatasetAPI.getList({ pageNum: pn, pageSize: PAGE_SIZE, keyword: keyword || undefined });
  ...
  const handleDelete = (item: Dataset) => {
    Alert.alert('确认删除', `确定要删除数据集"${item.name}"吗？`, [
      { text: '取消', style: 'cancel' },
      { text: '确定', style: 'destructive', onPress: async () => {
        try { await DatasetAPI.deleteById(item.id); fetchList(1); } catch { Alert.alert('错误', '删除失败'); }
      }},
    ]);
  };
  ...
  const renderItem = ({ item }: { item: Dataset }) => (
    <View style={styles.card}>
      <View style={styles.cardContent}>
        ...
        <Text style={styles.cardMeta}>{item.type} · 图片: {item.total ?? 0} · {item.status === 1 ? '启用' : '禁用'}</Text>
      </View>
      <View style={styles.cardActions}>
        {hasPerm('sys:dataset:edit') && (
          <TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('SystemDatasetForm', { datasetId: item.id })}>
            <Ionicons name="create-outline" size={16} color={theme.colors.primary} />
          </TouchableOpacity>
        )}
        {hasPerm('sys:dataset:delete') && (
      ...
      <View style={styles.container}>
        <View style={styles.searchBar}>
          <View style={styles.searchInputWrap}>
            ...
            <TextInput style={styles.searchInput} placeholder="搜索数据集" placeholderTextColor={theme.colors.text.tertiary} value={keyword} onChangeText={setKeyword} returnKeyType="search" />
          </View>
          {hasPerm('sys:dataset:add') && (
            <TouchableOpacity style={styles.addBtn} onPress={() => navigation.navigate('SystemDatasetForm', {})}>
        ...
        <FlatList
          ...
          ListEmptyComponent={!loading ? <View style={styles.empty}><Ionicons name="folder-open-outline" size={48} color={theme.colors.text.tertiary} /><Text style={styles.emptyText}>暂无数据集</Text></View> : null}
        />
      </View>
    </View>
  );
...
export default SystemDatasetScreen;
