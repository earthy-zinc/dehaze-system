...
import type { DictTypePageVO } from 'dehaze-sdk-js'

type Props = NativeStackScreenProps<ProfileStackParamList, 'SystemDict'>;

const PAGE_SIZE = 15;

const SystemDictScreen: React.FC<Props> = ({ navigation }) => {
  const hasPerm = useCallback((p: string) => (useAuthStore.getState().userInfo?.perms ?? []).includes(p), []);

  const [list, setList] = useState<DictTypePageVO[]>([]);
  ...
  const fetchList = useCallback(async (pn: number) => {
    try {
      const res = await DictAPI.getDictTypePage({ pageNum: pn, pageSize: PAGE_SIZE, keywords: keyword || undefined });
  ...
  const handleRefresh = async () => { setRefreshing(true); await fetchList(1); setRefreshing(false); };
  const handleLoadMore = async () => {
    if (loadingMore || list.length >= total) return;
    setLoadingMore(true); await fetchList(pageNum + 1); setLoadingMore(false);
  };

  const handleDelete = (item: DictTypePageVO) => {
    Alert.alert('确认删除', `确定要删除字典类型"${item.name}"吗？`, [
      { text: '取消', style: 'cancel' },
      { text: '确定', style: 'destructive', onPress: async () => {
        try { await DictAPI.deleteDictTypes(String(item.id)); fetchList(1); } catch { Alert.alert('错误', '删除失败'); }
      }},
    ]);
  };

  const renderItem = ({ item }: { item: DictTypePageVO }) => (
    <TouchableOpacity style={styles.card} activeOpacity={0.7} onPress={() => navigation.navigate('SystemDictItem', { typeCode: item.code, typeName: item.name })}>
      ...
      <View style={styles.cardActions}>
        {hasPerm('sys:dict:edit') && (
          <TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('SystemDictTypeForm', { dictTypeId: item.id })}>
  ...
  return (
    <View style={styles.container}>
      <AppHeader title="字典管理" showBack onBackPress={() => navigation.goBack()} />
      <View style={styles.container}>
        <View style={styles.searchBar}>
          <View style={styles.searchInputWrap}>
            <Ionicons name="search-outline" size={18} color={theme.colors.text.tertiary} />
            <TextInput style={styles.searchInput} placeholder="搜索字典类型" placeholderTextColor={theme.colors.text.tertiary} value={keyword} onChangeText={setKeyword} returnKeyType="search" />
          </View>
          {hasPerm('sys:dict:add') && (
            <TouchableOpacity style={styles.addBtn} onPress={() => navigation.navigate('SystemDictTypeForm', {})}>
        ...
        <FlatList
          ...
          ListEmptyComponent={!loading ? <View style={styles.empty}><Ionicons name="book-outline" size={48} color={theme.colors.text.tertiary} /><Text style={styles.emptyText}>暂无字典类型</Text></View> : null}
        />
      </View>
    </View>
  );
...
export default SystemDictScreen;
