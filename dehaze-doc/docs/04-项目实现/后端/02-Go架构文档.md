...
├── cmd/                        # 应用入口
│   └── main.go                 # 启动入口
├── .mockery.yaml               # mockery mock 生成配置（覆盖全部 wire 的 repository 接口）
├── Makefile                    # mocks / test / test-integration / cover targets
├── config/                     # 配置文件
│   ├── config.yaml             # 主配置（开发环境）
│   └── config.test.yaml        # 测试环境配置（MySQL dehaze_test / Redis db=4 / XXL-Job 禁用）
├── internal/                   # 内部业务代码（不对外暴露）
│   ├── api/                    # Controller 层（请求处理）
│   ├── service/                # Service 层（业务逻辑）
│   │   └── mocks/              # mockery 生成的 service 接口 mock
│   ├── repository/             # Repository 层（数据访问）
│   │   └── mocks/              # mockery 生成的 repository 接口 mock
│   ├── model/                  # 数据模型（sys_*.go / bo/ / dto/ / vo/ / query/ / read/ / enum/）
│   ├── router/                 # 路由注册
│   ├── testutil/               # 测试基建（真实 MySQL 测试库：指纹重建 + 事务回滚 + 池模式）
│   └── app/                    # 应用初始化、生命周期与依赖装配
└── pkg/                        # 可复用基础设施包
    ├── aiclient/               # AI 域 B 类端点转发（会话透传 / SSE 流式 / multipart）
...
测试以 `*_test.go` 形式分散在各包内（与源码同包，Go 惯例）。共享基建在 `internal/testutil`：`NewTestDB(t)` 连接真实 MySQL 测试库 `dehaze_test`（schema/种子同源根目录 `config/sql/`，跨进程 GET_LOCK + 指纹跳过重建），单事务 + Cleanup 回滚做数据隔离；`NewPoolTestDB(t)` 池化多连接供并发/CAS 用例；`LoadTestConfig(t)` 显式加载 `config/config.test.yaml`（不依赖 CWD 与 gin mode）供集成测试使用。Redis 单测用 miniredis（进程内真实协议），依赖接口一律 mockery 生成 mock（`make mocks` 重新生成）。分两层运行：默认 `go test ./...` 为单元测试；`go test -tags=integration ./...`（或 `make test-integration`）追加集成测试（真实 Redis db=4，模板见 `internal/service/auth/integration_test.go`，须用外部测试包 `auth_test` 规避同包 TestMain 链接冲突）。契约测试模板见 `internal/api/auth_controller_test.go`（httptest 验证 `{code,msg,data}` 信封）。规范详见 [Go后端测试规范化改造计划](../../05-改造计划/Go后端测试规范化改造计划.md)。
...
导出缺省分流规则（`async` 参数未传时）：`estimateCount > SYNC_THRESHOLD` 或 handler 声明 `UseDirectExport()`（如数据集 ZIP 打包导出）→ 创建异步任务返回 taskId；显式传 `async=false` 强制同步流式导出（direct handler 直接写 OutputStream）。三端行为一致。
...
FavoriteService 通过 GORM 模型 `SysFavorite` 映射 `sys_favorite` 表，支持 algorithm/result/dataset/image/preset 五类目标，容量按会员等级限制（普通 200 / VIP 500）。写入采用 `clause.OnConflict` 实现收藏幂等：取消后再次收藏时插入新行（唯一键含 `deleted`，软删行不占键位）；活跃行冲突时 `ON DUPLICATE KEY UPDATE` 幂等复位 `deleted=0`、`is_invalid=0`（详见 [数据库设计 §5.3.1](../../02-系统架构/03-数据库设计.md)）。收藏状态批量查询通过 `WHERE user_id=? AND target_type=? AND target_id IN (?)` 单次查询返回已收藏 ID 集合。
...
AI 域中强依赖 deepagents 推理、LLM 供应商、向量检索的端点（AI 对话流式推理与中断控制、Agent/Skill/Model/Provider 可用性测试、MCP 探测与工具试调用、智能体评测任务、定时任务手动触发、知识库**库级索引生命周期（创建/删除/索引状态）**与文档入库、召回调试、A2A 协议入口、OpenAI/Claude 兼容 API）在 Go 端不做业务实现，由 `pkg/aiclient` 原样转发给 dehaze-python（唯一行为事实源）。知识库库级三条（`POST /api/v1/kb`、`DELETE /api/v1/kb/:id`、`GET /api/v1/kb/:id/index-stats`）依赖 ES 索引生命周期（python `ensure_kb_index`/`delete_kb_index`/`_stats`），原生实现会出现「库建了但 ES 索引不存在」的缺口，故必须转发；库列表/详情/编辑与文档读、测试集、低质分块仍为 Go 原生（`internal/router/ai_kb.go`）。与 `pkg/algorithm` 的差异：不做幂等重试（POST 非幂等）、不解析信封（JSON 与 SSE 上游报文原样回传）。

| 能力 | 实现 | 要点 |
|------|------|------|
| `ForwardJSON` | `pkg/aiclient/client.go` | 请求体/响应体按上限（32MB）缓冲，状态码、响应头、响应体原样回传 |
| `ForwardSSE` | 同上 | 逐块回写 + `Flush()`；客户端断连（请求 context 取消）即时中断上游；空闲读超时（`ai.streamTimeout`，默认 300s）断开挂死的流；显式 `Accept-Encoding: identity` 避免压缩缓冲吞掉实时事件 |
| `ForwardMultipart` | 同上 | 请求体流式转发（不落盘、不整体读入内存），`Content-Type`（含 boundary）透传 |
| 身份透传 | `buildRequest` | 透传 `Authorization`/`Cookie`/`X-Session-Id`/`x-api-key`，python 复用同一 Redis 会话零改动鉴权；同时透传 `X-Trace-ID`/`traceparent`/`X-Forwarded-For`（按 Go 侧解析出的客户端 IP）/`User-Agent`，保证 python 的链路追踪、审计 IP 与按 IP 限流口径一致 |
| 不可达降级 | `writeUpstreamError` | Python 不可达时返回统一业务信封（`C0001`），不裸 500、不 panic |
| 路由白名单 | `internal/router/ai_proxy.go` | B 类端点集中声明，路径与 python 逐条对齐；未注册路径直接 404，不做兜底转发。路径参数统一用 `:id`（gin 同一路径位置只允许一个通配符名，跨模块共用节点时命名必须一致，否则启动 panic） |

OpenAI/Claude 兼容 API（`POST /api/v1/chat/completions`、`POST /api/v1/messages`、`GET /api/v1/models`）与 A2A 标准入口（`POST /a2a`、`GET /.well-known/agent.json`）注册在根引擎且**不经过 `AuthMiddleware`**：python 的 `ApiKeyAuthMiddleware` 是全局中间件，Claude 协议走 `x-api-key`、A2A 标准入口的鉴权只有 python 认，Go 侧先拦会一律 401。AI 域其余 A 类 CRUD 由 Go 原生实现。

> 归属边界：A2A 挂载路径 Agent Card（`GET /api/v1/ai/agents/{id}/a2a/.well-known/agent.json`）由 Go 原生实现（`internal/router/ai_a2a.go` + `internal/api/ai_a2a.go`，按已发布版本动态生成），**不得再进转发白名单**——同一「方法 + 路径」重复注册会让 gin 启动 panic（`ai_extra_routes_test.go` 守卫）；JSON-RPC 入口（`POST /api/v1/ai/agents/:id/a2a`、`POST /a2a`）与全局发现端点 `GET /.well-known/agent.json` 转发 python。

> 发布/回滚边界：`POST /api/v1/ai/agents/:id/publish` **转发 python**（发布门禁要跑真实回归评测——分钟级推理运行时，Go 侧无法本地执行；python `agent_version_service.publish` 内依次完成判分漂移门禁、固化草稿、回归门禁、写新已发布版本与 published 缓存失效）。`POST /api/v1/ai/agents/:id/versions/:versionNo/rollback` 保持 Go 原生（仅重放历史快照，无推理依赖）。
...
| 测试 | test | 真实 MySQL 测试库 dehaze_test（同实例）、Redis db=4 隔离、XXL-Job 禁用 |
...
**参数校验口径（A 类端点统一）**：可选参数**未传**取默认值；**传了但格式非法或越界一律 `A0400`**，不做静默回落默认、也不依赖 ORM 截断——分页 `pageNum ≥ 1`、`1 ≤ pageSize ≤ 100`（对齐 python `BasePageQuery`），可选区间参数（如 `status`、`limit`）逐项对齐 python 的 `ge/le`。注意：**GORM 对负 `Limit` 视为"取消 LIMIT"会返回全量**（python 的 SQLAlchemy 则直接报错），故负值必须在参数层拒绝，不能透传。

**已知跨端口径差异**：可选参数的**空串**在 Go/Java 视为"未传"（宽松），python 因 pydantic 无法解析空串返回 `A0400`；三端以"空串 = 不筛选"为既定宽松口径，待统一（与 [Java 架构文档](01-Java架构文档.md) 同项）。

**分页参数唯一实现**：Go 侧分页解析只有一处——`internal/api/sys_feedback.go` 的 `parsePaginationNamed(c, numKey, sizeKey, defaultSize)`（`parsePaginationWithSize` 固定 `pageNum/pageSize`；`parsePagination` 再固定 `defaultSize = 10`）。查询对象上的分页字段一律标 `form:"-"` **不参与 gin 绑定**（绑定层会把非数字兜底成 `B0001`、越界值无从表达 `A0400`），由 handler 解析后回填；service / repository **禁止再做静默钳制**——历史缺陷根因即为此：`sys_file.GetFilePage` 曾 `err == nil` 即采用（`pageSize=0/-1` 放行、负值让 `LIMIT` 消失），`sys_dataset` 的 repository 曾把 `pageSize <= 0` 静默改成 10。

**非 AI 域分页端点与守卫覆盖**（34 个端点，2026-09-18 盘点；守卫 `internal/api/pagination_sweep_test.go` 直接打真实 handler，每个端点 7 个拒绝用例断言 body `A0400` + 3 个放行用例断言"越过校验进入 service"）：

| `pageSize` 默认 | 端点（python 依据） |
|------|------|
| 20 | `favorite.GetPage`(favorite.py:26)、`message.GetPage`(message.py:23)、`message.Search`(message.py:46)、`messageTemplate.GetPage`(message_template.py:21)、`datasetItem.GetDatasetItems`(dataset_item.py:41) |
| 10 | 其余 29 个：`user.ListPagedUsers`、`role.GetRolePage`、`dict.GetDictPage`、`dict.GetDictTypePage`、`dataset.GetDatasetList`、`file.GetFilePage`、`order.ListMy/GetPage/ListRefunds`、`auth.ListLoginLogs`、`announcement.GetPage`、`promotion.GetPage`、`feedback.ListMyRatings/ListRatings/ListMyFeedback/ListFeedback`、`member.GetPage/GetGrowthLogs/GetMemberGrowthLogs/GetMemberConsumptionRecords/GetMemberOperationLogs`、`preset.ListPresets`、`prediction.ListPredictionLogs`、`task.GetTaskPage`、`package.GetPage/GetCouponPage`、`evaluation.GetMetrics/ListEvaluationLogs`、`inputHistory.ListHistory` |
| 无分页参数 | `algorithm.GetList`：python `GET /algorithms` 只声明 `keywords`，`pageNum/pageSize` 属未声明参数须静默忽略（Go 曾因绑定这两个字段而对 `pageNum=abc` 报 `A0400`，与 python 200 分叉，现已不参与绑定） |

默认值不可按"都是 10"推断：`parsePaginationWithSize(c, N)` 的 N 必须逐端点回到 python router 的 `Query(default=N)` 行号核实（系统模块为路由内联 `Query(...)`，AI 域 schema 继承 `BasePageQuery`，`grep 'Query('` 为空不等于无分页）。`limit`/`pageSize` 无上界是既定口径，不得"顺手"加界。
...
- **跨语言职责分工**：Go 负责业务编排与并发调度，预测/评估/指标计算委托 Python 算法服务，统一通过 `pkg/algorithm` 客户端（重试/熔断/幂等/traceID）调用；AI 域强依赖 deepagents/LLM/ES 的 B 类端点通过 `pkg/aiclient` 透传转发给 dehaze-python（唯一行为事实源），Go 侧不复制行为（见 §3.19）。
