# 部门管理模块 API 接口

## 1. 文档概述

本文档定义 **部门管理** 模块的 HTTP API 规范,是该模块 API 契约的**唯一权威来源**。

- **基础路径**:`/api/v1/dept`
- **公共约定**:参见 [02-系统架构/04-API规范.md](../../../02-系统架构/04-API规范.md)
- **需求规格**:[需求规格.md](./需求规格.md)

> **重要**:接口详细参数/响应结构可通过 API 文档 MCP 查询,本文档仅定义接口清单和权限标识。

## 2. 接口清单

| 路径 | 方法 | 功能描述 | 权限标识 | 关联功能点 |
|------|------|---------|---------|-----------|
| `/api/v1/dept` | GET | 获取部门列表(树形结构) | - | F-DM-001 |
| `/api/v1/dept` | POST | 新增部门 | `sys:dept:add` | F-DM-002 |
| `/api/v1/dept/{deptId}/form` | GET | 获取部门表单数据 | - | F-DM-003 |
| `/api/v1/dept/{deptId}` | PUT | 修改部门 | `sys:dept:edit` | F-DM-003 |
| `/api/v1/dept/{ids}` | DELETE | 删除部门(支持批量) | `sys:dept:delete` | F-DM-004 |
| `/api/v1/dept/options` | GET | 获取部门下拉选项(树形结构) | - | F-DM-008 |

## 3. 权限标识汇总

| 权限标识 | 说明 | 控制范围 |
|---------|------|---------|
| `sys:dept:add` | 新增部门 | 按钮显示 + 接口校验 |
| `sys:dept:edit` | 编辑部门 | 按钮显示 + 接口校验 |
| `sys:dept:delete` | 删除部门 | 按钮显示 + 接口校验 |

**权限说明**:
- 部门管理员的操作权限受数据权限控制,只能操作本部门及下级部门
- 删除部门的权限仅限系统管理员,确保组织架构的稳定性

## 4. 业务错误码

| 错误码 | 说明 | 触发场景 |
|--------|------|---------|
| `A0500` | 部门名称已存在 | 新增/编辑时同一层级下部门名称重复 |
| `A0501` | 部门不存在 | 编辑/删除不存在的部门 |
| `A0502` | 部门下存在用户,无法删除 | 删除部门时该部门或子部门下有关联用户 |
| `A0503` | 不能选择自身或其子部门作为上级部门 | 编辑部门上级部门时造成循环引用 |
| `A0504` | 部门层级超出限制 | 部门层级超过5级限制 |
| `A0505` | 根部门不可修改/删除 | 尝试修改/删除系统内置根部门 |

## 5. 接口详情查询

> 接口的详细请求参数、响应结构、Schema 定义可通过以下方式获取:
>
> 1. **API 文档 MCP**：调用 `read_project_oas_wht4eg` 获取 OpenAPI Spec
> 2. **Swagger UI**:访问 `/swagger-ui/index.html`(开发环境)
