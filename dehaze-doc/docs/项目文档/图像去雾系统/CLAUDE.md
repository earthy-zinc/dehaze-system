...
```bash
cd dehaze-java
mysql -u root -p < sql/schema.sql   # Database init (first time only)
mvn clean install
...
3. **Initialize database**: Run `sql/schema.sql` in the Java backend
...
- **Database init**: `dehaze-java/sql/schema.sql` (required before first startup)
...
1. `mysql -u root -p < dehaze-java/sql/schema.sql` (first time only)
