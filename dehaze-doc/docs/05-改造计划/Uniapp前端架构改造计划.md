...
3. 替换后移除一切 `u-*`/`up-*` 残留，并补一条 ESLint 规则禁止 `<u-`/`<up-` 前缀组件。
...
- `package.json` 声明了 `axios`、`vue-i18n`、`clipboard`、`dayjs`，但 `src/` 内均无 `import`（`axios` 经 SDK 间接使用，不应作为本项目直接依赖；`vue-i18n` 无 `createI18n` 调用）。

**改造**：移除未直接使用的依赖；若 i18n 规划中，移至改造计划追踪而非长期挂依赖。
