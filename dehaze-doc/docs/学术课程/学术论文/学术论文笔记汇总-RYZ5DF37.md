...
1.  目前基于真实雾霾的数据集较少，而神经网络的训练需要大量数据作为支撑。针对这个问题简单的方法是合并多个数据集进行网络的训练，但是由于每个数据集之间有一些细微的差异，例如颜色差异、物体复杂性、拍摄所用的相机差别等。有论文证明直接组合多个数据集反而会降低去雾的结果指标。设计一种数据预处理技术，来减少数据集之间的分布差距。
2.  针对当前图像去雾领域提出的各种人工先验知识的缺陷，如暗通道先验在天空和雪地上失效。由于人工设计的先验知识不能充分拟合所有雾霾场景。因此需要设计一种网络，本身能够通过无监督的方式训练获得无雾图像相关的高质量先验知识，随后将训练得到的先验知识用于去雾网络训练。
...
ViT-VQGAN模型整体包含一个编码器和解码器。编码器能够把图片压缩成较短向量。在训练时，输入图像会被编码器编码为一个较短的向量，再被解码器解码为为另一幅长得差不多的图像，网络学习的目标就是让重建出来的图像和原图像尽可能相似。换一个角度来说，解码器就是一个图像生成模型，在更早之前的模型<span style="color: rgb(44, 62, 80)"><span style="background-color: rgb(255, 255, 255)">自编码器（Autoencoder，AE）</span></span>中，编码器编码出来的向量空间是不完整的，也就是说，解码器只认识经过编码器编码出来的向量，而不认识其他向量。如果把自己随机生成的向量输入给解码器，解码器是不能生成有意义的图片，因此编码器不能够随机生成图片，只能起到图像压缩的作用。
...
通过离散化的特征输入解码器获得处理后的图像![\<img alt="" data-attachment-key="DN6MJTHC" width="168" height="59" src="attachments/DN6MJTHC.png" ztype="zimage">](attachments/DN6MJTHC.png)z<sub>q</sub>通过编码器和后续的逐元素离散的函数来逼近和他最接近的码本元素。
...
![\<img alt="" data-attachment-key="EFMIQY8D" width="842" height="80" src="attachments/EFMIQY8D.png" ztype="zimage">](attachments/EFMIQY8D.png)sg\[·]表示停止梯度stop-gradient。该损失函数主要是衡量编码器的输出e和离散向量z<sub>e</sub>之间产生的离散化损失。

先前的工作如 VQVAEs 通常由于码本初始化较差而导致码本使用率较低。因此训练过程中码本中有不少编码未被使用。因此码本中有效的编码数量较少，导致第一阶段图像量化器训练得到的图像重建效果变差，而第二阶段图像合成得到的多样性变差。ViT-VQGAN提出了两个改进能够在增加码本编码数量为8192时还能够显著提高码本中编码的利用率。
...
**L2正则化的编码** 编码后的隐变量ze ( x )和码本隐变量e进行了L2归一化。码本编码由正态分布初始化。通过将所有隐变量映射到球面上，L2-normalized潜在变量![\<img alt="" data-attachment-key="WSD2P43A" width="413" height="55" src="attachments/WSD2P43A.png" ztype="zimage">](attachments/WSD2P43A.png)演化为ze ( x )和e之间两个向量的余弦相似度，进一步提高了训练稳定性和重建质量。

针对码本的训练损失，我们采用在VGG网络的基础上使用了logit-laplace损失、l2损失、感知损失。在StyleGAN判别器上使用了GAN损失。通过超参数看来配置这几项损失所占权重。以优化图像重建质量、码本使用、FID和Inception Score。
...
![\<img alt="" data-attachment-key="A74T4ZRH" width="396" height="87" src="attachments/A74T4ZRH.png" ztype="zimage">](attachments/A74T4ZRH.png)最终我们选择该函数作为权重，接下来要解决的问题权重设置的大小问题。如果权重设置太小，会导致不必要的激活，而权重设置太大会无法激活该码本编码。因此我们需要一个超参数a来辅助控制权重的大小。

超参数a的取值。将编码器所得到的编码和清晰无雾图像的编码之间的差距用两个概率分布之间的差异来表示。这样，两个不同的域，无雾图像所在的领域和有雾图像所在的领**域适应**问题就转化为秋姐一个最优参数a，使得P<sub>clear</sub>( x = z<sub>k</sub>)和P<sub>haze</sub>( x = z<sub>k</sub>| α)前向库尔贝克-莱布勒散度最小
