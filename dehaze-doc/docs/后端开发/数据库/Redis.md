...
主要利用setnx命令。也就是set if not
exists如果不存在就成功设置缓存，否则返回0.在集群服务器中，定时任务可能在两台以上机器运行，所以想通过setnx设置一个分布式锁，如果成功则执行任务，如果没有成功上锁，则说明已经有一台机器正在执行任务。
...
接下来的String Hash List Set
Zset都是针对某个key的值而言的，换句话说，就是对某个key的值的数据结构进行操作。值的结构由单纯的字符串、也有进一步的键值对、列表、集合等。这都是针对某一特定key而言的，一个特定的key的值只能是某一种数据结构，而不能同时为多种。
...
哈希映射表是一个字符串类型的键值对。适合存放对象。每个键对应的值都是都是一个子键值对，键所对应的其中的一个值是由字段和其对应的值（field
value）组成的。而一个键可以有多个值。每一个值都是由上述的字段-值映射表构成的。
