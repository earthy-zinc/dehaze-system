...
| 模型类型 | 包路径 | 职责 | 示例 |
|----------|--------|------|------|
| **Entity** | `model/entity/` | 数据库表映射，GORM 标签 | `SysUser` |
...
| 组件 | 选型 | 说明 |
|------|------|------|
| ORM | GORM v2 | 自动迁移、Callback、Scopes |
...
| `afterQuery` | SELECT 后 | 数据权限过滤（DataScope） |

### 6.5 数据权限（DataScope）

基于 GORM Scopes 实现行级数据权限控制：
...
| 阶段 | 内容 |
|------|------|
| **Phase 1** | 部署 XXL-Job Admin（已完成），Go 端集成 xxl-job-executor-go SDK |
| **Phase 2** | 将现有 CleanupJob 迁移到 XXL-Job 管理 |
| **Phase 3** | 新增统计类、维护类定时任务直接注册到 XXL-Job |
...
flowchart LR
    ...
    ErrorHandler --> Prometheus["指标采集<br/>Prometheus"]
...
| **缓存** | go-redis + FreeCache | v9 | Redis 客户端 + 本地缓存 |
