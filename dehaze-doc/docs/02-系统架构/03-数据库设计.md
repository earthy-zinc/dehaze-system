...
数据库的建表语句和初始化数据存放在项目根目录 `config/sql/` 下，三个后端（dehaze-java / dehaze-go / dehaze-python）共享同一套 schema：

| 文件 | 说明 |
|------|------|
| `config/sql/schema.sql` | 全量建表语句（DROP + CREATE），用于全新初始化 |
| `config/sql/data.sql` | 初始化数据（菜单、角色、字典、部门等） |
| `config/sql/migration_20260711.sql` | 增量迁移脚本（算法版本表、输入历史表、算法审核字段） |
| `config/sql/sys_wpx_file_data.sql` | WPX 文件映射数据 |
...
) ENGINE=InnoDB CHARSET=utf8mb4 COMMENT='图像输入历史记录表';
```
...
| sys_input_history | idx_user_time | (user_id, create_time DESC) | 复合 | 用户历史按时间倒序查询 |
| sys_input_history | idx_user_favorite | (user_id, is_favorite, create_time DESC) | 复合 | 用户收藏历史查询 |
