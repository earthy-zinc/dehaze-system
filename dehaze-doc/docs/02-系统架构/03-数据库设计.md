...
| 表                         | 职责   | 关键设计决策                                                                              |
| ------------------------- | ---- | ----------------------------------------------------------------------------------- |
| `sys_ai_agent`            | 智能体配置 | Agent 主表（系统提示词、模型、推理范式、配置 JSON、类型标识、权限规则、护栏配置、`tags` 分类标签 JSON 数组），`agent_code` 唯一不可复用（§5.3.1 类别②），`is_exposed` 控制对外 A2A 暴露 |
...
| `sys_ai_agent_eval_dataset` | 评测集 | `dataset_type`（`dev`/`regression`/`heldout`）+ `agent_id`，开发集日常调试、回归集发布门禁、保留集阶段验收；逻辑删除（§5.3.1 类别①）；归 Agent 评测（F-M08-014） |
...
`sys_voice_provider`（语音引擎供应商）：`(provider_code, engine_type)` 为业务引用键，删除后不可复用（逻辑删除类别②，新建查重须绕过软删过滤查全表）。
...
`sys_voice_model`（ASR 模型 / TTS 音色注册）：`engine_type` 区分 asr/tts，`model_id` 标识模型或音色；`(model_id, provider_id)` 联合唯一（同一模型可切换引擎），删除后不可复用（类别②）。

| 字段 | 类型 | 默认 | 说明 |
| ---- | ---- | ---- | ---- |
| `id` | bigint | 自增 | 主键 |
| `provider_id` | bigint | - | 关联 `sys_voice_provider.id` |
| `model_id` | varchar(64) | - | 模型/音色业务编码（ASR：`sensevoice`/`paraformer`；TTS：`huayan`…），删除后不可复用 |
| `engine_type` | varchar(16) | - | 能力类型：`asr` / `tts` |
