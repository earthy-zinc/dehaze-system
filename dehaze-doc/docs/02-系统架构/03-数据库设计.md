...
| `result` | TEXT | 任务结果（导出任务为下载链接，导入任务为结果摘要 JSON） |
...
| task_type | 类别 | 说明 | params 结构 |
|-----------|------|------|------------|
| `dataset_export` | export | 数据集导出（ZIP 打包） | `{ "query": {...}, "format": "excel", "options": {...} }` |
| `user_export` | export | 用户导出 | `{ "query": {...}, "format": "excel\|csv", "fields": [...] }` |
| `role_export` | export | 角色导出 | 同上 |
| `dept_export` | export | 部门导出 | 同上 |
| `menu_export` | export | 菜单导出 | 同上 |
| `dict_export` | export | 字典导出 | 同上 |
| `algorithm_export` | export | 算法导出 | 同上 |
| `user_import` | import | 用户导入 | `{ "fileObjectName": "...", "mode": "all\|partial", "deptId": 1 }` |
| `role_import` | import | 角色导入 | 同上 |
| `dept_import` | import | 部门导入 | 同上 |
| `menu_import` | import | 菜单导入 | 同上 |
| `dict_import` | import | 字典导入 | 同上 |
| `algorithm_import` | import | 算法导入 | 同上 |

> **说明**：所有导入导出任务复用 `sys_task` 表，通过 `task_type` 区分模块和操作。导入任务 `result` 字段存储导入结果摘要 JSON（含 totalRows/successCount/failureCount/errors/errorReportObjectName）。
