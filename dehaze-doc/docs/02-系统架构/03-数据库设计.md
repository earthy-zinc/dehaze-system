...
AI 对话模块是 AI 工作平台的交互中枢，以通用大语言模型为底座，通过对话式统一入口调度图像处理、指标评估、算法选择等系统能力。数据模型参考 OpenAI Assistants API（Thread/Run/RunStep）、Dify（Conversation/Message/MessageAgentThought）、LangGraph（checkpoint 持久化）等主流实现，由 23 张业务表（其中 `sys_ai_billing`/`sys_ai_credit_log`/`sys_ai_refund` 归 AI 计费管理模块）+ LangGraph 框架管理的检查点表构成：
...
| `sys_ai_provider`         | 模型供应商 | 供应商配置（API 地址、协议类型、认证方式、健康检查开关、备注），`provider_code` 唯一不可复用（§5.3.1 类别②）；健康状态（健康/可疑/熔断）为运行时聚合，熔断标记走 Redis 不落库                          |
| `sys_ai_provider_key`     | 供应商 API Key | 加密存储（哈希查重+密文解密），优先级+权重选取，日额度/频率限额，失败冷却走 Redis 不落库，不逻辑删除（状态控制） |
| `sys_ai_model`            | 模型配置 | `model_id` + `provider_id` 联合唯一（同一模型可切换供应商）；`fallback_model_pk` 引用主键降级；计费比例与能力标识；删除后不可复用（§5.3.1 类别②）                      |
...
| `sys_ai_billing`          | 计费记录 | 记录 Token 消耗明细与积分扣减，只追加不逻辑删除（财务记录不可删） |
| `sys_ai_credit_log`       | 余额变动流水 | 充值/扣减/退款/赠送的余额变动记录，只追加不逻辑删除 |
...
| 算法管理 | 算法、算法版本、推荐规则配置 |
| AI 对话 | AI 模型配置、AI 对话会话、AI 对话消息、AI 智能体配置、AI 长期记忆、AI 消息反馈、AI 评测集、AI A2A 端点 |
