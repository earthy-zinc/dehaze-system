...
| AI 对话 -> 内部 MCP 能力网关 | LLM 决定调用工具时 | JSON-RPC 2.0 over HTTP | 网关透传用户 API Key 调用后端 API |
...
| 内部 MCP 能力网关 -> 后端 API | tools/call 调用时 | HTTP + API Key | 网关不校验权限和配额，由底层 API 负责 |

> AI 对话通过内部 MCP 能力网关（系统能力出口，自建 MCP Server）间接调用图像处理、效果对比、算法选择等后端能力，不直接依赖这些业务模块。外部 MCP 服务（GitHub/数据库等第三方）则经 MCP Server 管理注册接入（外部能力进口），概念界定见 [MCP/需求规格 §1.0](../03-模块设计/核心模块/MCP/需求规格.md)。
...
### 3.3 子 Agent 计费（父预扣 + 子实报实销）

主图持有预扣-结算链路；子智能体（含 Team 多子图协作）不独立预扣配额/余额：

- 主 Agent 预扣总预算（预估自身消耗），滚动预算护栏仅在主图生效
- 每个子智能体完成后按实际 token 用量实报实销，独立写入 `sys_ai_billing`（bill_type=chat_subagent，归属主会话用户/消息）
- 子智能体直接扣减配额与余额，不参与主图预扣记录的差额退补（不重复结算）
- 统计侧 chat_subagent 与 chat 同口径聚合，统计与实际扣减一致

详见 [AI计费管理 后端实现 §4.5](../03-模块设计/基础模块/AI计费管理/后端实现.md)。
...
AI 对话通过 AgentHooks 在推理全生命周期注入横切逻辑，计费模块通过钩子集成而非硬编码。deepagents 中间件实际只暴露 `abefore_agent` / `awrap_model_call` / `awrap_tool_call` / `aafter_agent` 四个入口，dehaze 的六点注册口径（`before_agent` / `before_model` / `after_model` / `before_tool` / `after_tool` / `after_agent`）由 `DehazeHooksMiddleware` 桥接到上述入口执行。钩子链路与映射详见 [AI 对话 后端实现-架构与公共 §4.4](../03-模块设计/核心模块/AI对话/后端实现-架构与公共.md)。
...
| `before_tool` | 无计费集成 | 并行子 Agent 写冲突仲裁（资源占用登记/让行/失败重试/覆盖确认），实现见 [智能体管理 §5.3.1](../03-模块设计/核心模块/AI对话/智能体管理/后端实现.md#531-多-agent-协作治理) |
