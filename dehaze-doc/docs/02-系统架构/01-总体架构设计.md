...
**MQ 统一拓扑**（三端共享，任一端启动均声明相同参数，不产生 PRECONDITION 冲突）：

| 队列 | 用途 | x-message-ttl | x-dead-letter-exchange | x-dead-letter-routing-key | 路由键 |
|------|------|---------------|----------------------|--------------------------|--------|
| `task.export` | 导出任务主队列 | 86400000 (24h) | `dehaze.tasks.dlx` | `task.export.dlx` | `task.export` |
| `task.export.dlx` | 导出任务死信队列 | - | - | - | `task.export.dlx` |
| `task.export.retry.0/1/2` | 导出任务重试队列 | 5000/30000/300000 | `dehaze.tasks` | `task.export` | `task.export.retry.0/1/2` |
| `feedback.low_rating` | 低分告警主队列 | 86400000 (24h) | `dehaze.tasks.dlx` | `feedback.low_rating.dlx` | `feedback.low_rating` |
| `feedback.low_rating.dlx` | 低分告警死信队列 | - | - | - | `feedback.low_rating.dlx` |
| `feedback.low_rating.retry.0/1/2` | 低分告警重试队列 | 5000/30000/300000 | `dehaze.tasks` | `feedback.low_rating` | `feedback.low_rating.retry.0/1/2` |

**路由键规则**：路由键 = 队列名（不再使用路由键前缀拼接，避免前缀重复 Bug）。
