...
| [07-日志架构设计](./07-日志架构设计.md)         | 日志契约、存储切割、字段规范     |
...
> 📁 目录结构详见 [前端架构文档](../04-项目实现/前端/)（Vue/React/Taro/RN/Flutter/Android/Uniapp/sdk 各端一档）
...
> 📁 目录结构详见 [后端架构文档](../04-项目实现/后端/)（Java/Go/Python 各一档）
...
| **任务路由层** | 按 `task_type`（去雾/去雨/去雪/低光/超分/去噪/修复）路由到对应算法集合 |
| **算法调度层** | 算法选择、参数配置、任务队列           |
| **模型加载层** | 模型缓存、动态加载、GPU 管理           |
| **图像处理层** | 图像预处理、推理、后处理、质量评估     |
| **结果返回层** | 结果图像存储、指标计算、响应构建       |

> 🔗 算法按 `sys_algorithm.task_type` 字段归类，目录结构维持平铺（`algorithm/<name>/run.py`），不按任务类型物理分目录。支持的任务类型与算法列表见 `dehaze-python/README.md`
...
1. 用户上传待处理图像
       ↓
2. 前端发送请求到业务服务（含 task_type 参数）
       ↓
3. 业务服务验证用户权限和配额
       ↓
4. 业务服务将图像存储到 MinIO
       ↓
5. 业务服务调用 Python 算法服务（按 task_type 路由）
       ↓
6. 算法服务加载指定模型
       ↓
7. 算法服务执行图像处理（去雾/去雨/去雪/低光/超分/去噪/修复）
       ↓
8. 算法服务返回处理结果图像
       ↓
9. 业务服务存储结果并记录日志
...
| **审计日志**     | MongoDB   | 登录审计日志（login_log）、业务操作审计日志（audit_log，含变更前后快照） |
...
> **存储分层原则**：是否需要和业务表做 JOIN 查询？需要 → MySQL；不需要且为只追加审计 → MongoDB；需要向量/全文检索 → Elasticsearch；纯内存临时数据 → Redis；文件二进制 → MinIO；异步解耦 → RabbitMQ。
...
**技术方案**：Java 端使用 STOMP/SockJS + Redis Pub/Sub，Python/Go 端使用原生 WebSocket，三端消息格式一致（`new_message` 事件）；WebSocket 用于在线用户实时 UI 更新，外部渠道（APP 推送/邮件/短信）由推送分发器异步执行。详见 [消息通知/后端实现 §8](../03-模块设计/基础模块/消息通知/后端实现.md)
...
> CORS 通配符 + 凭据组合问题与监控端点鉴权缺失问题**已解决**：Java `CorsConfig` 改为 `setAllowedOrigins(白名单)`，Go 改为 `CorsByRules()` 严格白名单模式；Java `/actuator/**` 要求 `hasRole("ADMIN")`，Go `/metrics` 要求 `SessionAuth + RequireAdmin`。
...
| **健康检查端点不完整** | P1 | 已解决 | 三端均实现 `/health`（liveness）+ `/ready`（readiness）双端点规范。Java `/ready` 检查 DB/Redis/MQ；Python `/ready` 检查 DB/Redis/RabbitMQ；Go `/ready` 检查 RabbitMQ Consumer/Publisher |
