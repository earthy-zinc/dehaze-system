...
| [10-后端额外说明](./10-后端额外说明.md) | 后端补充说明 |
| [11-前端额外说明](./11-前端额外说明.md) | 多端前端现状与 SDK 层分析 |
...
> 以下为 Java、Go、Python 三端在架构设计层面存在的**通用问题**（不区分语言），详细分析与三端现状对比见 [通用基础设施问题与改进](../04-改造计划/通用基础设施问题与改进.md)。各语言特有问题见 [04-改造计划](../04-改造计划/) 下对应文档。

### 8.1 可观测性体系

| 问题 | 严重级别 | 概述 |
|------|----------|------|
| **TraceId 全链路追踪缺失** | P0 | 三端均未接入 OpenTelemetry，无法形成跨 Python↔Java↔Go 的可视化全链路；Java 端 TraceId 完全缺失，Go 端多处 `context.Background()` 断裂 |
| **日志集中化管道未落地** | P1 | 三端日志均为本地文件输出，缺乏集中检索、告警与审计管道；Java 端日志无结构化输出 |
| **多实例/多 Worker 指标聚合** | P1 | Python 端 uvicorn 多 Worker 下 `/metrics` 仅返回局部数据；XXL-Job executor 端口多 Worker 冲突 |
...
| 问题 | 严重级别 | 概述 |
|------|----------|------|
| **CORS 安全配置缺陷** | P0 | Java 端 `allowedOriginPatterns("*")` + 凭据导致凭证盗用；Go 端 `AllowOrigins:["*"]` + `AllowCredentials:true` 为 W3C 违法组合 |
| **监控端点鉴权缺失** | P1 | Java `/actuator/**` 与 Go `/metrics` 无鉴权，可被外部抓取泄露 JVM/线程/堆/业务规模等内部指标 |

**改进方向**：三端统一改为 Origin 白名单（按环境区分 dev/prod），生产环境禁止通配符 + 凭据组合；监控端点要求 `ROLE_ADMIN` 或独立 Basic Auth，或在 Nginx 层拦截。

### 8.3 缓存与弹性容错

| 问题 | 严重级别 | 概述 |
|------|----------|------|
| **缓存防护体系未落地** | P0 | 三端架构设计均规划多级缓存 + 四重防护（L1/布隆/SingleFlight/熔断），实际全部未落地；缓存穿透/击穿/雪崩三大风险无防护 |
| **熔断/重试机制缺失或错误** | P1 | Java 自研熔断器存在根本性计算错误；Go 无熔断/重试；Python `RedisCircuitBreaker` 为死代码 |

**改进方向**：Java 引入 Caffeine L1 + Resilience4j；Go 启用 config 中 `multiLevel` + `protection` 段；Python 激活已定义的 `RedisCircuitBreaker`；灰度策略先启用 L1 + SingleFlight（低风险）。

### 8.4 异步任务与健康检查

| 问题 | 严重级别 | 概述 |
|------|----------|------|
| **异步任务可靠性不足** | P0 | 三端均缺乏任务幂等键；Java MQ 代码全禁用但存在（双路径死代码）；Go 无 DLX 导致毒药消息无限重投 |
| **健康检查端点不完整** | P1 | Go 端 `/health` 恒返回 200 未检查依赖；三端均未区分 liveness 与 readiness |

**改进方向**：任务创建引入客户端幂等键（`Idempotency-Key` 头）；RabbitMQ 配置死信交换机 + `x-retry-count`；三端统一 `/health`（liveness）+ `/ready`（readiness 检查 DB/Redis/MQ）规范。
