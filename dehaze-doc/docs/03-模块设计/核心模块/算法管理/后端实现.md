...
算法管理模块依赖文件管理和任务管理两个基础模块。模型权重文件统一托管在 nginx-dataset 静态服务（`/models` 路径，对应宿主机 `dehaze-python/trained_model/` 目录），Java 后端通过 HTTP HEAD 校验 `sys_algorithm.path` 字段可访问性并回填 `size`；Python 算法服务通过 `model_loader.resolve_model_path` 按需 HTTP 下载到本地缓存后由 `torch.load` 加载。算法的导入导出（Excel/CSV 元数据）通过通用导入导出框架统一实现：
...
- **模型权重 Nginx 静态托管**：算法权重文件不复用 `FileService`/MinIO，而是统一挂载到 nginx-dataset 容器 `/usr/share/nginx/html/models/` 下，访问 URL = `{algorithm.model.baseUrl}/{algorithm.path}`（如 `http://127.0.0.1:9000/models/AECR-Net/NH_train.pk`）。`sys_algorithm.path` 字段为相对路径，是预测流程的单一真实来源
...
| 删除算法 | 读写事务 | 级联删除子孙算法 + 主表 + 版本记录，物理文件异步删除（三端统一使用 TreeDataUtils 递归收集子孙 ID 后批量删除，支持父子节点同时选中去重） |
...
| 文件类型 | 路径格式 | 存储介质 | 说明 |
|---------|---------|---------|------|
| 模型权重 | `{algorithm.path}`（相对路径，如 `AECR-Net/NH_train.pk`） | nginx-dataset 静态服务（宿主机 `dehaze-python/trained_model/`） | 访问 URL = `{algorithm.model.baseUrl}/{path}`；CRUD 时 Java/Python 后端通过 HTTP HEAD 校验可访问性并回填 `size`；预测时 Python 服务通过 `model_loader.resolve_model_path` HTTP 下载到 `MODEL_CACHE_DIR` 后 `torch.load` 加载 |
...
- Java `algorithm.model.baseUrl`（[application-dev.yml](file:///e:/DehazeSystem/dehaze-java/src/main/resources/application-dev.yml)）：默认 `http://127.0.0.1:9000/models`
...
- nginx-dataset 卷挂载：[docker-compose.yml](file:///e:/DehazeSystem/docker-compose.yml) 中 `./dehaze-python/trained_model:/usr/share/nginx/html/models:ro`
