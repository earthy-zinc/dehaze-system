...
- **模型权重 Nginx 静态托管**：算法权重文件不复用 `FileService`/MinIO，而是统一挂载到 nginx-dataset 容器 `/usr/share/nginx/html/models/` 下，访问 URL = `{algorithm.model.baseUrl}/{algorithm.path}`（如 `http://{NGINX_STATIC_HOST}:{NGINX_STATIC_PORT}/models/AECR-Net/NH_train.pk`）。`sys_algorithm.path` 字段为相对路径，是预测流程的单一真实来源
...
算法 CRUD 操作需对应权限标识，仅管理员角色可执行：

| 操作 | 权限标识 |
|------|---------|
| 新增算法 | `sys:algorithm:add` |
| 编辑算法 | `sys:algorithm:edit` |
| 审核算法 | `sys:algorithm:audit` |
| 删除算法 | `sys:algorithm:delete` |
| 版本管理 | `sys:algorithm:version` |
| 导入算法 | `sys:algorithm:import` |
| 导出算法 | `sys:algorithm:export` |

普通用户无算法管理权限，仅可查看已发布算法。
...
| 删除约束 | 仅草稿(1)/已停用(5)/已归档(6)状态可删除，存在进行中预测任务时拒绝删除 |
...
算法变更后触发全系统缓存刷新，由事件驱动机制保证（见 §4.9）：

| 事件 | 缓存刷新范围 |
|------|------------|
| 算法创建 | 算法列表缓存 |
| 算法更新 | 算法列表、算法详情缓存 |
| 算法删除 | 算法列表、算法详情缓存 + 异步删除模型文件 |
| 版本切换 | 算法详情缓存 + 该算法全部历史预测缓存 |
