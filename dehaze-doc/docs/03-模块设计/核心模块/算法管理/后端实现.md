...
> 状态枚举定义见 `AlgorithmStatusEnum`（Java）/ `bo.AlgorithmStatus`（Go），状态值为 1-6。Go 后端在 `bo/algorithm.go` 的 `allowedTransitions` 中定义了完整的状态流转校验；Java 后端 `validateStatusTransition` 当前仅校验终态不可变更及禁止跳过审核直接发布。
