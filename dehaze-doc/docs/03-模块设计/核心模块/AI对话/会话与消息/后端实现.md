...
本功能域涉及两张核心表，表结构详见 [数据库设计 §4.7](../../../../02-系统架构/03-数据库设计.md)：
...
**命中定位**：ES 索引把整个会话消息拼成单个 `message_contents` 字段，无法定位到具体消息，故 `matched_message_id` 采用两段式：ES 按全文检索返回会话 ID 列表 → 回查 `sys_ai_message`（`content LIKE %keyword%`，`deleted=0`）按会话取 `MAX(id)` 即最新一条命中消息 ID，随会话列表响应返回；前端配合消息**游标分页**（`before`/`limit`，`id` 倒序）跳转定位至命中消息。

| 场景 | `matched_message_id` |
|------|---------------------|
| 关键词命中消息内容 | 该会话最新一条命中消息 ID |
| 关键词仅命中会话标题 | `null`（标题已可见，无需定位；不再回查消息） |
| ES 命中但 DB 未匹配（中文分词差异：ES 单字切分 OR 匹配，DB 为整词 LIKE） | `null`（可选字段，不影响列表返回） |

不为此改造 ES 为消息级 nested 索引：存量数据需全量 reindex，成本与风险高于消息定位收益。
...
flowchart TD
    ...
    B --> C["清除相关用户的会话列表缓存 ai:conv:list:{userId}"]
```

归档只改会话状态与列表缓存：检查点键（`ai:checkpoint:*`）由 LangGraph Saver 独占管理，业务层不清理，统一由 30 天 TTL 回收（见 [后端实现-架构与公共 §4.3](../后端实现-架构与公共.md)）。
...
- 回收站：`GET /conversations/trash` 返回当前用户 `deleted != 0` 且未超 30 天的会话，按 `delete_time` 倒序分页
- 物理清理：定时任务 `purgeDeletedConversations` 每日清理 `deleted != 0 且 delete_time < NOW()-30天` 的记录，物理 DELETE 会话并级联物理删除其消息（sys_ai_message）
...
批量删除：校验二次确认标记 confirm=true → 逐条软删除（写 delete_time，deleted 写为行 id）
```
...
幂等性校验：
  ...
  - 命中 → 返回已有消息结果（防重复发送）
  - 未命中 → SET ai:msg:idempotent:{key} = pending, EX = 180（流式超时 120 + 60 余量）
    ↓
校验：
  - 会话存在且属于当前用户
  - 会话状态为活跃（status=1）
...
服务端校验：
  ...
  - 未命中 → SET {key} = pending, EX = 流式超时 + 60（即 AI_MESSAGE_STREAM_TIMEOUT+60=180s，长推理不提前过期）
    ↓
推理后台任务接管：SET {key} = pending:{streamSessionId}（保留原 TTL）
  - 每 TTL/3 周期性 EXPIRE 续期，直到推理结束（值不再是本轮标识则停止）
    ↓
消息处理完成后更新（Lua CAS：仅当值仍为本轮标识时写入）：
  - SET {key} = {messageId, status}, EX = 300（保留 5 分钟供重试查询）
    ↓
异常/取消时清理（Lua CAS：仅当值仍为本轮标识时）：
  - DEL {key}（允许客户端用相同 key 重试）
```

> pending TTL 对齐流式超时（120s）并留 60s 余量，**并在推理期间由后台任务周期性续期**——推理（含空闲超时后继续跑完的部分，如 plan_execute 30 步）可能长于初始 TTL，不续期会让 pending 中途过期、同 key 重试被判为首次发送而二次落库。完成/清理写入必须与"本轮抢占标识"做 **Lua 比较并写（CAS）**：pending 一旦过期被新一轮请求抢占，上一轮的成功结果不得覆盖新一轮的 pending，否则第二次发送会被误判为已完成。
>
> **409 冲突语义以业务码 `REPEAT_SUBMIT_ERROR`（A0002）表达，HTTP 载体为项目统一的 400**（异常体系统一 `BusinessException→HTTP 400`，不为此单一场景改全局映射）。pending 值可能是 `pending` 或 `pending:{streamSessionId}`，服务端以 `startswith("pending")` 判定"处理中"（其余值按完成态 JSON 解析）。`Idempotency-Key` 请求头在发送端点必传（缺失经校验层拒绝：**HTTP 400 + 业务码 `A0400`**，非 FastAPI 默认 422）；OpenAI/Claude 兼容协议端点由 SDK 自动生成 uuid，无幂等头。
...
> **`message.end` 的子智能体用量（`usage.subAgents`）**：`ReasoningService._push_end` 组装 `usage` 主用量四字段（口径不变）后，从运行期 trace 采集器取子 Agent 粒度用量，**仅当存在子智能体调用时**追加 `subAgents` 键，否则整键省略（不下发空数组）；归属经子 Agent 自身 `DehazeHooksMiddleware.awrap_model_call` 期间的 `subagent_scope` ContextVar 绑定、内存聚合，非 DB join 事后推断；`credits` 复用既有计价路径。契约与字段语义（含 `agentCode` 取子 Agent 名称、远程 A2A 不计入）见 [API接口.md §2.2](../API接口.md)。
...
| 异常场景 | 处理方式 |
|---------|---------|
| 用户点击"停止" | 调用消息停止端点，经 ReasoningService.stop 置协作式停止标志（`ai:run:stop:{thread_id}`）+ 关闭 SSE 连接 + 清理该消息关联的中断点（`ai:interrupt:{conv_id}:{msg_id}`）+ 取消关联异步批量任务（async_wait 场景），消息标记为 status=4（已取消）；推理循环在事件间隙检测到标志后走取消收尾——保留已生成内容到消息 content、预扣全额退回（adjustment 结算，不产生流水与成长值事件）。send 路径消息经反查键 `ai:msg:stream:{msg_id}` 定位流会话 |
| 流式超时 | 120 秒无新 token 判定超时，只结束本条 SSE 连接（停止事件推送、清理连接），**不推 error 事件、不标记消息失败**：执行与连接解耦，后台推理继续跑完并正常落库计费，用户重连（Last-Event-ID 重放）或刷新消息历史即可看到完整回复。心跳 ping 直连下发且不进队列/缓存，故纯心跳不会刷新空闲计时。生成器结束即关闭连接，**不 await 后台任务**（否则超时后连接继续挂起、无心跳无事件，代理超时保护失效）；任务引用由模块级 `_pending_tasks` 持有 |
| 客户端断连（未重连） | SSE 检测到断连，token 缓存保留 5 分钟等待重连；未重连时推理自然完成（边生成边缓存，重连后可回放完整内容），消息正常终态。设计取舍：重连体验优先，本地模型成本下不启用断连即停 |
| LLM 调用失败 | 推理侧标记 status=3（失败，error 字段存脱敏后的错误文案，内部细节只进日志），流侧推一次 error 事件 + `message.end`（stopReason=`error`）后关闭连接 |

> error 事件推送后必须补 `message.end`（stopReason=`error`）收尾，保证客户端总能走统一的完成处理逻辑，不会因缺少终结事件而挂起。error 事件由入口最外层推一次（send 路径为 `message_streaming.run_reasoning`，resume 路径为 `ReasoningService.resume`），推理侧只落失败态，避免同一异常推两份 error。多实例部署需 sticky session（同一 stream_session 的活跃队列仅存在于单实例内存），断线重连仅重放 Redis 缓存，不做跨实例续流；重连先注册队列再重放缓存，重放内容已含 `message.end` 时即结束。
...
```
message.end 事件推送后（若会话级开关 suggestions_enabled=1 且该条回复非中断/取消）：
    ↓
...
- 会话开关：`sys_ai_conversation.suggestions_enabled` 会话级列（0/1，默认 1；创建会话传 `suggestionsEnabled`，PATCH 可切换），关闭则不推送
...
**消息分页**：`GET /conversations/{id}/messages` 采用**游标分页**（`before`/`limit`，`id` 倒序；`id` 单调自增等价时间倒序）：`before` 为游标（仅返回 `id < before` 的消息，缺省取最新一页），`limit` 默认 `50`、范围 `1..100`、越界返回 `A0400`；响应 `{list, total, hasMore}`，`hasMore` 由"取 `limit+1` 条"探测（多取的一条即"是否还存在更早消息"），`total` 为会话消息总数（与游标无关）。前端首次加载最新一页（`limit=50`），以本页最早消息 `id` 作为下一次 `before` 向上滚动加载更早页。旧 `pageNum`/`pageSize` 已**移除，无兼容分支**；原设计的「聊天历史游标分页（`before` + `limit`）」**已落地**（不再是提案）。分支回溯语义不变（沿 `current_branch_message_id` 链），上下文组装窗口（token 预算动态裁剪，见 [上下文管理/后端实现](../上下文管理/后端实现.md) §3.2）与展示分页独立。
...
校验：消息归属当前用户、role=assistant、deleted=0
    ↓
标记软删除（deleted 写为行 id，30 天内可恢复，物理清理随定时任务执行）
    ↓
删除后影响：
  - 消息列表不再展示该消息
  - 上下文组装过滤 `deleted != 0` 消息（见 §6.1），不参与后续推理
  - 该消息的"重新生成"入口不再可用
  - 兄弟分支与其他消息不受影响
```
...
后台任务调用 ReasoningService.resume：
  - type=confirm → 按 interrupt.data.confirmKind 路由（_handle_confirm_interrupt）：
      algorithm_recommend → handle_user_confirmation（提交正向反馈）
      tool_permission / dangerous_op → 仅透传确认结果（重放动作在各自触发方）
      未知子类型 → 抛业务异常，不做静默兜底
  - Command(resume=resume_data) 从图中断处继续
    ↓
resume 成功后清理中断点（失败则保留，用户可再次确认重试）
    ↓
SSE 续流
```
...
3. 会话摘要：如果 summary 非空，作为 system 消息注入（"之前的对话摘要：..."）
    ↓
4. 对话历史：从 sys_ai_message 沿 `current_branch_message_id` 的 parent_message_id 链回溯
  - 过滤 `deleted != 0` 的消息，只取 user/assistant 且 content 非空
  - 分支切换后严格取当前激活分支链上的消息，其他分支消息不混入上下文
  - 链异常防护：成环自动截断、单次查询边界 200 条链上消息
  - token 预算窗口：按模型窗口计算历史预算，从最早消息起裁剪至预算内（按时间正序），
    窗口外消息由摘要保底（见 [上下文管理/后端实现](../上下文管理/后端实现.md) §3.2/§4.1）
    ↓
...
| 能力 | 实现 |
|------|------|
| 会话审计列表 | 会话列表接口支持 `view=admin` 查询参数（需 `ai:conversation:audit`），返回全量用户会话（含用户、模型、消息数、Token 与积分消耗、异常标注），支持按状态/关键词筛选 |
| 会话详情 | `GET /conversations/{id}?view=admin` 读取任意用户会话（需 `ai:conversation:audit`），并带审计字段 |
| 消息浏览 | 消息列表/消息详情接口复用用户视角链路（数据同源，不另建审计副本） |
| AI 链路追踪 | 复用消息详情接口（含推理步骤、工具调用），管理员可下钻单条消息的完整推理链定位问题根因 |

**审计聚合字段**（仅 `view=admin` 返回，普通用户视角为 `null`；字段表见 [API接口.md](../API接口.md) §2.1）：

| 字段 | 聚合口径 |
|------|---------|
| `userName` | 批量查 `sys_user`，昵称优先、缺省回退用户名（含软删用户，审计需追溯历史归属） |
| `tokenConsumed` / `creditsConsumed` | `sys_ai_billing` 按 `conversation_id` 分组求和（input+output Token、credits），与计费明细同源；无计费记录为 `0`。**不取消息表**：`tool_llm`/`kb_inject`/`asr`/`tts` 等计费项无对应消息，消息聚合会漏计 |
| `anomalyType` / `anomalyLabel` | 会话级异常标注，见下 |

**异常标注口径**：单条 SQL 批量取（每页少量批查，避免逐会话 N+1），优先级 `failed` > `quota` > `risky_tool` > `canceled`：

| 类型 | 判定来源 |
|------|---------|
| `failed`（存在失败消息） | `sys_ai_message.status = 3` |
| `quota`（配额不足中断） | `sys_ai_billing_anomaly.anomaly_type = 'consecutive_quota_fail'` join `sys_ai_billing` 取 `conversation_id`（跨模块只读消费计费表） |
| `risky_tool`（存在高风险工具调用） | 会话的过程链存在失败的工具调用：`sys_ai_llm_call.tool_call` 非空且调用未成功（join `sys_ai_trace` 取 `conversation_id`，口径见 [可观测性/后端实现.md](../可观测性/后端实现.md) §2.6.2） |
| `canceled`（存在已取消消息） | `sys_ai_message.status = 4` |
...
| 附件校验 | 上传前校验大小（单附件 ≤ 20MB、单消息 ≤ 10 个）、类型（magic 字节校验，防伪装扩展名）+ 恶意文件扫描；命中拒绝上传并告警，防超大附件拖垮服务与恶意文件进入会话 |
