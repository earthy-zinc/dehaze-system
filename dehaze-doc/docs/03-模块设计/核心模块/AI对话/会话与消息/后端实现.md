...
**命中定位**：ES 索引把整个会话消息拼成单个 `message_contents` 字段，无法定位到具体消息，故 `matched_message_id` 采用两段式：ES 按全文检索返回会话 ID 列表 → 回查 `sys_ai_message`（`content LIKE %keyword%`，`deleted=0`）按会话取 `MAX(id)` 即最新一条命中消息 ID，随会话列表响应返回；前端配合消息分页（`before` 游标）跳转定位至命中消息。
...
> pending TTL 对齐流式超时（120s）并留 60s 余量，避免长推理期间 pending 过期后同 key 重复提交造成二次落库。**409 冲突语义以业务码 `REPEAT_SUBMIT_ERROR`（A0002）表达，HTTP 载体为项目统一的 400**（异常体系统一 `BusinessException→HTTP 400`，不为此单一场景改全局映射）。`Idempotency-Key` 请求头在发送端点必传（缺失返回 422）；OpenAI/Claude 兼容协议端点由 SDK 自动生成 uuid，无幂等头。
...
**消息分页**：`GET /conversations/{id}/messages` 支持 `before`（上一页最早消息 ID 游标）+ `limit` 参数，按时间倒序分页；前端首次加载最近 N 条（默认 50），向上滚动用 `before` 游标加载更早消息。分支回溯语义不变（沿 `current_branch_message_id` 链），上下文组装上限（20 轮/200 跳）与展示分页独立。
