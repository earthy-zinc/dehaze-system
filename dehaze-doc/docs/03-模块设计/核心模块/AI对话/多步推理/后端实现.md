...
推理循环本身不持有任何业务能力，全部通过工具调用委派；业务能力（推荐/批量/Skills/MCP）见[后端实现-能力扩展](../能力扩展/后端实现.md)与[后端实现-智能调度](../智能调度/后端实现.md)。
...
本功能域涉及一张业务表，表结构详见 [数据库设计 §4.7](../../../../02-系统架构/03-数据库设计.md)：
...
检查点由 LangGraph Saver 体系管理（Redis `RedisSaver` 自定义实现单层承载，无 MySQL 兜底层），业务层仅通过 Checkpointer API 交互。详见 [后端实现-架构与公共 §4.3 检查点管理器](../后端实现-架构与公共.md)。
...
ReAct 循环由 deepagents 内置循环承担（LLM 调用 + 工具调度 + 结果注入），dehaze 通过 `before_model`/`after_tool` 钩子挂载横切逻辑，无需自建 `react_loop`/`execute_tools` 节点：
...
flowchart TD
    ...
    C -->|ReAct| E["deepagents ReAct 循环<br/>(LLM 调用 + 工具调用决策)"]
    E --> E1{"有 tool_call 且 step < max_steps?"}
    E1 -->|是| T["deepagents 工具执行<br/>(执行工具 + 注入 tool 结果到 messages)"] --> E
...
三种循环共享基于 LLM Function Calling 的工具调度与检查点持久化，差异在于规划策略和反思机制。工具定义经 `before_model` 钩子装载（含 MCP 命名空间预筛选），LLM 在流式响应中自主决定是否调用工具；deepagents 执行工具后将结果以 `role=tool` 消息注入 `messages`，继续推理循环。
...
```mermaid
flowchart TD
    A["deepagents ReAct 循环（LLM 调用）<br/>装载可用工具定义 → 调用 stream_chat(tools)<br/>流式输出 text_delta + 收集 tool_calls"] --> B{"finish_reason == tool_calls?"}
    B -->|否| E["synthesize_response"] --> END([END])
    B -->|是| C["deepagents 工具执行<br/>执行 LLM 决策的工具<br/>记录 agent_thought（Thought + Action + Observation）<br/>将结果以 role=tool 消息注入 messages"] --> A
```
...
推理引擎基于 deepagents，工具来源分为 deepagents 内置工具与 dehaze 业务工具两类，完整清单与装载策略见 [后端实现-能力扩展 §2 工具执行体系](../能力扩展/后端实现.md)。dehaze 业务工具通过 `DehazeToolsBuilder` 按 Agent 配置装载，与 deepagents 内置工具合并后传给 LLM；工具定义采用 OpenAI Function 定义格式，Anthropic 适配在 `DehazeChatModel` 内完成。
...
**产物引用化**：工具返回大体积数据（图像、评估报告）时，通过 `ArtifactService` 注册产物，仅将引用 ID + 摘要作为 tool 消息内容回传给 LLM，避免上下文膨胀。详见 [后端实现-上下文管理 §产物引用化](../上下文管理/后端实现.md)。

**虚拟文件系统补充**：Agent 推理过程中产生的中间临时文件通过 deepagents 虚拟文件系统（`write_file`/`read_file`）暂存于 LangGraph State，会话内有效，不持久化。与 `ArtifactService`（持久化用户可见产物）互补。详见 [后端实现-智能体管理 §7.1 虚拟文件系统](../智能体管理/后端实现.md)。
...
```mermaid
flowchart TD
    A["planner（规划器）<br/>LLM 分解任务为有序子任务列表<br/>标注子任务间依赖关系（独立子任务可并行）"] --> B["executor（执行器）<br/>按依赖关系分批执行子任务<br/>批内由 _PlanExecutor 的 worker 池并行（max_parallel 限流）<br/>跨批串行执行"]
...
**计划数据结构**（`state["plan"]`，**内部状态**，与 `agent_thought` 分开持久化）：

| 字段 | 说明 |
|------|------|
| `tasks[]` | 子任务列表：`{id, description, depends_on[], paradigm?, tool_hint?, status, result?}` |
| `status` | 计划整体状态：`pending`（待执行）/ `executing` / `revised` / `done` |
| `revisions[]` | 计划修订记录：`{revisionNo, reason, changedTaskIds}`（用户干预或重规划均可产生；与对外同名同形，`changedTaskIds` 为任务 id 数组） |

> 本表为**内部状态**形状：任务项为 snake_case（`depends_on`/`tool_hint`），修订项与对外同名同形（`{revisionNo, reason, changedTaskIds}`）；Planner/Replanner 提示词产出与 Redis 中断点存储同此口径，属内部实现、非契约。对外任务项由 `to_wire_plan` 归一为 camelCase（`dependsOn`/`toolHint`），resume 入口由 `apply_plan_edit` 反向归一；完整契约见 [API接口.md](../API接口.md) §2.2「计划统一形状」。
...
| 调整计划 | 调用独立恢复端点 `POST /messages/{id}/resume`，携带 `plan_edit`（`{remove: [taskId], reorder: [taskId...], add: {description, dependsOn?, toolHint?, paradigm?}}`），执行前生效；上行请求体为对外契约、键名 camelCase |
...
入口程序化编排，完成后在返回 state 中置 `jump_to="end"` 直接收尾——该跳转目标即
`after_agent` 节点，Hooks 的计费结算/trace/记忆/`final_response` 提取照常执行。

范式内的 LLM 调用虽直连 `DehazeChatModel`（不经图内 model 节点），但仍经
`before_model`/`after_model` 钩子：步数/Token/滚动预算护栏与主循环同一判定源，并累计
`ctx.step_count`/`ctx.token_used`；每次调用的 usage 汇总挂到最终消息的
`response_metadata`（`usage` + `call_meta`），由 `after_agent` 统一结算——多次调用不
汇总会导致预扣按空用量退补、用量不落账。
...
- Replanner：子任务失败时只重规划受影响部分（`plan.revisions` 追加修订记录），不重跑全计划；
  修订后按最新计划重算批次（修订轮次上限 5，耗尽后未执行任务按失败收尾），保证修订出的
  子任务进入执行而非停在 pending。
- 计划持久化：计划随最终 state 写入 checkpoint；`plan_approve` 中断后图节点整体重跑、
  中断前的 state 写入随中断丢弃，故 resume 时从中断点记录（`ai:interrupt:{threadId}`
  的 `data.data.plan`）恢复用户已确认的计划并合并 `plan_edit`，不重建计划。
- 配额：批次派发前 `quota_recall.precharge_batch` 按子任务描述预估预留，失败整批降级；
  批结束 `settle_batch` 退回预留；执行中 `check_and_recall` 召回未启动子任务（详见 §7.2）。
...
每个 Thought-Action-Observation 循环在工具执行时记录一条 agent_thought：

- `thought` = LLM 的思考文本（ReAct 循环中 `text_delta` 累积的内容，说明为什么选这个工具）
...
推理模型思考 token 的预算控制与过度思考检测（对齐需求规格 §2.2.6.2）：

| 机制 | 实现要点 |
|------|---------|
| 思考预算 | 按模型配置 `max_thinking_tokens`（无思考能力模型为 0）；思考 token 计入输出预留（`max_context − max_tokens` 中的输出预算），防思考挤占答案空间 |
| 过度思考检测 | `after_model` 钩子统计思考 token：思考占比 > 80% 或 L0/L1 简单任务思考超阈值 → 标记 `overthinking` |
| 处理 | 命中后终止思考流（仅输出答案），推理链标注"已精简思考"；SSE 推送思考已截断提示 |
| 观测 | 思考 token 独立统计（thinking_tokens），过度思考会话在可观测性中标注（F-M08-013） |

### 6.3 工具调用安全控制

| 控制项 | 说明 | 实现位置 |
|--------|------|---------|
| 工具白名单 | Agent 只能调用 `before_model` 钩子装载的工具定义（内置工具 + Skill + MCP），LLM 无法越权调用未注册工具 | `before_model` 工具装载 + `LlmClient` 透传 `tools` 参数 |
| 危险操作拦截 | 删除文件、执行危险命令前需用户确认（工具权限/Shell 分别打 `confirmKind=tool_permission`/`dangerous_op`） | `DehazeHooksMiddleware._do_permission_interrupt` / `dehaze_tools_builder` → interrupt，确认后重放 |
| 参数校验 | 工具参数 schema 校验（LLM 输出的 `arguments` 必须符合 JSON Schema） | 工具执行前校验 |
| 超时控制 | 单工具调用超时（默认 60s）；`task`（子 Agent）工具豁免——其为多步推理，内部自有一轮步数/Token 护栏 | `DehazeHooksMiddleware.awrap_tool_call` + `McpGatewayClient` 超时配置 |
| MCP 工具范围 | MCP 网关本身的工具白名单由后端 OpenAPI 文档决定，AI 对话只暴露 3 个元 tool | `McpGatewayClient` |
| 路径白名单 | 文件操作限制在用户目录内 | MCP 网关侧文件工具实现（如有） |

#### 6.3.1 目标保持与偏移检测

多步推理执行中守护用户原始目标，防止 Agent 因理解偏差/错误观察而偏离任务（对齐需求规格 §2.2.6.1）：

| 能力 | 实现要点 |
|------|---------|
| 目标基线 | 推理入口固化用户原始目标与关键约束（`task` + 首轮 user 消息 + 明确约束），写入 `state.task_goal` |
| 周期性校验 | 每 N 步（默认 5 步）由 `after_agent` 钩子触发偏移校验：对比当前计划/行为与 `task_goal` 的相关性 |
| 偏移判定 | 相关性阈值 + 规则（出现越界操作、无关工具调用、范围放大）；命中标记 `goal_drifted` |
| 偏移处理 | 命中 → `interrupt(type=confirm)` 展示偏移说明，用户选择继续/修正/终止；未响应回退到偏移前最后一致步骤（经检查点） |
| 记录 | 偏移检测结果写入过程链（消费可观测性 F-M08-013），供评测协作效率评估 |
...
flowchart TD
    ...
    B --> C{"错误类型"}
    C -->|参数错误（4xx）| D["LLM 分析错误信息<br/>修正参数重试（最多 2 次，按 tool_name 计数）"]
    C -->|超时错误| E["降低并行度后重试<br/>或切换为串行模式"]
    C -->|权限不足| F["interrupt(type=confirm, confirmKind=tool_permission)<br/>请求用户授权；确认后重放该工具调用"]
    C -->|服务不可用（5xx）| G["降级策略"]
...
Plan-and-Execute 的独立子任务可并行执行，由 `_PlanExecutor` 在批内起 worker 池实现（`paradigm_middleware.py`）：

```
Planner 识别可并行的独立子任务 [A, B, C]
    ↓
compute_batches 按依赖拓扑分批，批内子任务进 pending 队列
    ↓
起 min(max_parallel, 批大小) 个 asyncio worker 争抢 pending
    ↓
每个子任务由 worker 以自然语言指令直接调用 DehazeChatModel（非 task 工具：
    task 工具为单次委派，不支持自动并行编排）
    ↓
批内全部完成 → 进入下一批；全部完成后汇总结果
```

**并行度**：`max_parallel`（Agent 配置，默认 5）决定 worker 数，超过则排队等待。与 §7.1 的子智能体委派是两条独立路径：`task` 工具用于主 Agent 主动把单个子任务交给专用子 Agent（独立上下文窗口），Plan-and-Execute 的批内并行走直接 LLM 调用。
...
```python
class QuotaRecall:
    async def precharge_batch(self, ctx: dict, tasks: list[dict]) -> int | None  # 预留积分；None=不足
    async def settle_batch(self, ctx: dict, reserved: int, usage: dict) -> None
    def check_and_recall(self, ctx: dict, pending: list) -> list  # 返回被召回的 pending（纯判定，无 IO）
    async def notify_partial_skipped(self, ctx: dict, count: int) -> None
```

- `precharge_batch`：派发前按子任务描述逐条预估（`EstimateService.estimate_step_credits`）
  预留，`QuotaService.pre_deduct` 扣减配额 + 扣减 `billing_context.remaining_budget`；无计费
  上下文（非 AI 计费）时返回 0 放行。预算/配额不足返回 None，调用方整批降级（标记 failed
  +"配额不足，本批未执行"）。
- `settle_batch`：批结束退回本批预留（`QuotaService.refund`），失败/跳过任务的预留同此退回；
  剩余预算按本批实际消耗（`rate_provider` 换算）修正，售价未配置时按预扣计。
- **为什么预扣只作预留、不按批自结算**：计费只有一个出口——范式内所有 LLM 调用（Planner/
  子任务/Replanner/evaluator）的 usage 已汇总进最终消息，`after_agent` 钩子据此做一次
  预扣差额退补。若批次再留下一份实际消耗，就变成两个计费出口：同一批 token 被扣两次，
  且只有"批次"这一条路径有预扣——顶层 reflexion、Planner、Replanner 等非批次调用没有
  预扣，按批自结算会让这部分消耗完全漏计费。因此预扣只承担"派发前确认余量够不够"的
  职责，批结束即全额释放，实际扣费统一由主结算完成。
- `check_and_recall`：`ctx.token_used >= ctx.token_budget` 或 `billing_context.remaining_budget <= 0`
  ...
  已完成保留。该方法为纯 ctx 判定（无 IO），召回提示由调用方在临界区外经
  `notify_partial_skipped` 推送 `thought` 事件 `status=3`（"部分子任务因配额不足未执行"），
  避免持锁 await 把批内并行退化为串行。
...
Team 成员异步并行执行，通过 Supervisor 中转通信，成员之间不直接通信，避免耦合。详见 [后端实现-智能体管理 §6 Team 团队协作](../智能体管理/后端实现.md)。
...
|---------|---------|---------------|---------|
| `confirm` | LLM 推荐算法后需用户确认 / 危险操作需确认 / 工具权限不足需授权 | `data.confirmKind` 子类型 + 对应业务数据（推荐算法/参数、危险操作详情、工具与原因） | 用户确认或修改后 resume |
| `quota` | 配额不足 | 已用配额、限额、升级提示 | 用户升级 VIP 后 resume |
| `async_wait` | 异步任务（图像处理）提交后等待 | 任务 ID、任务类型、预计耗时、图片数 | 任务完成后自动 resume |

> 中断载荷键名（`interrupt.data.*`）全 camelCase，唯一定义见 [API接口.md](../API接口.md) §2.2「中断载荷键名清单」；Redis 中断点 `ai:interrupt:{threadId}` 内的载荷为内部存储，非对外契约。

**confirm 子类型（`interrupt.data.confirmKind`，常量见 `interrupt_handler.ConfirmKind`）**：

| confirmKind | 触发方 | 恢复动作 |
|-------------|--------|---------|
| `algorithm_recommend` | `algorithm_recommend_service` 算法推荐 | 确认后提交推荐正向反馈，确认结果透传给中断点 |
| `tool_permission` | `DehazeHooksMiddleware` 工具权限不足 | 确认通过后重放该工具调用（不重放会让 LLM 视为失败重试、再次触发中断形成循环） |
| `dangerous_op` | `dehaze_tools_builder` Shell / 高风险 Python 执行 | 确认通过后重放沙箱执行 |

> resume 按 `confirmKind` 路由副作用，未知子类型直接抛业务异常（不做静默兜底）。中断点只在 resume **成功后**清理，恢复失败保留中断点供重试。
...
`interrupt_data` 含 `{usedDaily, dailyLimit, usedMonthly, monthlyLimit, upgradeTip}`；用量数据
组装失败时附加 `quotaDataError=true` 标记（记 error 日志），避免伪造"达标"用量展示，其余字段保持
...
```
推理过程中在中断点调用 interrupt()（deepagents 内置循环，无 should_continue 节点）：
  - 工具内：algorithm_recommend_service（算法推荐确认）、
    dehaze_tools_builder（批量异步任务等待 / Shell 与高风险代码执行确认）
  - 模型调用钩子与范式编排：run_quota_interrupt（预扣阻断、滚动预算不足统一转配额中断）
  - 工具权限：dehaze_hooks_middleware._do_permission_interrupt
  - 计划确认：paradigm_middleware（plan_approve）
    ↓
LangGraph interrupt 暂停 Agent 执行，自动保存 checkpoint
    ↓
中断点由触发方经 interrupt_handler.save_interrupt 落 Redis：ai:interrupt:{threadId}
  - type = confirm / quota / async_wait / plan_approve
  - data = 中断相关数据（type=confirm 时含 confirmKind 子类型，见 §8.1）
    ↓
通过 SSE 推送 interrupt 事件（SseEventConverter 统一处理 __interrupt__）
    ↓
前端展示中断交互（确认卡片 / 升级引导 / 进度展示）
```

> 护栏（`guardrail_middleware` 的 `abefore_model` 注入/敏感话题过滤、`awrap_tool_call` 越权检测）只做过滤与拦截，**不产生中断点**；配额类的中断统一走 `run_quota_interrupt`。中断点落库由各触发方完成，不存在 after_agent 钩子统一记录的环节。
...
|---------|------------|-----------|
| `confirm` | `{confirmed, algorithmId, ...}`（用户确认结果原样透传给 `interrupt()` 返回点） | `Command(resume=resume_data)` |
| `quota` | 空（用户升级 VIP 后恢复） | `Command(resume=True)` |
| `async_wait` | `{async_task: 任务结果摘要}` | `Command(resume=resume_data)`（回调触发，不走用户端点） |

> `confirm` 的副作用（如推荐正向反馈）按 `data.confirmKind` 由 `ReasoningService._handle_confirm_interrupt` 路由，中断点在 resume 成功后统一清理。
...
```
LLM 调用批量工具（deepagents 工具执行，batch_process）
    ↓
按批量阈值分流（`batch_process_service.BATCH_ASYNC_THRESHOLD`，默认 3）：
...
大批量异步提交：
  ...
  登记 Redis 反查键：ai:async_task:{task_id} → {thread_id, conv_id, msg_id, user_id, stream_session_id}
    ↓
interrupt(type=async_wait, data={taskId, taskType, estDuration, imageCount})
...
成功恢复后在 `finally` 中 `release_lock(conv_id)`，避免同 thread 双跑导致 checkpoint 冲突。
锁被占用（用户手动 `/resume` 或恢复期间新消息 run）时**回滚反查键并延迟重试**（默认 2s、
最多 3 次，重试计数存反查键）：清键后直接放弃会让中断点挂起到 24h TTL 过期，重试耗尽才
交回用户侧接管并记 warning 日志。

### 8.5 并行任务的中断与恢复

Plan-and-Execute 的批内并行子任务是直接 LLM 调用（§7.2），不经过图节点，因此**没有独立中断点**——中断只发生在主循环的工具调用处，不能在子任务中途挂起。

```
批内并行执行中，某子任务触发配额不足
    ↓
check_and_recall 在 worker 取任务的临界区内判定，召回全部未启动子任务（记 skipped）
    ↓
正在执行的子任务等当前调用返回后自然收尾（不抢占中断）
    ↓
批结束 settle_batch 退回预留，notify_partial_skipped 在临界区外推送提示
    ↓
已完成任务结果保留，参与最终汇总
```

需要用户参与的中断（confirm / quota / async_wait）在子任务之外触发：子任务全部汇总后回到主循环，由工具调用处的 `interrupt()` 挂起并等待 resume（§8.1/§8.2）。
...
调用消息停止端点
    ↓
置协作式停止标志（ai:run:stop:{thread_id}，TTL 5 分钟）+ 结束 SSE 流 + 更新消息 status=4 + 清除中断点
（async_wait 挂起场景：stream_session_id 传批量异步任务 ID，取消后台批量任务）
    ↓
推理循环在事件间隙检测到标志 → 中断 astream 消费（图执行随生成器关闭终止，含并行子智能体）
    ↓
取消收尾：保留已生成内容到消息 content、trace 记中断态、预扣全额退回（adjustment 结算，不计费不产生成长值事件）
    ↓
...
| 记忆提取 | after_agent 钩子异步提取本次对话的记忆，写入 sys_ai_memory；子 Agent 子图 run 跳过（is_subagent） |
