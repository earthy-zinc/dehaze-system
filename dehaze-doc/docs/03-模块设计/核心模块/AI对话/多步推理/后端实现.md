...
检查点由 LangGraph Saver 体系管理（运行时 Redis `RedisSaver` + 持久化 `MySQLCheckpointSaver`，均为自定义实现，表不纳入本系统 schema），业务层仅通过 Checkpointer API 交互。详见 [后端实现-架构与公共 §4.3 检查点管理器](../后端实现-架构与公共.md)。
...
| 超时控制 | 单工具调用超时（默认 60s） | deepagents 工具节点 + `McpGatewayClient` 超时配置 |
