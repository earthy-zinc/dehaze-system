...
| 取舍点 | 方案对比 | 选择与理由 |
|--------|---------|-----------|
| 上下文组织 | 全部消息平铺 vs 分层结构（系统/摘要/最近原文/任务状态/产物引用） | 选分层。分层让"哪些信息必须保留、哪些可压缩"显式化，也为 Prompt Caching 优化提供"稳定前缀 + 动态后缀"的组装基础（见[AI模型管理/后端实现.md §2.9](../../基础模块/AI模型管理/后端实现.md#29-prompt-caching-优化)） |
...
系统提示词内部再分"稳定层"与"变量层"，稳定内容长期复用、动态内容随 Agent/会话注入，两者共同构成 Prompt Caching 的"稳定前缀 + 动态后缀"组装基础（见 [AI模型管理/后端实现.md §2.9](../../基础模块/AI模型管理/后端实现.md#29-prompt-caching-优化)）：
