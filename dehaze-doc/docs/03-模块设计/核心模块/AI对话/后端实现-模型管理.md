...
本文档描述 F-M08-007 模型供应商管理与模型选择的后端实现，包括模型供应商配置管理、API Key 管理、供应商健康与容错、模型配置管理、模型生命周期、模型选择与切换、计费集成、用量与成本分析。

> 与 [需求规格-模型管理](./需求规格-模型管理.md) 同步：供应商连通性测试、供应商健康与容错（§2.4）、模型生命周期（§2.6）、用量与成本分析（§4）为新增需求；Key 级 RPM 限流、计费比例版本化为待确认项（见 §2.3/§2.12），落地前保持现有字段与逻辑。

### 1.1 设计思想

#### 设计动机

模型管理是 AI 对话的"地基层"，本功能域的根本矛盾是：**供应商外部服务的不可控性**与**平台服务可用性/成本的要求**之间的张力。
...
| 实体 | 职责 | 服务层关键用途 |
|------|------|-------------|
| `sys_ai_provider` | 模型供应商配置（API 地址、协议类型、认证方式、健康检查开关、备注） | 供应商 CRUD、协议适配、连通性测试 |
| `sys_ai_provider_key` | 供应商 API Key（加密存储、优先级+权重选取、日额度/频率限额） | Key CRUD、Key 选取与轮换、失败冷却 |
| `sys_ai_model` | AI 模型配置（计费比例、能力标识、降级链、VIP 门槛） | 模型 CRUD、计费换算、能力校验、降级路由、VIP 过滤、生命周期 |
...
| `sys_ai_provider_key` | `rpm_limit` | 分钟调用频率上限（可选，**待确认项**），超限后本轮跳过该 Key |
...
排除超出调用限额的 Key：
  ...
  - 分钟调用计数达到 rpm_limit（ai:provider_key:{key_id}:minute:{yyyyMMddHHmm}，待确认项）
    ↓
...
**实现（ProviderHealthService，`app/service/ai/provider_health_service.py`）**：
...
**标记"即将下线" = 禁用 + 通知**：供应商发布模型停服公告后，管理员将模型禁用（`status=0`），服务端向使用该模型的所有活跃会话用户推送替换模型推荐（消息中心站内信，经 `MessageService.send`，去重键 `bizModule=ai_model, bizId=model_id`），推荐复用 `fallback_model_pk` 指向的模型，无配置则提示管理员配置替代模型：
...
查使用该模型的活跃会话用户（list_active_conversation_users，去重）
    ↓
取替换模型（fallback_model_pk 指向的启用模型，无则 None）
    ↓
...
flowchart TD
    ...
    B --> C["返回模型信息：<br/>model_id, display_name, provider_id（关联供应商信息）<br/>能力标识（supports_multimodal, supports_tool_call, supports_streaming）<br/>积分单价（input_rate, output_rate, cached_rate）<br/>max_context_tokens, max_output_tokens<br/>速度档位（由延迟聚合，见 §4）"]
    C --> D{"根据用户 VIP 等级过滤"}
    D -->|基础模型| E["所有用户可用"]
    D -->|进阶模型| F["VIP1+"]
    D -->|高端模型| G["VIP2+"]
...
| 降级标识 `is_fallback_target` | 该模型是否被其他启用模型 `fallback_model_pk` 引用（repo `count_fallback_targets`） | 减少用户对"模型变化"的困惑 |
...
优先尝试同模型备用供应商（同 model_id 的其他 provider_id 行，若存在）
    ↓
检查 sys_ai_model.fallback_model_pk（引用 sys_ai_model.id 主键）
    ↓
有降级模型：
  - 校验降级模型状态（status=1 且 supports 能力匹配）
  - 切换到降级模型重新调用（按 fallback_model_pk 查询，含 provider_id）
...
**降级链**：支持多级降级（A → B → C），每级模型的 `fallback_model_pk` 指向下一级模型的 `id`（主键）。降级时逐级尝试，全部失败才报错。使用主键引用而非 `model_id`，避免同一 `model_id` 在多供应商下降级指向歧义。
...
> **计费比例版本化（待确认项）**：需求规格 §2.8.3 提出计费比例变更记录价格版本、历史消息按当时价格计费。当前实现为按最新比例换算，历史消息账单不受影响（消息已记 credits）。是否引入价格版本字段取决于计费对账精细度要求，落地前不改现有字段。

### 2.13 内置本地 provider（local_llm）

内置本地 LLM 作为零外部依赖的降级 provider（数据库中 `provider_code = "local"`），以独立子进程运行（OpenAI 兼容，默认 `127.0.0.1:8992`），由 `local_llm_manager`（懒启动 + 健康探测 + 进程守护）管理，`local_llm_server`（`app/service/ai/local_llm_server.py`）承载推理：
...
> 与 [AI 计费管理模块](../../基础模块/AI计费管理/需求规格.md) 协同：计费模块负责积分计量与账单统计，本节聚焦供应商/模型的运营健康与成本归因，数据同源（`sys_ai_billing` 等调用日志），面向管理员。
...
| 降级与故障统计 | `sys_ai_billing`（actual_model 非空） | 各模型降级频率、Key 失败切换次数，驱动轮换决策 |
| 成本归因 | `sys_ai_billing` | request_id、供应商、模型、Token 明细、计费比例、是否降级、错误码、延迟，支撑对账与异常追溯 |
