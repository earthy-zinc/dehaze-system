...
`reasoning_mode` 判定，固定 `direct` 才跳过工具能力校验。
...
**实现说明**（`app/service/ai/quota_recall.py`，`QuotaRecall` 单例 `quota_recall`，由 `_PlanExecutor` 集成调用）：
...
`interrupt_data` 含 `{used_daily, daily_limit, used_monthly, monthly_limit, upgrade_tip}`，并经
`interrupt_handler.save_interrupt` 落 Redis。用户升级 VIP 后经 resume 恢复：`reasoning_service.resume`
...
**实现说明**（`dehaze_tools_builder` + `app/service/ai/async_resume.py`）：
