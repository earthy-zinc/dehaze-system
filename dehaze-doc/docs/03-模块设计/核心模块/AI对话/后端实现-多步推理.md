...
### 2.1 sys_ai_agent_thought（推理过程）

| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | bigint | 主键 |
| `message_id` | bigint | 关联消息 ID（触发推理的 assistant 消息） |
| `conversation_id` | bigint | 会话 ID（冗余，便于按会话查询推理链路） |
| `position` | int | 步骤序号（从 1 开始，同一消息内排序） |
| `thought` | TEXT | LLM 思考内容（为什么选这个工具；预期效果） |
| `tool` | varchar(128) | 工具名称（工具标识，如 mcp:image_dehaze / code_execute / skill:code_review） |
| `tool_input` | json | 工具输入参数 |
| `observation` | TEXT | 工具返回摘要（结构化摘要，不含大体积原始数据） |
| `status` | tinyint | 步骤状态（1:成功;2:失败;3:跳过） |
| `latency_ms` | int | 工具调用耗时（毫秒） |
| `error` | TEXT | 失败原因（status=2 时填充） |
| `create_time` | datetime | 创建时间 |

**索引设计**：
- `idx_message_position(message_id, position)`：优化按消息查询推理步骤并按序排列
- `idx_conversation(conversation_id)`：优化按会话查询推理链路

**只追加表**：推理记录为只追加，不使用逻辑删除，过期数据通过定时任务物理清理。
...
用户消息进入 Agent
    ↓
evaluate_complexity（复杂度评估节点）
  - LLM 快速评估任务特征：步骤数、确定性、精度要求、是否需迭代优化
    ↓
按复杂度路由：
  ├── L0 简单问答（单步可完成） → direct_response（直接生成回复 → END）
  ├── L1 单步决策（步骤不确定，需动态调整） → ReAct 循环
  ├── L2 全局规划（步骤可预见，多步骤） → Plan-and-Execute
  └── L3 高精度任务（需迭代优化） → Reflexion
```
...
START
  ↓
evaluate_complexity ──路由──┬── direct_response → END
                            ├── ReAct 循环
                            ├── Plan-and-Execute 循环
                            └── Reflexion 循环
                                    │
                                    ▼
                            synthesize_response（结果整合）→ END
```
...
understand_intent（理解意图）
  ↓
  - 分析当前状态，决定下一步动作
  - 输出：Thought（思考）+ Action（工具调用 or 直接回复）
  ↓
should_act?
  ├── 需要工具 → execute_tools → observe_results → understand_intent（循环）
  └── 无需工具 → synthesize_response → END
```
...
planner（规划器）
  ...
  - LLM 分解任务为有序子任务列表
  - 标注子任务间依赖关系（独立子任务可并行）
  - 输出：plan = [{step, description, tool, depends_on, parallelizable}]
    ↓
executor（执行器）
  ↓
  - 按依赖关系执行子任务
  - 独立子任务并行执行（SubagentManager 调度多个子智能体）
  - 依赖子任务串行执行
    ↓
replanner（重规划器）
  ↓
  - 检查执行结果，判断是否需要调整计划
  ├── 计划有效 → 继续执行下一批子任务
  ├── 计划需调整 → 更新计划，重新执行受影响部分
  └── 所有子任务完成 → synthesize_response → END
```
...
```
actor（执行器，ReAct 或 Plan-and-Execute）
  ↓
  - 执行任务，生成初步结果
    ↓
evaluator（评估器）
  ↓
  - 评估结果质量（0-1 分）
  - 评估维度：正确性、完整性、规范性、用户满意度
  - 评估方式：LLM 自评 + 规则校验 + 工具验证（如代码执行验证）
    ↓
score >= threshold（默认 0.8）？
  ├── 是 → synthesize_response → END
  └── 否 → self_reflection（自我反思）
            ↓
            - 分析问题根因
            - 检索历史经验（MemoryService 中的反思记忆）
            - 生成改进策略
            ↓
          更新 state，注入改进策略
            ↓
...
工具调用失败
    ↓
错误分类（after_tool 钩子）：
  ├── 参数错误（4xx） → LLM 分析错误信息，修正参数重试（最多 2 次）
  ├── 超时错误 → 降低并行度后重试，或切换为串行模式
  ├── 权限不足 → interrupt(type=confirm) 请求用户授权
  ├── 服务不可用（5xx） → 降级策略：
  │     - 切换备用工具（如 MCP 不可用则用代码执行模拟）
  │     - 跳过该步骤，记录为 skipped，继续后续步骤
  │     - 告知用户该步骤失败，提供手动操作建议
  └── 不可恢复错误 → 记录错误，生成包含失败信息的回复
    ↓
重试次数耗尽 → 触发降级策略
    ↓
Plan-and-Execute 模式下：replanner 重新规划，跳过或替换失败步骤
Reflexion 模式下：反思失败原因，调整策略后重新执行
```
