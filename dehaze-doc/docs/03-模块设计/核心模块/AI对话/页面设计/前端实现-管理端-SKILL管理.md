...
| SkillFormDialog | Skill 表单弹窗：新建 = zip 压缩包上传（后端解析 SKILL.md 并校验），编辑 = 名称/描述/场景/指令文本表单 + frontmatter 只读 + 资源文件清单。上传成功后回显被跳过的资源文件清单，并提示"已入库，待启用" |
| SkillTestPanel | Skill 试运行面板：输入测试数据 → 以 Skill 指令为系统提示词真实推理一次，展示模型输出与用量（独立调试会话，不入库不推送） |
...
| `testResult` | Skill 试运行结果（模型输出 + 用量 + 指令/输入回显，`SkillTestResult`） |
...
| `saveSkill` | 创建/更新 Skill（指令内容校验：长度限制、危险操作拦截；新建入库为禁用态） |
| `uploadSkill` | 上传 zip 压缩包创建 SKILL（后端解析校验；返回 `skippedFiles` 供部分失败提示） |
| `switchSkillStatus` | 启停 Skill（禁用后 LLM 不再自动选择） |
| `deleteSkill` | 删除 Skill（软删除；校验 Agent 关联，有则提示先解绑） |
| `testSkill` | 试运行 Skill（输入测试数据，以指令为系统提示词真实推理一次，不入库不推送） |
...
- **试运行不入库**：SkillTestPanel 走真实推理但不写数据、不推送（独立调试会话 thread_id）
...
| 入库即禁用 + 启用引导 | SkillFormDialog 保存后回到列表，AiSkills 询问"是否立即启用"（不走试运行） | 新建/上传即禁用态（§2.6.11"启用才可被使用"）：管理员显式启用后才进 Agent 索引，避免误上架 |
| 部分失败可见 | 上传返回 `skippedFiles` 时用 `ElMessageBox.alert` 列出"路径：原因" | 越限/目录外成员不静默丢弃，管理员需知道哪些资源没入库 |
