...
本文档描述 AI 对话模块**管理端智能体管理页**的 Web 端（React）前端实现方案，聚焦 Agent 全生命周期（配置、版本、评测、发布、A2A）的页面编排与交互。页面结构与交互规范见 [需求规格.md](../需求规格.md) §3.3.2，接口契约见 [API接口.md](../API接口.md) §2.5-2.7。Android / Flutter / Taro 等多端参考本 Web 端的组件结构与状态管理方案按各自技术栈适配。
...
| PublishDialog | 发布确认弹窗（发布前评测门禁状态提示 + 判分漂移豁免强制发布入口，豁免原因必填并二次确认） |
| EvalPanel | 评测区入口：评测集/样本管理（复用 EvalDatasetPanel）+ 异步评测进度 + 执行记录与发布门禁状态，详细管理收敛至评测中心（见 [Agent评测/前端实现.md](../Agent评测/前端实现.md)） |
| A2aPanel | A2A 端点管理区：外部端点注册/刷新 Agent Card/删除；Agent 对外暴露开关 |
| A2aEndpointTable | 外部 A2A 端点列表（端点地址/状态/凭证状态） |

> Agent 配置表单字段与业务规则见 [需求规格-智能体管理](../智能体管理/需求规格.md) §2.11；评测集/评分/门禁能力见 [Agent评测/前端实现.md](../Agent评测/前端实现.md)（F-M08-014）。
...
| `deleteAgent` | 删除 Agent（校验会话引用/子 Agent 引用，有则提示先解绑；删除同时级联清理该 Agent 的评测集/样本/评测执行记录，二次确认文案已注明不可恢复） |
| `fetchVersions` | 拉取版本历史 |
| `compareVersions` | 版本差异对比（base/target） |
| `publishAgent` | 发布 Agent（草稿通过评测集回归门禁后发布；`force=true` 为判分漂移豁免） |
| `rollbackVersion` | 回滚到历史发布版本（生成新版本号） |
| `fetchEvalDatasets` / `createEvalDataset` / `updateEvalDataset` / `deleteEvalDataset` | 评测集 CRUD（删除级联样本，二次确认） |
| `fetchEvalSamples` / `createEvalSample` / `updateEvalSample` / `deleteEvalSample` | 样本 CRUD（二次确认；支持评测失败样本回流预填） |
| `runEval` | 触发回归评测：`runEvalAsync` 取 `taskId` 后轮询 `getEvalTask` 至 `succeeded`/`failed`，返回门禁判定 |
...
| 推理参数继承可视化 | 三级优先级（系统默认 ← Agent ← 会话级）在表单中显式标注（"留空继承系统默认"） | 参数合并规则透明，避免"配了不生效"的困惑 |
| 判分漂移豁免 | 发布弹窗内开关 + 豁免原因必填 + 二次确认风险提示，原因随 `change_note` 留档（后端仅 `force` 一个开关字段） | 判分一致率低于阈值时门禁暂停会阻塞发布，豁免是运维必需的逃生口；风险必须由发布人显式确认并留痕 |
...
| 弹窗（Dialog） | Agent 配置表单、发布确认（含豁免）、回滚确认、测试 Agent、评测集/样本表单 |
| 抽屉（Drawer） | 评测集样本列表 |
