...
> 3. 会话详情/消息列表/消息详情三接口的管理端 `view=admin`（需 `ai:conversation:audit`）已由 SDK 统一透出：`getConversation(id, { view })` / `getMessages(id, { ..., view })` / `getMessageDetail(id, { view })`，前端均走 SDK 调用。
