...
flowchart TD
    ...
    EvalCenterPage --> EvalDatasetPanel
...
| EvalDatasetPanel | 评测集与样本管理（评测集 CRUD + 样本列表抽屉 + 样本 CRUD，删除二次确认）；智能体详情页评测区复用同一组件，避免两处维护 |
| EvalSampleFormDialog | 样本表单（新建/编辑，字段校验；`datasetId` 为空时表单内选评测集；`preset` 为空载入评测失败样本的 Bad Case 回流入口） |
| EvalPanel | 智能体详情页评测区容器：门禁状态 + 异步评测进度 + 评测执行记录 |
| EvalDetailDrawer | 评测详情抽屉：报告 + 样本明细 + 版本对比 |
| ReportPanel | 评测报告（总分 + 四维得分 + 指标级明细（任务完成/工具调用准确率/幻觉/死循环/效率）+ pass@k 通过率） |
| SampleResultList | 样本明细（通过/失败/差异说明），失败样本可"纳入数据集"回流（预填任务目标与风险等级） |
| VersionCompare | 变更前后版本得分对比，显著退化（超阈值）显式标注 |
| TrendPanel | 评测历史趋势（退化发现，与门禁退化阈值对照） |
| JudgeStatusPanel | 判分模型状态：一致性验证状态、校准告警、漂移暂停门禁提示 |
| ReviewPanel | 人工复核（页面级）：线上抽样判定按比例抽取待确认项，复核详情据实判定（样本定义 + 实际输出 + 四维得分），复核回填用于判分校准 |
...
| 复核详情（ReviewPanel 弹窗） | `GET /api/v1/ai/eval-center/runs/{runId}/samples/{sampleId}`（runId/sampleId 取复核队列项） | 打开复核弹窗 |
| 手动触发评测（EvalPanel） | `POST /api/v1/ai/agents/{id}/eval/runs` 返回 `taskId` → 轮询 `GET /api/v1/ai/agents/{id}/eval/tasks/{taskId}` | 点击"开始评测"后轮询到 `succeeded`/`failed` |
...
> - 手动触发评测为异步任务：`POST /eval/runs` 返回 `taskId` 后轮询 `GET /eval/tasks/{taskId}`（`pending`/`running`/`succeeded`/`failed`，`progress` 为已执行/总样本数），`succeeded` 时 `result` 即门禁判定；任务状态保留 24h，过期需重新触发。
> - 复核详情一次返回样本定义（`taskGoal`/`allowedInput`/`expectedResult`/`expectedProcess`/`forbiddenBehavior`/`tools`）+ 实际输出（`actualOutput`）+ 执行异常（`error`）+ 四维得分与判分说明（`scores`/`notes`）+ 风险等级与判分结论（按复核项的 runId/sampleId 取数），前端无需拼接多次请求。
...
| Bad Case 回流 | run 样本明细"纳入数据集"复用 EvalSampleFormDialog，预填任务目标与风险等级后由用户选定评测集 | 失败样本一键沉淀为回归考题，闭环修复；避免翻抄字段 |
