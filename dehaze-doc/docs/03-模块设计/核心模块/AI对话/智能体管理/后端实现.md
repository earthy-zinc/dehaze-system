...
本文档描述 F-M08-011 智能体管理的后端实现，包括 Agent 实体数据模型、基于 deepagents 的推理引擎集成、配置动态化、子 Agent 协作、Team 团队协作、复杂任务辅助工具、版本管理与发布、发布门禁、安全护栏。
...
| 原则 | 说明 |
|------|------|
| 配置驱动 | Agent 的全部行为（系统提示词、模型、推理范式、推理参数、Skills、MCP 命名空间、子 Agent、权限）由 `sys_ai_agent` 表管理，前端可动态配置。推理参数按"系统默认 ← Agent 配置 ← 会话覆盖"三级合并（详见 §4.2） |
...
| 发布门禁 | Agent 发布前须通过回归集评测，评分下降或高风险样本失败则阻断发布，让变更可量化、可比较（评测由 Agent 评测模块承载，见 [Agent评测/后端实现.md](../Agent评测/后端实现.md)） |
...
本功能域涉及 9 张业务表，表结构详见 `config/sql/schema/` 目录，顶层设计详见 [数据库设计 §4.7](../../../../02-系统架构/03-数据库设计.md)：

| 实体 | 职责 | 逻辑删除 |
|------|------|---------|
| `sys_ai_agent` | Agent 主表（系统提示词、模型、推理范式、配置 JSON、类型标识、权限规则、护栏配置） | 是（唯一键含 deleted，软删后可重建同 agent_code） |
...
| `sys_ai_agent_eval_dataset` | 评测集（开发集/回归集/保留集） | 是（唯一键含 deleted，软删行不占键位） |
...
> 评测集/样本/执行记录三表的数据模型归 Agent 评测模块（F-M08-014），详见 [Agent评测/后端实现.md §2.1](../Agent评测/后端实现.md#21-数据模型)。
| `sys_ai_agent_endpoint` | 外部 A2A Agent 注册（Agent Card 地址、端点、认证凭证密文） | 是（唯一键含 deleted，软删行不占键位） |

### 2.1 `sys_ai_agent` 关键设计

| 字段 | 设计决策 |
|------|---------|
| `agent_code` | 业务引用键，唯一；唯一键含 deleted，软删后可重建同 code |
...
| `tags` | 分类标签（JSON 字符串数组，可空），管理端筛选/展示用；创建/更新表单透传，列表与详情返回 |
...
推理参数全局默认值为代码常量（`REASONING_DEFAULTS`，见 §10.2）——参数随算法演进由版本调整，无管理员运行时修改场景；Agent 级与会话级配置逐级覆盖（见 §4.2）。护栏等安全合规开关的默认值存 `sys_dict`（有运维调整场景）。
...
评测集/样本/执行记录三表的关键字段设计与数据模型见 [Agent评测/后端实现.md §2.1](../Agent评测/后端实现.md#21-数据模型)。本功能域仅消费评测结果作为发布门禁判定（见 §9）。
...
flowchart TD
    ...
    subgraph Runtime["LangGraph 运行时（dehaze 已有）"]
        C1["检查点由 LangGraph Saver 体系管理<br/>（详见架构与公共 §4.3）"]
    end
    Business --> Harness --> Runtime
```

**为什么选 deepagents（架构选型）**：deepagents 把"单个 Agent 的推理循环"做成标准件，且返回**标准 `CompiledStateGraph`**，可作为节点/子图嵌入 dehaze 已有的 LangGraph 运行时，Checkpointer、中断/恢复、流式这套底子无需重写、直接兼容；全自研方案则要从零实现这些能力且失去上游维护与生态演进（取舍详见 §1.1）。
...
| 评测与回归 | 发布门禁触发（回归集判定由 Agent 评测模块承载，见 §9） |
...
| `EvalRunner` | 评测执行器（归 Agent 评测模块），发布前回归集判定（见 [Agent评测/后端实现.md](../Agent评测/后端实现.md) §2.3） |
...
| `before_tool` | `DehazeHooksMiddleware.awrap_tool_call` 运行 `before_tool` 注册点（并行子 Agent 写冲突仲裁，见 §5.3.1）；`GuardrailMiddleware.awrap_tool_call` 做越权 MCP 命名空间检测，命中返回拦截 `ToolMessage` |
...
deepagents 的 `checkpointer` 参数直接接收 LangGraph `Checkpointer` 实例，dehaze 现有检查点体系（见 [架构与公共 §4.3 检查点管理器](../后端实现-架构与公共.md)）直接传入，中断/恢复流程不变。
...
> **Python 端实现**：Agent CRUD 路由 `app/router/ai_agent.py`（prefix=`/api/v1/ai/agents`）——列表（管理员分页，支持 `keyword`/`status`/`type`（`agent`/`subagent`/`team`）筛选，列表项经 `count_skills_by_agent_ids`/`count_mcp_by_agent_ids`/`count_subagents_by_agent_ids` 批量聚合返回 `skillCount`/`mcpCount`/`subAgentCount`；普通用户仅启用项）、`enabled` 列表、详情、创建/更新/删除/启停（`PATCH /status`）、测试（`POST /test`）、复制（`POST /copy`）、Skills/MCP/子 Agent 关联（覆盖式）、版本/发布/回滚（`POST /publish`、`GET /versions`、`GET /versions/diff`、`GET /versions/{versionNo}`、`POST /versions/{versionNo}/rollback`）。管理端接口均需 `ai:agent:manage` 权限（`@require_permission`，越权 403/`A0301`），普通用户 `_is_manager` 判定走仅启用列表。核心服务 `app/service/ai_agent_service.py` + `ai_agent_version_service.py`。

| 职责 | 说明 |
|------|------|
| Agent CRUD | 创建/查询/更新/删除/启停，`agent_code` 唯一性校验绕过软删查全表；删除时校验会话/子 Agent 引用并级联清理评测资产（见 §4.5） |
| 关联管理 | 设置 Agent 的 Skills/MCP 命名空间/子 Agent 关联（覆盖式更新，写入前做引用完整性校验） |
...
| 危险操作审计 | 删除 Agent/版本回滚/强制发布/删除 A2A 端点写入 `audit_log`（见 §4.5） |

**关联写校验**（覆盖式写入前，不满足即拒绝，不留无效绑定）：

| 关联 | 校验 |
|------|------|
| Skills | `skill_name` 必须存在于 `sys_ai_skill`（未删），缺失返回 A0401 并列出前 5 个 |
| MCP 命名空间 | 命名空间必须已注册在未软删的 MCP Server 下（`ai_mcp_namespace_repository.list_registered_names`），否则运行时装载不到任何工具 |
| 子 Agent | ①不得绑定自身（A0400）；②子 Agent 必须存在（A0401）；③绑定后不得形成环（DFS 检测，A0400 并回显环形路径） |

> 环校验的必要性：子 Agent 关联被 `DeepAgentBuilder` 递归展开，环会使推理期无限递归。
...
```mermaid
flowchart LR
    A["系统默认<br/>代码常量 REASONING_DEFAULTS"] --> B["Agent 配置<br/>sys_ai_agent.config JSON"]
...
| 护栏默认配置 | `ai:config:guardrail_defaults` | 10 分钟 | `ai_guardrail_defaults` 字典更新时失效 |
| 可选 Agent 列表 | `ai:agent:list:enabled` | 10 分钟 | Agent 启停/创建/删除时失效 |

> **失效时机**：上表失效统一登记为事务提交后（afterCommit）执行。提交前失效会被并发请求
> 击穿回填——新值尚未提交，回源读到的是旧数据，脏缓存存活至 TTL；事务回滚时失效自动丢弃。
...
sequenceDiagram
    ...
    AgentSvc->>Ver: 将当前可编辑态固化为草稿（门禁评测对象即最新草稿，保证被评测配置与发布配置一致）
    ...
    else 回归集无考题（insufficient_eval）
        AgentSvc-->>Admin: 阻断发布，提示先维护评测样本
    else 样本失败
        AgentSvc-->>Admin: 阻断发布，返回失败样本与差异点
    else 评分退化
        AgentSvc-->>Admin: 阻断发布，提示评分较上次完成评测退化超阈值
    end
```

**发布的事务边界**：回归评测是分钟级长跑，不得在请求事务内执行——发布先提交草稿（独立事务读不到未提交的草稿快照），再在独立 session 中执行评测，请求连接不随评测被长期占用；门禁失败时草稿保留供继续修订。

**门禁失败的三类文案**：回归集已配置但无考题（`insufficient_eval`）→"回归集无考题，请先维护评测样本后再发布"；评分较上次完成评测退化超阈值（`degraded`）→ 提示退化并建议回退修改；样本失败 → 回显失败样本明细。三类均为 A0502。无回归集的 Agent（首发）平凡放行，避免严格阻断造成发布死锁。

**版本号并发**：`version_no` 取号（MAX+1）与插入非原子，并发发布由 `(agent_id, version_no)` 唯一键兜底：冲突时在 SAVEPOINT 内回滚该次插入、按冲突号递增重试，重试用尽返回 A0501 提示重试发布。

**版本历史分页**：列表接口分页下推 SQL 且不加载 `snapshot` 大字段（历史随发布持续增长，全量加载后内存切片会把整个版本历史的快照 JSON 读进内存）；快照仅在版本详情/差异对比/发布读取时加载。

**回滚流程**：选取历史 `published` 版本 → 以其 `snapshot` 反序列化覆盖主表可编辑态 → 写入新版本记录（`status=published`，新版本号）。完整历史不覆盖，`change_note` 记录"回滚自 vN"。

**回滚目标限定**：回滚目标必须是已发布版本（`status=2`）。草稿是发布链路的评测中间态、配置未经门禁验证，传入草稿版本号返回 A0502（服务层单点校验，路由无旁路）。
...
**审计**：管理端危险操作写入 MongoDB `audit_log`（平台统一审计基建 `mongo_audit_log_repository`），字段为 `operator_id` / `target_type` / `target_id` / `action` / `module` / `before_value` / `after_value`：

| 操作 | 接口 | action | 记录内容 |
|------|------|--------|---------|
| 删除 Agent | `DELETE /api/v1/ai/agents/{id}` | `delete` | before：agent_code/name；after：级联清理数量（评测集/样本/执行记录） |
| 版本回滚 | `POST /api/v1/ai/agents/{id}/versions/{versionNo}/rollback` | `rollback` | after：from_version_no / to_version_no |
| 强制发布 | `POST /api/v1/ai/agents/{id}/publish`（`force=true`） | `publish_force` | after：version_no / change_note / drift_exempted |
| 删除 A2A 端点 | `DELETE /api/v1/ai/a2a/endpoints/{id}` | `delete` | before：name / base_url |

写入登记在事务的提交后回调（`defer_after_commit`）：事务回滚则不产生审计记录；回调为协程、异步落库不阻塞主流程（`create_audit_async` 为同步 fire-and-forget，需包一层 async 才能满足回调契约）。

**删除 Agent 的引用校验与级联**：

- 拒绝条件：默认 Agent（A0503）、存在使用该 `agent_code` 的活跃会话（A0504）、被其他 Agent 作为子 Agent 引用（A0504）；
- 级联清理评测资产：评测集软删 + 其样本物理删除（样本表无逻辑删除列，随评测集管理）+ 该 Agent 的评测执行记录物理删除（`sys_ai_agent_eval_run` 为只追加轨迹、无逻辑删除列）。Agent 软删后不再出现在任何管理入口，`agent_id` 不复用（重建同 `agent_code` 得到新 id），不级联则评测集列表与评测中心聚合将永久残留孤儿数据。
...
多 Agent 协作的沟通冗余、冲突、重复工作与上下文浪费治理（对齐需求规格 §2.11.4.1）：

| 机制 | 实现要点 |
|------|---------|
| 委派去重 | 委派前按"任务描述 + 参数"指纹查重，命中已执行结果（含 `sys_ai_artifact` 产物引用）直接复用，不重复执行 |
| 结果精简 | 子 Agent 返回结果摘要（已有）；大结果写 `sys_ai_artifact` 走产物引用，主 Agent 上下文只注入引用句柄 |
| 冲突消解 | 已实现：`before_tool` 注册点（`_write_conflict_hook`）+ `DehazeHooksMiddleware.awrap_tool_call` 的持有登记/释放闭环。写入资源键由 `write_resource_key` 识别——文件工具（`write_file`/`edit_file`/`delete`）按 `file_path`、MCP 写接口按 `post_`/`put_`/`patch_`/`delete_` 前缀 + 参数指纹，只读工具与沙箱命令不纳入；持有者身份为 Agent 名 + 优先级（子 Agent 取关联关系 `priority`，主 Agent 固定 0）+ 本次派发实例（同子 Agent 被并行派发时实例不同，判为冲突而非重入）。仲裁：低优先级后到 → 有界让行等待（`_WRITE_YIELD_TIMEOUT` 30s，超时按 `skip` 跳过本次写入）；同优先级 → 失败重试（`retry_max` 耗尽升级为写冲突 interrupt）；高优先级后到 → 持有者已开始执行不可抢占，同样让行等待，超时按失败重试。同 run 内该资源已被其他 Agent 写过 → interrupt 提示本次写入将覆盖既有结果，用户确认后放行且不再重复提示，拒绝则放弃写入。中断以 `confirmKind=dangerous_op` + `action=write_conflict` 落点，携带 `resource`/`previousWriter`；资源在工具执行 `finally` 释放（成功/异常/中止路径均释放，同名同实例重入计数归零才真正让出） |
| 冗余巡检 | `after_agent` 钩子统计重复委派/无用委派，写入过程链（消费可观测性 F-M08-013）标注 |
...
> **Python 端端点管理实现**：外部 A2A 端点路由 `app/router/ai_agent_endpoint.py`（prefix=`/api/v1/ai/a2a/endpoints`，全部需 `ai:agent:manage` 权限）——注册（POST，拉取并缓存 Agent Card）、更新（PATCH）、删除（DELETE）、分页列表（GET）、Agent Card 刷新（POST `/{id}/refresh-card`）。核心服务 `app/service/ai_agent_endpoint_service.py`（凭证 AES 加密存储、SSRF 防护）。

> **Python 端 A2A 标准协议端点实现**：由路由 `app/router/a2a.py` 承载，外部鉴权走 API Key（Bearer）。
> - **全局入口**（遵循 A2A 标准路径）：`POST /a2a` 与 `GET /.well-known/agent.json`（`global_router`）。`POST /a2a` 从 JSON-RPC `params.agentId`/`agentCode` 定位目标 Agent（`A2AServer.resolve_agent`）后走同一核心处理；`/.well-known/agent.json` 返回平台**首个可对外服务**（`is_exposed=1` 且启用且非子 Agent）Agent 的 Card（`resolve_default_exposed_agent`）。
> - **Agent 挂载路径**（按路径定位单 Agent）：`/api/v1/ai/agents/{agent_id}/a2a` 与 `/api/v1/ai/agents/{agent_id}/a2a/.well-known/agent.json` 保留，供既有按路径调用方使用。核心 JSON-RPC 处理已抽取 `_handle_a2a_request` 供两套入口共用，无重复逻辑。
...
评测集/样本管理、评测执行、四维评分、评测报告、生产持续评估与 Bad Case 回流由独立功能域 **Agent 评测（F-M08-014）** 承载，详见 [Agent评测/后端实现.md](../Agent评测/后端实现.md)。本功能域保留两个协作点：

| 协作点 | 说明 |
|--------|------|
| 门禁触发 | `AgentService` 发布 Agent 前调用 `EvalService` 执行回归集（`trigger_type=publish`），评分下降或 `risk_level=high` 样本失败则阻断发布，返回失败样本与差异点；发布前先检查判分漂移状态（评测中心 judge-status），`drifted` 时门禁暂停硬阻断（A0503），管理员 `force=true` 可豁免（`change_note` 前缀 `[漂移豁免]` 记录，不绕过回归门禁结果） |
| 审计轨迹 | 发布成功后 `sys_ai_agent_eval_run` 保留，支持回溯每次发布对应的评测结果 |

评测执行实现（`EvalRunner` 逐样本四维评分、平台专用 Token 池配额隔离）见 [Agent评测/后端实现.md §2.3](../Agent评测/后端实现.md#23-评测执行)。
...
推理参数全局默认值为代码常量（`REASONING_DEFAULTS`），不提供运行时配置；个性化由 §10.1 Agent 级配置覆盖。常量经 `GET /api/v1/ai/agents/config-defaults`（登录即可读）只读透出给 Agent 配置表单作"空值继承的系统默认"提示，避免前端硬编码随版本漂移；契约模型 `AgentConfigDefaults` 字段与常量键同名，由测试断言一致。
...
| 规则 | 检测通道 | 命中处置 |
|------|---------|---------|
| `prompt_injection` | 最后一条用户消息 + 工具结果（ToolMessage 与折叠进 assistant 的 `[工具调用结果]`）+ 记忆注入块（system 消息） | 拒绝本轮推理，返回固定文案 |
| `prompt_injection_warn` | 会话提示词拼接通道（`conversation_prompt`） | 仅告警留痕，不拦截（提示词由配置方提供，拦截会让会话不可用） |
| `unauthorized_access` | 工具调用的命名空间前缀；`mcp_execute_tool` 按其入参 `tool_name` 判定 | 拦截调用，返回越权提示 |
| `pii_mask` | 流式出口（`SseEventConverter` 推送前逐块脱敏）+ 落库前的 AI 消息 | 身份证/手机号/银行卡/密钥等替换为 `***` |

> 流式出口脱敏为基线防护（敏感串可能被 chunk 边界切断，转换器保留尾部候选窗口，流结束 flush），不受 `pii_mask` 开关影响；开关作用于落库通道。
