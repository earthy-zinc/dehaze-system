...
本文档描述 AI 对话模块**管理端 SKILL 管理页**的 Web 端（React）前端实现方案，聚焦工作流指令包（Skill）的市场分发与管理页面编排与交互。页面结构与交互规范见 [需求规格.md](./需求规格.md) §3.3.4，接口契约见 [API接口.md](./API接口.md) §2.14。Android / Flutter / Taro 等多端参考本 Web 端的组件结构与状态管理方案按各自技术栈适配。
...
```mermaid
flowchart TD
    SkillManagePage --> SkillMarketSection
    SkillManagePage --> SkillManageSection
    SkillMarketSection --> MarketSkillList
    SkillManageSection --> SkillTable
    SkillManageSection --> SkillFormDialog
    SkillManageSection --> SkillTestPanel
    SkillFormDialog --> SkillInstructionEditor
    SkillFormDialog --> SkillScriptConfig
```

| 组件 | 职责 |
|------|------|
| SkillManagePage | SKILL 管理页容器与路由入口，编排市场/管理两区 |
| SkillMarketSection | SKILL 市场区：预设/共享 Skill 目录（名称/描述/适用场景/步骤预览/启用状态） |
| MarketSkillList | 市场 Skill 列表，一键启用；展示已关联 Agent 数 |
| SkillManageSection | SKILL 管理区：全部 Skill 列表 + 创建/编辑/启停/删除/试运行 |
| SkillTable | Skill 表格：名称/描述/适用场景/启用状态/被 Agent 关联数/步骤数 |
| SkillFormDialog | Skill 表单弹窗：Markdown 指令 + 可选脚本/模板 |
| SkillInstructionEditor | Skill 指令编辑器（Markdown，含危险操作拦截校验提示） |
| SkillScriptConfig | 可选脚本/模板配置 |
| SkillTestPanel | Skill 试运行面板：输入测试数据 → 预览指令执行效果（不入库不推送） |

> Skill 的渐进式加载（会话启动仅加载名称描述，需要时加载完整指令）与执行机制见 [后端实现-能力扩展](./后端实现-能力扩展.md) §4；管理操作业务规则见 [需求规格-能力扩展](./需求规格-能力扩展.md) §2.6.11/§2.6.14。
...
| Markdown 编辑器 | Skill 指令编辑（含危险操作校验提示） |
