...
| [后端实现-模型管理](./后端实现-模型管理.md) | F-M08-007 模型管理 |
...
地基（模型与计费）→ 供应商/模型/Key/反馈 + 计费集成，见[后端实现-模型管理][后端实现-消息反馈]
```
...
| `sys_ai_provider` | 模型供应商配置（API 地址、协议类型、认证方式、健康检查开关、备注） | 模型管理 | 是（类别②，provider_code 不可复用） |
| `sys_ai_provider_key` | 供应商 API Key（加密存储、优先级+权重选取、日额度/频率限额） | 模型管理 | 否（状态控制：启用/禁用/过期） |
| `sys_ai_model` | AI 模型配置（计费比例、能力标识） | 模型管理 | 是（类别②，model_id 不可复用） |
...
所有 LLM 调用统一通过 `LlmClient`（`app/service/ai/llm_client.py`，单例 `llm_client`）发起，屏蔽不同提供商的协议差异，返回统一的流式数据结构 `LlmStreamChunk(type, content, usage, tool_call_id, tool_call_name)`：
...
| 认证 | API Key 从 `sys_ai_provider_key` 选取（按优先级+权重），运行时 AES-256 解密获取明文；详见 [后端实现-模型管理 §2.3 API Key 管理](./后端实现-模型管理.md#23-api-key-管理) |
