...
```
sys_ai_model ──引用──▶ sys_ai_conversation ──包含──▶ sys_ai_message
                              │                        │
                              │                        ├──分支──▶ sys_ai_message (parent_message_id)
                              │                        ├──推理──▶ sys_ai_agent_thought
                              │                        ├──产物──▶ sys_ai_artifact ──引用──▶ sys_file/sys_pred_log/sys_eval_log
                              │                        └──反馈──▶ sys_ai_message_feedback
                              │
                              └──计费────▶ sys_ai_billing

sys_ai_memory ◀──用户── sys_user
sys_ai_conversation.api_key_id ──关联──▶ sys_api_key

LangGraph 检查点（框架管理，不纳入本系统 schema，业务层通过 Checkpointer API 交互）
```

### 3.3 状态编码

各表状态字段取值统一使用 `tinyint`，由各端枚举类管理：

| 表 | 字段 | 取值 |
|----|------|------|
| `sys_ai_model` | `status` | `1=启用`，`0=禁用` |
| `sys_ai_conversation` | `status` | `1=活跃`，`2=已归档` |
| `sys_ai_message` | `status` | `1=流式输出中`，`2=已完成`，`3=失败`，`4=已取消` |
| `sys_ai_agent_thought` | `status` | `1=成功`，`2=失败`，`3=跳过` |
| `sys_ai_memory` | `status` | `1=启用`，`0=禁用` |
| `sys_ai_message_feedback` | `rating` | `1=点赞`，`-1=点踩` |

各端枚举类：Java 端 `AIMessageStatusEnum`、`AIAgentThoughtStatusEnum`；Python 端 `AIMessageStatus`、`AIAgentThoughtStatus`；Go 端 `AIMessageStatus`、`AIAgentThoughtStatus`。
...
```
START → evaluate_complexity ──路由──┬── direct_response → END
                                    ├── ReAct 循环
                                    ├── Plan-and-Execute 循环
                                    └── Reflexion 循环
                                            │
...
| 预校验 + 实扣减 | 推理前预校验配额，推理后按实际 Token 消耗扣减，失败不扣减 |
