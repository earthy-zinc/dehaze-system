...
| 原则 | 说明 |
|------|------|
| 能力与推理解耦 | 系统能力以"工具"形态接入，通过 MCP 网关动态发现。新增后端 API 后重启网关即生效（无定时同步），AI 对话模块零改动，这是模块可扩展性的根基 |
...
| 检查点存储 | 仅 Redis vs Redis + MySQL 双层 | 选 Redis 单层（`RedisSaver` 自定义实现，键 TTL 30 天，AOF 持久化覆盖崩溃恢复）。检查点是运行时可重建数据，落 MySQL 需随 LangGraph 版本维护 schema，收益低于成本；Redis 丢失时挂起状态从零重跑（历史消息靠上下文重建，会话不断头） |
...
└── CheckpointManager            # 检查点管理（LangGraph RedisSaver 自定义实现，直接传入 deepagents）
...
| 表 | 职责 | 所属功能域 | 逻辑删除 |
|----|------|-----------|---------|
| `sys_ai_agent` | 智能体配置（系统提示词、模型、推理范式、配置 JSON、类型标识、权限规则） | 智能体管理 | 是（唯一键含 deleted，软删后可重建同 agent_code） |
...
| `sys_ai_message_feedback` | 消息反馈（点赞/点踩） | 消息反馈 | 是（唯一键含 deleted，软删行不占键位） |
...
>
> 检查点不落库：只存 Redis（dehaze 自定义 `RedisSaver`，见 §4.3），无 `langgraph_*` 表，故不在此列出。
...
**Redis 运行时层**：checkpoint 是"高频写、低频恢复"的数据（每步推理都写、只在中断恢复时读），由 Redis 单层承载：

- **运行时**：LangGraph 每步推理自动通过 `RedisSaver` 读写 checkpoint，全部键（检查点/最新指针/待写）TTL 30 天，Redis 开启 AOF 持久化覆盖崩溃恢复。注：当前 langgraph 1.2.x 未内置官方 RedisSaver，`RedisSaver` 为自定义实现（继承 `BaseCheckpointSaver`，使用项目 `get_redis_client()`），按 LangGraph 标准 Checkpointer 协议实现，支撑中断/恢复；`aput_writes` 的待写合并走 Lua 原子执行（GET→SET 读改写会在并发写同一 checkpoint 时丢 writes）
- **键结构**：检查点 `ai:checkpoint:{threadId}:{ns}:{checkpointId}`、最新指针 `ai:checkpoint:latest:{threadId}:{ns}`、待写 `ai:checkpoint:writes:{threadId}:{ns}:{checkpointId}`，三类键同为 30 天 TTL
- **回收**：无主动清理入口——会话归档/删除不触碰检查点键（业务层不读写该命名空间），全部键由 30 天 TTL 统一回收，避免用户不再恢复的线程永久占内存
- **恢复策略**：Redis 键丢失时该 thread 挂起状态不存在，LangGraph 从零重跑（用户重发消息，历史消息靠 get_chain 上下文重建，会话不断头）；挂起中的中断点（interrupt）与 checkpoint 同存于 Redis（`ai:interrupt:{thread}`），同丢同灭，无不对称窗口
- **时间旅行**：通过 LangGraph Checkpointer API 可回溯历史检查点
- **业务层隔离**：业务代码不直接读写 checkpoint 存储，全部通过 LangGraph 的 Checkpointer API 交互

### 4.4 生命周期钩子（AgentHooks）

提供 Agent 推理全生命周期的钩子机制，支持在关键节点注入横切逻辑（安全、审计、计费、记忆等），避免硬编码到流程中。

deepagents 0.7.6 的 `AgentMiddleware` 只暴露 6 个入口——`abefore_agent` / `aafter_agent` / `abefore_model` / `aafter_model` / `awrap_model_call` / `awrap_tool_call`，**没有独立的 `before_tool` / `after_tool` 方法**，工具拦截统一走 `awrap_tool_call`。下表的六个钩子点是 dehaze 自有的注册口径（`AgentHooks` 注册表），由 `DehazeHooksMiddleware` 桥接到上述入口执行：

```
abefore_agent → awrap_model_call（before_model → LLM 调用 → after_model）
              → awrap_tool_call（before_tool → 工具执行 → after_tool）
              → aafter_agent
```

映射关系与适配器职责见 [后端实现-智能体管理 §3.3 关键适配器](智能体管理/后端实现.md)。
...
| `before_tool` | 每次工具调用前 | **并行子 Agent 写冲突仲裁**（资源占用登记/让行/失败重试/覆盖确认，见 [智能体管理 §5.3.1](智能体管理/后端实现.md)）、工具参数校验、权限校验、危险操作拦截（如删除文件确认） | 是（阻止调用） |
...
- **主/子 Agent 计费口径**：钩子在每个图 run 的 before_agent/after_agent 都会触发（主图与子图各自一次）。主图持有预扣-结算链路；子图 run（deepagents task 工具拉起，含 Team 多子图嵌套）经 run_context 子 Agent 作用域标记 is_subagent——跳过预扣、after_agent 走实报实销（`BillingService.settle_subagent`，独立记录 bill_type=chat_subagent，归属主会话用户/消息），不与主图预扣记录重复结算；子图未完成（异常/超时）不产生计费记录。**run 上下文传播机制（2026-09-13 修复）**：langchain factory 将 before_agent 挂为独立图节点（节点为独立 asyncio 任务，节点内 `set_run_ctx` 无法跨节点传播），run 上下文由驱动任务（`reasoning_service.run/resume`）在 `astream` 前经 `ensure_run_ctx` 预置，图内全部节点（含子 Agent）共享同一 dict；子 Agent 的 per-run 计数重置被抑制（计数按整 run 口径），`is_subagent` 经 ContextVar 判定（结算钩子以其为唯一口径，防子图二次结算主图预扣）。结算失败（如价格配置缺失）记 ERROR 日志不中断 after_agent 链（trace 落库/记忆提取不被计费异常吞掉）。详细口径见 [AI计费管理 后端实现 §4.5](../../基础模块/AI计费管理/后端实现.md#45-子-agent-计费口径父预扣--子实报实销)
- **主/子 Agent 生命周期钩子分流**：同 is_subagent 标记，子图 run 的 after_agent 跳过记忆提取与会话标题更新——子图消息源于任务描述，提取入长期记忆会污染用户画像，标题口径归属主会话首条对话
...
| 消息幂等键 | `ai:msg:idempotent:{userId}:{key}` | 180 秒（pending，流式超时 120 秒 + 60 秒余量）/ 5 分钟（已完成） | 发送时 SET pending（`SET NX` 原子抢占）；完成后更新为最终结果；失败或抢流锁失败时 DEL 允许重试 |
...
| 批量处理（并行上限 / 调度策略 / 单次最大图片数 / 异步阈值） | 5 / auto / 20 / 3 |
