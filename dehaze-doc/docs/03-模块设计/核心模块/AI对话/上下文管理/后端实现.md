...
### 4.1 上下文压缩（预算化摘要 + 强制驱逐）

压缩在**组装上下文后、推理前**按实际上下文 token 校验触发（非消息累计），采用业界软硬阈值 + 显著性分级 + 预算化摘要组合（对齐 Working Memory 管理的 Salience Scoring / Eviction Policy / Token Budget / Pinned Context 实践）：

```
组装上下文（ContextManager，见 §7.4）
    ↓
计算实际上下文 token（系统提示 + 记忆 + 摘要 + 最近 N 轮原文 + 工具定义 + 当前输入）
    ↓
校验① 输出空间预留：实际输入 > max_context − max_tokens？
    是 → 先压缩（走校验③），保证长回复输出空间，杜绝"压缩前撑爆"
    ↓
校验② 软阈值：实际输入 > max_context × 70%？
    是 → 后台异步触发预算化摘要压缩（不阻塞当前轮）
    ↓
校验③ 硬阈值：实际输入 > max_context × 90%？
    是 → 强制驱逐（安全阀）：按显著性从最低分起丢弃更早消息，保证组装完成不超限
    ↓
组装完成，进入推理
```

**摘要异步执行**：预算化摘要由后台任务执行（`asyncio.create_task`），当前轮先用强制驱逐（校验③）保证输入不超限；摘要完成后更新 summary，**下轮生效**——不拖慢当前首 Token 延迟（对齐可观测性 TTFT 指标）。
...
**输出空间预留**：模型窗口 = 输入 + 输出。组装校验时预留输出预算（`max_tokens`），实际输入 ≤ `max_context − max_tokens`，从源头防止"上一次回复过长撑满窗口后，下一轮输入无空间或长回复被截断"。
...
>
> `file_ref` 类型用于"用户消息附图"注册文件引用产物；消息附图以 markdown 图片 URL 内联在 `content` 中，无独立附件文件表机制，不注册 `file_ref`。
...
摘要压缩机制见 §4.1（预算化摘要：超阈值触发、保留最近 10 轮原文、summary 注入）。本节的记忆侧作用：被摘要消息中的关键决策、任务状态、用户偏好经 `extract_episodic` 提取为情景记忆存入 `sys_ai_memory`（长期、跨会话），与短期 `sys_ai_conversation.summary` 互补；由摘要巩固定时任务（每日 4 点执行）周期性推进，保证长期记忆随时间沉淀。
