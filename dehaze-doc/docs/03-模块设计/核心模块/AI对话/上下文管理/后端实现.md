...
| 分层注入 | 长期记忆不是统一检索注入，而是按类型分层：用户偏好常驻（每轮生效）、历史事件检索（语义相关才召回）、操作习惯场景触发（任务出现才注入）。工具定义按命名空间预筛选（见 [后端实现-能力扩展 §3](../能力扩展/后端实现.md)）。信息按需供给，而非全量供给 |
...
| 取舍点 | 方案对比 | 选择与理由 |
|--------|---------|-----------|
| 上下文组织 | 全部消息平铺 vs 分层结构（系统/摘要/最近原文/任务状态/产物引用） | 选分层。分层让"哪些信息必须保留、哪些可压缩"显式化，也为 Prompt Caching 优化提供"稳定前缀 + 动态后缀"的组装基础（见[AI模型管理/后端实现.md §2.10](../../../基础模块/AI模型管理/后端实现.md)） |
...
本功能域涉及两张表，表结构详见 [数据库设计 §4.7](../../../../02-系统架构/03-数据库设计.md)：
...
每次推理前，ContextManager 组装发送给 LLM 的消息列表。**组装为单点**：由 `reasoning_service.run` 内部调用 `build_context` 一次性完成，调用方无需传 messages/system_prompt，避免 build_context 重复执行导致记忆 touch 副作用翻倍：
...
1. 系统消息层（系统提示词 = 稳定层 + 变量层，详见 §3.3）
   ...
   - 记忆块以独立 system 消息注入、不参与历史裁剪，其 token 先从窗口预算扣除
    ↓
2. 对话历史层（token 预算窗口，2026-09-13 重构）
   - 沿 current_branch_message_id 的分支链回溯加载（单次查询边界 200 条链上消息）
   - 过滤 `deleted != 0` 的消息；工具调用结果（role=tool）折叠进前一条 assistant 消息
   - 产物引用行在本层裁剪前追加（见第 5 层），使其 token 计入预算；
     裁剪后再追加会让引用行整体撑出窗口
   - 按模型窗口计算历史 token 预算：
     max_context − max_output − 系统提示 − 摘要 − 记忆块 − 组装余量(2000)
   - 模型未在注册表登记时按保守窗口 32k 推算并打 warning：此时放弃裁剪会把整条链
     原文送入，必然超窗
   - 从最早消息起裁剪至预算内（至少保留最后一条），最近消息原文优先保留
   - 裁剪发生时写 truncate 事件（前后 token/条数）进过程链（F-M08-013）
   - 窗口按 token 而非固定条数划定：固定条数截断在大窗口模型下会让超出窗口的
     旧消息既不进上下文、也永远达不到摘要触发阈值，被静默丢弃
    ↓
3. 摘要层
   - 如果 summary 非空，作为 system 消息注入（位于记忆块之后）
   - "之前的对话摘要：{summary}"
    ↓
4. 任务状态层
...
系统提示词内部再分"稳定层"与"变量层"，稳定内容长期复用、动态内容随 Agent/会话注入，两者共同构成 Prompt Caching 的"稳定前缀 + 动态后缀"组装基础（见 [AI模型管理/后端实现.md §2.10](../../../基础模块/AI模型管理/后端实现.md)）：

| 层 | 内容 | 实现落点 | 变更频率 |
|----|------|---------|---------|
| 稳定层 | 平台级角色定位、通用行为准则、安全合规约束、输出格式规范 | 平台统一维护（固定前缀）；合规约束同时由 `GuardrailMiddleware` 独立拦截（见 [后端实现-智能体管理 §8.5](../智能体管理/后端实现.md)） | 极低 |
...
针对通用对话、图像处理调度、多步推理、算法推荐、定时任务等场景预设提示词模板。会话创建时根据场景选择对应默认提示词写入 `sys_ai_conversation.system_prompt`（见 [后端实现-会话与消息 §3.1](../会话与消息/后端实现.md)），保证同类任务行为一致、可预期。
...
结合消息反馈（F-M08-008）的点赞/点踩数据驱动提示词迭代，采用"**建议 + 人工审批**"流程（不自动修改）：

```
非安全类负反馈标签累计超阈值（如 tags=["too_long"] 出现 10 次）
    ↓
异步生成优化建议（LLM 产出差异说明，不直接改模板）
    ↓
平台管理员人工审批（三态）
    ├─ 确认 → 应用至提示词模板（新会话生效），经评测门禁（§3.3.5）
    ├─ 编辑后应用 → 管理员修改建议差异后应用（支持部分采纳，如仅采纳"回复保持简洁"、剔除不合适的其余建议）
    └─ 拒绝 → 丢弃建议，记录
```

**安全红线**：`harmful`（有害/不安全）等安全类反馈**不进入自动优化**，直接触发安全告警并人工介入——防止恶意点踩诱导削弱安全护栏（如对"拒绝生成有害内容"点踩）。优化不修改进行中会话的 `sys_ai_conversation.system_prompt`（避免中途行为突变）。

> 提示词优化是"数据驱动生成建议 + 人工把关"的闭环（Reflexion 范式的受控触发版）：LLM 负责产出优化方向，管理员负责把关落地，安全约束不交由反馈自动改写。同一反馈还会经记忆提取沉淀用户偏好（见 §7.2 反馈提取）。

#### 3.3.5 提示词模板库（可运维）

场景化提示词模板与稳定层提示词纳入模板库管理：版本化（草稿/已发布/回滚）、变更发布经 Agent 评测回归门禁（见 [Agent评测/后端实现.md](../Agent评测/后端实现.md) §2.5）、支持灰度验证后全量生效，平台管理员可运维。

---

## 4. 对话历史管理

### 4.1 上下文压缩（token 预算窗口 + 溢出摘要保底）

压缩在**组装上下文后、推理前**触发：组装时对话历史按 token 预算窗口裁剪（见 §3.2），凡存在"被窗口裁剪掉、且尚未摘要"的消息即触发增量压缩（对齐 Working Memory 管理的 Salience Scoring / Eviction Policy / Token Budget / Pinned Context 实践）：

```
组装上下文（ContextManager，见 §7.4；对话历史按 token 预算窗口裁剪，见 §3.2）
    ↓
存在"摘要水位之后、当前窗口之外"的未摘要消息？
  是 → 推理前同步触发增量摘要压缩（SummaryService.maybe_compress）
       （旧实现按"组装后 token > max_context × 70%"触发，但组装结果已被
         窗口裁剪、大窗口模型永远到不了阈值，窗口外消息被静默丢弃——
         2026-09-13 修复为窗口溢出即触发，摘要保底成立）
    ↓
压缩后的 summary 下轮生效（当前轮上下文已在触发前组装完成）
```

**显著性分级**与**预算化摘要**（保留清单/分块再压/前序再压缩/覆盖度检查）不变，摘要范围：

```
选取摘要范围（增量）：
  - 上界：summary_upto_message_id（上次摘要水位）
  - 下界：当前组装窗口的最早消息（token 预算动态计算，非固定条数）
```

> 输出空间预留已并入窗口预算计算（历史预算 = max_context − max_output − 系统提示 − 摘要 − 组装余量）；直接路径（`_run_direct`）内置同口径的输入预算裁剪。显式的显著性分级驱逐表（Pinned/可压缩/可驱逐）中"强制驱逐"部分为规划中。

**显著性分级**（决定保留优先级，规则前置 + LLM 摘要承载背景，不靠 LLM 自由发挥）：

| 级别 | 判定规则 | 处理 |
|------|---------|------|
| 豁免（Pinned） | 用户显式约束/偏好、决策（"选择 RIDCP"类）、关键数值/指标、任务状态、增量指令 | 压缩前先转结构化记忆/任务状态（双通道防丢）；摘要 prompt 列入**保留清单**，摘要中必须覆盖 |
...
选取摘要范围（增量）：
  ...
  - 下界：当前组装窗口的最早消息（token 预算动态计算，非固定条数）
    ↓
关键信息先固化（双通道防丢）：
  - Pinned 内容调用 extract_episodic 等转结构化记忆/任务状态（失败静默记日志）
    ↓
调用 LLM 生成摘要，Prompt 含明确预算与保留清单：
  - 目标：压缩后 summary token ≤ max_context × 15%
  - 保留清单：用户偏好/决策/关键数值/任务状态/增量指令
  - 输出超预算 → 分块再压（先压更早段，直至达标）
    ↓
更新 sys_ai_conversation：
  - summary：新摘要追加到旧摘要之后（"前序摘要：{旧}\n近期摘要：{新}"）
  - 旧摘要超 token 预算时，追加前先对旧摘要做一次再压缩（防长会话累积膨胀）
  - summary_upto_message_id：推进到本次覆盖的最后一条消息 ID
    ↓
覆盖度检查：抽样核对被压缩消息的关键点是否已入摘要/记忆
  - 缺失 → 补录记忆兜底（extract_episodic_from_summary）
    ↓
压缩事件（前后 token/丢弃范围/是否驱逐）写入过程链（消费可观测性 F-M08-013）
    ↓
后续组装上下文时，被摘要的消息不再发送原文，仅注入 summary 作为 system 消息
```
...
更新 sys_ai_message：
  ...
  - cached_input_tokens = 其中缓存命中的输入 token 数
    ↓
按用户售价换算积分（绝对单价，见 AI 模型管理模块 §2.12）：
  - credits = (inputTokens − cachedInputTokens) × input档位/1M + cachedInputTokens × cached档位/1M + outputTokens × output档位/1M
  - 从 sys_ai_model_price 读取模型的用户售价（高峰/空闲双档，按调用时刻取档）
    ↓
...
> 图片 URL 运行时通过 `FileService.getUrl()` 拼接，不落库（见 §2 数据模型）。
>
> 产物注册入口是工具执行结果（推荐/批量/评估链路），用户消息附图以 markdown 图片 URL 内联在 `content` 中，不注册产物——无独立附件文件表机制，也没有 `file_ref` 注册点。
...
异步触发记忆提取（after_agent 钩子）：
  - 子 Agent 子图 run 跳过（is_subagent）：子图消息源于任务描述，
    提取入长期记忆会污染用户画像，记忆口径归属主会话完整对话
  - 调用 LLM 分析本次对话
...
记忆/检索/工具结果等**注入内容**在进入 prompt 前与用户入口护栏一致过注入检测（对齐需求规格 §2.3.6 注入内容安全校验）：
...
`before_model` 钩子中任一组装环节异常（记忆检索/摘要加载/工具定义获取）捕获后降级：仅注入系统提示 + 最近 N 轮原文（跳过记忆/摘要/工具定义），保证推理可发起、用户永远能获得回复；降级事件写入过程链（F-M08-013）。
...
摘要压缩机制见 §4.1（预算化摘要：窗口溢出即触发，保留窗口为按 token 预算动态计算的条数、无固定"最近 N 轮"口径，summary 注入）。本节的记忆侧作用：被摘要消息中的关键决策、任务状态、用户偏好经 `extract_episodic` 提取为情景记忆存入 `sys_ai_memory`（长期、跨会话），与短期 `sys_ai_conversation.summary` 互补；由摘要巩固定时任务（每日 4 点执行）周期性推进，保证长期记忆随时间沉淀。
...
读取 Redis 当日计数 ai:multimodal:{userId}:{date}
  ...
  - 新会话不重置（用户维度全局计数，避免"开新会话绕过限额"）
    ↓
校验当日计数 < 用户等级日限额（sys_member_benefit.multimodal_limit）
    ↓
未超限：
  - Redis INCR 当日计数 + 1
  - 调用多模态模型读取图片 URL
