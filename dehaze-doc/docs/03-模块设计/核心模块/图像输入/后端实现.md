...
- 前端实现：`03-模块设计/核心模块/图像输入/前端实现.md`
...
| idx_user_favorite | (user_id, is_favorite, create_time DESC) | 收藏记录查询 |
...
| 默认排序 | ORDER BY is_favorite DESC, create_time DESC |
...
```typescript
// 伪代码
function checkQuota(userId: number): boolean {
  const userType = getUserType(userId)
  const quota = getQuotaByType(userType)
  const used = countHistoryRecords(userId)

  if (used >= quota.total) {
    // 配额已满，删除最旧的记录
    deleteOldestRecord(userId)
  }
...
| 方法名 | 说明 |
|--------|------|
| selectByUserId | 查询用户历史记录（分页） |
| selectByUserIdAndTimeRange | 按时间范围查询 |
| selectFavorites | 查询收藏记录 |
| countByUserId | 统计用户记录数量 |
| deleteOldestNonFavorite | 删除最旧的非收藏记录 |
...
```sql
-- 使用覆盖索引，避免回表
SELECT id, original_thumbnail_url, result_thumbnail_url,
       algorithm_name, create_time, status, is_favorite
FROM sys_input_history
WHERE user_id = :userId
ORDER BY is_favorite DESC, create_time DESC
...
```typescript
// 伪代码
function checkOwnership(recordId: number, userId: number): boolean {
  const record = getHistoryRecord(recordId)
  if (record.userId !== userId) {
    throw new ForbiddenException('无权限访问此记录')
  }
  return true
}
```
