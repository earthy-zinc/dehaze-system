...
合并说明：原 `useUploadStore`/`useSampleStore`/`useImageStore` 并入 `useImageInputStore`（上传队列、样例筛选、选中图片同属输入流程强相关状态，原命名易混）；原 `useDeviceStore` 并入 `useCameraStore`（设备能力检测仅服务于相机）。上传任务队列与进度归属 `useImageInputStore` 的 uploadQueue/progress，不再单列 uploadStore。
...
| 图像输入 -> 去雾处理 | imageUrl + taskType + source | 上传/拍照/样例/历史选择后统一输出给处理页 |
| 去雾处理 -> 图像输入 | 历史记录回写 | 处理完成后回写历史记录（原图/结果图/算法/耗时） |
...
上传前由前端计算文件 MD5，调用文件查询接口校验是否已存在：命中则复用已有文件 URL 跳过上传并 Toast 提示；未命中则正常上传并写入 md5Cache。去重适用于上传与拍照两种方式，文件指纹存储复用数据集管理模块，本模块不维护。
...
| FavoriteButton | 收藏按钮 | 复用收藏管理模块，targetType="result" |
