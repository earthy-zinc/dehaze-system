...
本文档描述算法选择模块的前端实现方案，聚焦组件架构、状态管理、路由设计等架构决策。算法选择模块是用户选择算法的核心入口，提供任务类型切换、树形浏览、智能推荐、关键词/拼音搜索和算法对比功能。会员权益见 [需求规格 §4.1](./需求规格.md)。
...
| 公共组件 | 配置要点 |
|---------|---------|
| AlgorithmCard | 长按进入对比多选 |
| AlgorithmTree | 按 taskType 过滤仅展示已发布算法 |
| AlgorithmComparison | 长按多选触发（上限 3），并排展示结果图 + 耗时 + 指标 |
| FavoriteButton | targetType=algorithm |
| 效果对比组件 | 复用效果对比模块 ParallelImageShow |
