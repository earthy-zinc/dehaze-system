...
本文档描述效果对比模块的后端实现方案，包括服务层设计、核心业务流程（效果评估、对比报告生成）、评估指标计算、异步任务设计与模块间协作。该模块在 dehaze-java、dehaze-python、dehaze-go 三端实现：dehaze-go 复用与 dehaze-java 相同的编排链路（评估/报告异步任务 + 轮询 Python 算法服务），指标计算统一收敛在 dehaze-python 的 `algorithm/metrics.py`。
...
| 创建报告任务 | 复用 sys_eval_log 表，status=1 处理中，task_type='report'（区分效果评估任务）；pred_url=处理结果图 URL，gt_url=原图 URL；result 字段存请求元数据（logId 等） |
...
|------|------|
| 查询算法名称 | 用于报告头部展示 |
| 渲染 HTML | 原图+结果图并排、算法名称/ID、生成时间（动态字段 HTML 转义，防报告页存储型 XSS） |
| 完成回写 | status=2，result 写入报告 HTML 与生成时间（JSON 结构见 §5.3） |
| 失败回写 | status=3，写入 errorMessage |

> 报告当前仅含头部/图片对比区/处理信息区，评估指标区与处理参数区为规划中（见[需求规格.md §2.8.3](./需求规格.md)）。
...
| 边界口径 | pred/gt 尺寸不一致直接失败；全同图 PSNR=inf 钳制为 100 dB；其余非有限指标值视为评估失败（详见[需求规格.md §2.5.2](./需求规格.md) 计算规则） |
| 返回指标列表 | [{label, value, better, description}] |
| 写入日志 | 写入 sys_eval_log.result（JSON 类型列，ORM 自动序列化，禁止手动 json.dumps 双重编码），status=2 |
| 读取归一化 | result 读取统一经 parse_eval_result：dict 直返、JSON 字符串（存量双重编码行/Java 写入路径）解析为 dict，解析失败按空处理 |
...
> **状态**：规划中，当前版本未实现评估缓存，重复评估会重新计算并重复扣减配额。
...
| 配置项 | 说明 |
|--------|--------|
| 任务载体 | sys_eval_log 表（taskId 即评估日志 ID，task_type='report' 与效果评估任务同表隔离） |
...
> 以上缓存均未实现，规划中。当前评估结果持久化于 sys_eval_log.result，任务状态经 GET 接口实时查询。
...
- 归属校验口径：评估任务状态查询、评估日志列表、报告状态查询/下载按 `create_by == 当前用户` 过滤或校验；报告生成按处理记录 `create_by` 校验，非本人资源一律按不存在处理（A0401），不泄露资源存在性
