...
| DraggableLine | 拖拽分隔线（用于重叠对比 F-M02-009，本期落地目标，尚未接入对比页） | 通用组件 |
...
- **受保护路由**：三个页面均为登录用户路由，导航守卫校验登录态。当前各会员等级在效果对比模块无功能差异，评估接口的会员配额校验由后端执行，配额不足返回业务错误前端展示提示。
...
使用 ECharts 渲染雷达图和柱状图。雷达图各维度使用**固定指标上限**（PSNR=100、SSIM=1、LPIPS=1、NIQE=10、NIMA=10、BRISQUE=100），保证不同评估结果横向可比，禁止按当前值动态缩放最大值。柱状图展示各项指标数值。指标表格展示指标名、数值（保留 4 位小数）、方向标记（↑越高越好 / ↓越低越好）、指标描述。

### 6.4 对比报告导出

报告格式固定 HTML（移除未实现的 PDF/PNG UI 选项，避免误导）。报告内容含图片对比区、处理信息区、评估指标区、处理参数与默认值对比。请求体仅 logId，指标与处理参数默认纳入报告。
...
| 公共组件 | 用途 | 配置要点 |
|---------|------|---------|
| ECharts 图表组件 | 雷达图/柱状图 | 固定指标上限保证横向可比，按需渲染 |
| 指标表格组件 | 指标名/数值/方向标记/描述 | 数值保留 4 位小数，方向标记区分越高/越低越好 |
| 报告导出组件 | 异步生成 HTML 报告 | 提交后轮询状态，完成后触发文件下载 |
| ParallelImageShow | 并排图片展示 | 自适应横/竖布局，同步缩放 + 放大镜 + 滤镜，跨三个页面复用 |
| DraggableLine | 拖拽分隔线 | 重叠对比（F-M02-009）本期落地目标，通用拖拽组件已具备待接入 |
