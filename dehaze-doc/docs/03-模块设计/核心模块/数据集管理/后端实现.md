...
- **任务管理**：[任务管理/后端实现.md](../../基础模块/任务管理/后端实现.md)（导入导出任务的调度与执行）
...
**导出范围与选项**：导出请求须在 queryParams 中指定导出范围之一（`datasetId` 导出整个数据集、`itemId` 导出单个数据项、`itemIds` 批量导出多个数据项），未指定任何范围时返回 RESOURCE_NOT_FOUND「未找到可导出的数据项」。导出选项参数（文件组织方式、包含图片类型、是否含缩略图等）属于 API 契约范畴，详见 [API接口.md](./API接口.md)。

**导入流程**：
1. 调用 `ImportExportService.import("dataset", params)` 提交导入任务，params 含上传 ZIP 的 `fileUrl` 与目标 `parentId`
2. 框架调度 `DatasetImportHandler` 执行：解析 ZIP 内元数据 JSON → 按原层级创建数据集（挂载到 `parentId` 下）→ 遍历图片文件，MD5 命中则跳过存储并记入报告，未命中则上传物理文件并建立关联
3. 完成后生成导入报告，任务结果可下载

> 导入图片的 MD5 去重策略与上传一致，详见 [需求规格.md §2.8.1](./需求规格.md)
...
> 该接口的 API 契约（路径、请求/响应结构）待 [API接口.md](./API接口.md) 补齐，本节仅描述后端服务层设计。
...
> 本模块核心查询依赖的索引设计详见 [数据库设计.md](../../../02-系统架构/03-数据库设计.md)
...
> 任务状态缓存由任务管理模块统一管理，详见 [任务管理/后端实现.md](../../基础模块/任务管理/后端实现.md)
...
| 并发任务限制 | 委托任务管理模块控制（`MAX_CONCURRENT_PER_USER` 常量已定义但未强制执行，见任务管理模块） |
...
> 导入导出任务的过期清理、僵尸任务修复由任务管理模块统一处理，详见 [任务管理/后端实现.md](../../基础模块/任务管理/后端实现.md)
