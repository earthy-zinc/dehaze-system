...
| 文件上传安全 | 扩展名白名单（jpg/jpeg/png/gif/bmp/webp）+ 魔数/PIL 内容解析校验 + 大小上限（`settings.MAX_UPLOAD_SIZE`，默认 100MB），清晰图与有雾图逐文件校验 |
| 防重复提交 | Redis 分布式锁，锁定时间 3 秒 |
| 并发任务限制 | 由任务管理模块的 `MAX_CONCURRENT_PER_USER` 常量控制（见任务管理模块） |

### 7.2 数据权限

- 数据集为共享管理资源：查询接口对登录用户开放，不按 `create_by` 数据隔离
- 写操作按 `sys:dataset:add/edit/delete` 权限标识控制（见 [API接口.md](./API接口.md) §3）；图片文件删除与数据项删除一致使用 `sys:dataset:delete`，图片上传/修改使用 `sys:dataset:edit`
- 三端实现：dehaze-python `require_permission` 装饰器、dehaze-go `middleware.Permission`、dehaze-java `@PreAuthorize("@ss.hasPerm(...)")`
...
| 防护项 | 说明 |
|---------|------|
| 标注完整性校验 | `haze_level` 字段支持多种规范格式（light/medium/heavy、beta=X、A=X,beta=Y 等），不做硬性枚举校验；`type` 仅允许 clear/hazy/trans/depth/segment |
| 批量标注 | 规划中（当前仅支持单图标注修改） |
