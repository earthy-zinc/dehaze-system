...
graph TB
    ...
    subgraph 基础模块-任务管理
        ...
        K[DatasetExportStrategy]
        L[ItemDownloadStrategy]
    end
...
- **任务策略注册**：DatasetExportStrategy 等策略实现在数据集模块内，注册到任务管理模块的 TaskStrategyFactory，由任务管理模块统一调度执行
...
| PairedImageValidator | 配对图片校验器 | 分辨率一致性、格式/大小限制、雾霾程度有效性 |
| DatasetExportStrategy | 数据集导出策略（注册到任务管理模块） | ZIP 打包、文件组织、进度回调 |
...
数据集的导出和下载功能通过任务管理模块的策略模式实现，数据集模块仅需：

1. **注册策略**：实现 `TaskStrategy` 接口，注册到 `TaskStrategyFactory`
...
| 策略名称 | 任务类型 | 功能描述 |
|---------|---------|---------|
| DatasetExportStrategy | `dataset_export` | 数据集导出为 ZIP 压缩包 |
| ItemDownloadStrategy | `item_download` | 单数据项下载 |
| BatchDownloadStrategy | `batch_download` | 批量下载（多数据项打包） |
...
| sys_item_file.type | ENUM(hazy/clear/depth/segment) | 图片类型枚举 |
| sys_item_file.thumbnail_file_id | 可空外键 | 关联缩略图文件 |
| sys_item_file.haze_level | ENUM(light/medium/heavy) | 仅 type=hazy 时有值 |
