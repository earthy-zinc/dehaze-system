...
- 不再写 `url`/`path` 字段，访问 URL 由 `storage.baseUrl + object_name` 运行时拼接
...
导出请求须在 queryParams 中指定导出范围之一：`datasetId`（导出整个数据集）、`itemId`（导出单个数据项）、`itemIds`（批量导出多个数据项）；未指定任何范围时返回 RESOURCE_NOT_FOUND「未找到可导出的数据项」。
...
<select id="selectWithFiles" resultMap="DatasetItemWithFilesMap">
    SELECT di.*, sif.*, sf.object_name, sf.storage, sf.size_bytes
...
> `sys_file` 不再存 `url` 字段，文件访问 URL 由响应层按 `storage.baseUrl + object_name` 运行时拼接。查询时取 `object_name` 与 `storage`，由 Service 层组装 VO 时动态生成 url。
