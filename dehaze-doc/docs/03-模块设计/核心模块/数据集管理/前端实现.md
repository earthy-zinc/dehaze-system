...
依据 [需求规格 §2.3.4](./需求规格.md)，图片为静态文件，必须经 nginx-dataset（端口 9000）访问，禁止经 Java 后端代理图片流：后端仅返回相对路径，前端拼接 `http://{host}:9000{相对路径}`；上传仍走 Java 后端 `/api/v1/item-files`（需鉴权、配额校验、MD5 去重）。

### 5.3 导入导出任务流

依据 [API接口 §2.4](./API接口.md)，导出/导入统一经任务管理模块 `/api/v1/tasks` 异步执行：创建任务 -> 加入 TaskCenterDrawer -> 轮询状态 -> 完成后下载（导出）或刷新数据集树（导入）；导入完成展示导入报告（成功数、跳过数含 MD5 命中重复、失败数及原因）。
...
| 交互点 | 技术选型 | 理由 |
|--------|---------|------|
| 大数量图像浏览 | 列表模式虚拟滚动 + 所有模式图片懒加载 | 数据集图片可达上千张，全量渲染卡顿；虚拟滚动仅渲染可视区域，懒加载降低带宽 |
...
| 图像预览 | 大图查看（缩放/拖动/旋转）+ 配对 Tab 切换 + 上一张/下一张导航 | 配对图按 haze_level 排序展示；URL 经 nginx 静态服务 |
...
| 配对上传校验 | 取消清晰图/退化图必填硬校验，仅保留分辨率一致性/格式/大小 | 适配多种数据集规范（GT+退化配对型/仅退化无 GT 型/多退化对一 GT 型） |
...
| 图像预览组件 | 缩放/拖动/旋转/配对切换/导航；URL 经 nginx-dataset:9000 拼接 |
| 进度条组件 | 上传进度与任务轮询进度展示 |
| 树形表格组件 | 数据集树形展示，第一级 task_type 分组，子节点懒加载 |
