...
本文档描述推荐管理模块的前端实现方案（dehaze-front-vue），聚焦组件架构、状态管理、路由设计、数据流与关键交互决策。推荐管理根据图像特征分析结果为用户推荐合适的图像处理算法，前端提供可嵌入业务页面的推荐位组件与管理员规则配置/效果报表页面。业务需求见[需求规格.md](./需求规格.md)。
...
> 推荐位采用 RecommendWidget 单组件承载推荐全流程（特征分析+推荐列表+反馈），不做 Widget/Card/Progress 等子组件拆分。推荐功能仅在算法选择页接入。
...
| ruleStore | loadRules | 加载规则列表与算法选项（用于名称映射） |
| ruleStore | saveRule | 新增/编辑规则 |
| ruleStore | toggleRule | 行内启停规则（删除为禁用） |

> 算法 ID -> 名称映射在 ruleStore 加载时完成。
...
| 规则更新 | 规则增删改后刷新规则列表；后端规则缓存（CopyOnWriteArrayList）写时复制，前端无需主动失效 |
...
| 规则删除语义 | 删除操作为禁用（enabled=false），规则保留可追溯，无物理删除 | 保留规则历史便于审计与效果分析 |
...
| 公共组件 | 使用方式 |
|---------|---------|
| 推荐卡片组件（RecommendCard） | RecommendWidget 内使用，展示算法名/匹配度/理由/操作 |
| 图表组件 | 效果报表页覆盖率/采纳率/趋势图表展示 |
| 表单组件 | 规则新增/编辑对话框使用，含场景类型/算法多选/权重滑块 |
| 进度条组件 | 匹配度可视化展示（颜色分级） |
| 折叠面板组件 | 特征分析结果展示 |
