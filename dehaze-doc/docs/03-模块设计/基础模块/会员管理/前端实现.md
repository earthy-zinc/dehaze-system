...
| Store | 核心状态 | 职责 |
|-------|---------|------|
| memberStore | profile（含等级、成长值、到期时间、权益配置） | 跨页面共享会员档案，提供 loadProfile（有缓存直接复用）、refreshProfile（强制刷新）、invalidate（会员信息变更后失效缓存） |

- memberStore 在用户登录后加载会员档案，等级/权益/成长值作为跨页面共享状态（套餐商城购买按钮文案依赖当前等级）
- 会员信息变更点失效缓存：支付成功后 invalidate（订单详情页），签到成功后 refreshProfile（会员中心页）
