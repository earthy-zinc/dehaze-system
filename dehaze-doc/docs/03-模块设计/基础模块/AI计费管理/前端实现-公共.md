...
组件统一放在 `src/components/billing/`，由用户端/管理端页面直接组合嵌入——**不设 BillingDataViews 容器组件**：数据范围的切换由 `useBillingDataStore(scope)` 键控实例承担（见 §3），各组件在挂载与 `scope` 变更时获取对应实例并重拉数据，省去一层无状态的壳组件。

```mermaid
flowchart TD
    Store[(billingDataStore<br/>按 scope 键控实例)]
...
| RecordStatusTags | `components/billing/RecordStatusTags.vue` | 行内状态标签组：降级标识（`actualModel` 非空，悬浮展示实际模型 `model`）、缓存节省（`creditsSaved > 0`）、申诉状态（`refundStatus`） |
...
| Store 模块 | 职责 | 生命周期 |
|-----------|------|---------|
| 计费数据 Store（billingDataStore） | 余额与配额、计费明细、积分流水；数据范围由 `scope`（self/userId）区分 | **按 scope 键控的 Pinia 多实例**（`self` 一个实例、每个 userId 一个实例），同 scope 的组件共享实例，不同 scope 互不干扰 |

> **为何按 scope 键控多实例而非全局单例**：管理端计费页会同时嵌入多个不同数据范围的公共组件（下钻面板按 `selectedUserId` 查看明细/流水，积分调整面板按 `form.userId` 查看余额/流水）。若全局单例共享一个 `scope`，后挂载的面板会覆盖数据范围，导致先挂载的表单在翻页/刷新时拉到错误用户的数据。按 scope 键控后每个数据范围独享实例，互不污染。

### 3.2 核心状态

| 状态 | 说明 |
|------|------|
| `scope` | 数据范围（`self`=本人 / `userId`=指定用户），由 `useBillingDataStore(scope)` 在获取实例时注入 |
...
| 操作 | 说明 |
|------|------|
| `initScope` | 注入数据范围（由 `useBillingDataStore` 包装函数在实例创建时调用，业务代码不直接使用） |
...
| `refreshRecordStatus` | 申诉提交/审核后刷新对应行申诉状态（本地置位，下次拉取以服务端为准；跨组件联动时按目标 scope 取对应实例调用） |
