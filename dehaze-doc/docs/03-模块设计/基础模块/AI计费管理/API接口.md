...
> **响应契约（重要）**：本模块**全部成功端点**统一返回 `{code, msg, data}` 信封结构（`code="00000"`、`msg="一切ok"`），与全局 API 规范一致。涉及端点：`/balance`、`/records`、`/credit-logs`、`/bills/{month}`、`/bills/{month}/download`、`/refunds`、`/stats`、`/adjust`、`/refunds/{id}/audit`。前端解析时必须取 `data` 字段。
