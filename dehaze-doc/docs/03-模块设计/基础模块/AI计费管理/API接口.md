...
| `/api/v1/ai-billing/summary` | GET | 消耗汇总查询（日/月消耗趋势、模型分布、节省汇总） | - | F-MB-009 |
| `/api/v1/ai-billing/records` | GET | 计费明细查询（分页查询自己的计费记录，含申诉状态） | - | F-MB-009 |
...
> 用户侧查询接口（余额/计费明细/流水/账单查询/账单下载/退款申请）均为登录态访问，仅可查询/操作本人数据。积分卡购买、支付与售后退款审核由订单管理模块承接，不在本模块定义；本模块退款申请接口仅面向**误扣补偿**场景。
