...
| KbConfigPanel | 配置管理区：嵌入公共 KbConfigForm（全量配置，模型选项从 [AI模型管理模块](../AI模型管理/API接口.md) 注册表拉取） |
...
| Store 模块 | 职责 | 生命周期 |
|-----------|------|---------|
| 管理端知识库 Store（adminKbStore） | 私有库监控、索引状态、召回测试、低质量片段 | 页面级，进入知识库管理页初始化，离开销毁 |
...
| 权限收敛 | 详情页/质量区按权限标识显隐 | 检索质量数据属高权能力，界面显隐与后端权限一致 |
