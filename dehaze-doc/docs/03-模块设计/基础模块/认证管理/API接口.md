...
| `/api/v1/auth/me` | GET | 获取当前用户权限信息（平铺结构，含 avatar 头像地址） | - | F-AM-005 |
| `/api/v1/auth/password` | PATCH | 修改个人密码（登录态，旧密码校验 + 新密码 8-20 位含字母数字，成功后踢出本人全部在线会话；契约登记见用户管理 API接口.md） | - | F-UM-005 |
...
| `A0210` | 用户名或密码错误 | 登录认证失败；个人改密旧密码校验失败 |
| `A0202` | 用户账户被冻结（禁用） | 禁用用户尝试登录 |
| `A0211` | 用户输入密码错误次数超限（临时锁定） | 连续 5 次登录失败锁定 30 分钟 |
| `A0213` | 验证码已过期 | 验证码超时 |
| `A0214` | 验证码错误 | 验证码校验失败 |
| `A0230` | token 无效或已过期 | Session 失效或过期；API Key 无效、已吊销或已过期 |
