...
| 用户名/密码错误 | 统一提示"用户名或密码错误"，清空密码与验证码 |
| 账户已禁用/锁定 | 显示对应业务提示 |
| 网络错误 | 显示网络连接失败提示 |
| 401 会话失效 | 清空 authStore 登录上下文，跳转登录页 |

**会话失效场景统一处理**：Session 过期、主动注销、被管理员踢出、账户被禁用/删除均会在下次请求收到 401，前端统一拦截并跳转登录页，不区分失效原因。
...
| 会话失效感知 | 统一拦截 401 响应，清空 Store 跳转登录页 | 被踢出/禁用/过期统一处理，逻辑收敛 |
...
| 搜索表单组件 | 登录日志、在线会话查询 | 复用通用搜索布局与输入防抖 |
| 表单弹窗组件 | API Key 创建 | 复用通用弹窗交互与表单校验 |
| 二次确认弹窗 | 注销、API Key 吊销、会话踢出 | 复用通用确认文案与交互 |
