...
>
> 幂等机制：三端 send 方法中先查询 `biz_module + biz_id + recipient_id`（在当前接收人列表中）是否已存在消息（提前失败），DB 层 `uk_biz_dedup(biz_module, biz_id, recipient_id)` UNIQUE INDEX 作为并发兜底，避免 select-then-insert 竞态产生重复消息。注意 MySQL 唯一索引对 NULL 不生效——仅当 `bizModule` 与 `bizId` 均非空时才防重；未传入幂等键的消息（如管理员手动触发的临时通知）不防重，允许重复生成。
...
> 消息详情接口（`GET /messages/{id}`）仅查询，不触发已读。前端打开详情页后需显式调用 `PATCH /messages/{id}/_read` 标记已读。
...
- **分批投递**：接收人列表按 `message.batch-insert.size`（默认 500）分批，每批独立事务提交，避免全体用户公告在单事务内长事务锁表。全部批次成功后更新公告状态为已发送并记录 `sentCount`
...
| 标记已读 | 读写事务 | 单表更新 |
| 公告发送 | 分批事务 | 每批独立提交（避免长事务），全部成功后更新公告状态 |
...
> **未读数缓存竞态说明**：「全部已读置0」与「新消息+1」若并发执行，可能出现新消息先 +1 随后被置 0 导致角标少 1 的窗口。该竞态由每小时全量刷新任务兜底修正，属可接受的短暂降级。
