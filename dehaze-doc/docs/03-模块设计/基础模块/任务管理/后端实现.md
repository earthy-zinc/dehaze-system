...
1. 参数校验、生成 UUID taskId、校验任务类型已注册
2. 查询同用户同类型未完成任务数，超限则返回 B0307
3. 保存任务记录(status=PENDING)，缓存任务状态
...
1. 查询任务当前状态，非 PENDING/PROCESSING 终态则返回 B0304
2. CAS 更新状态为 CANCELLED
3. 设置缓存取消标志位(TTL)，执行中策略下次检查时检测并退出
...
Exchange(`dehaze.task`, type: direct) → 主队列(`task.execute`) + 重试队列(`task.retry`, TTL 到期后转发回主队列) + 死信队列(`task.dlx`)。所有队列均开启持久化。
...
策略抛出异常 → 判断可重试性 → 不可重试：ACK + FAILED；可重试且未超上限：回退 PENDING → NACK → 进入重试队列(延迟后重新消费)；可重试但超上限：ACK + FAILED + 死信队列。
