...
> **说明**：通用策略本身不包含模块特定逻辑，仅负责文件生成/解析、进度回调、结果上传、状态更新等通用流程。各模块的导出/导入业务逻辑由对应的 `ExportHandler`/`ImportHandler` 实现，通过注册表查找。详见通用导入导出改造方案。
