...
graph TB
    ...
    subgraph 策略层
        E[GenericExportStrategy]
        F[GenericImportStrategy]
        G[其他业务策略...]
    end
...
> **接口变更说明**：原 `getTaskType()` 返回单个任务类型字符串，无法满足通用导入导出策略处理多个 taskType（如 `GenericExportStrategy` 需处理 user_export/role_export/dept_export 等 7 种类型）的需求。已改为 `getTaskTypes()` 返回 `List<String>`，工厂按多个 key 注册同一策略实例。
...
预测日志（`sys_pred_log`）和评估日志（`sys_eval_log`）在异步化改造后同样存在僵尸任务风险：Java `@Async` 线程池或 Go goroutine 在服务重启后会丢失，但 DB 中 `status='processing'` 的记录仍然存在。

**检测条件**：`status = 'processing' AND update_time < NOW() - INTERVAL 10 MINUTE`

**处理方式**：标记为 `failed`，`error_message = '任务执行超时，服务可能已重启'`

**Cron 表达式**：`0 */1 * * * ?`（每 60 秒扫描一次）

**实现位置**：

| 后端 | 实现类/文件 | 说明 |
|------|------------|------|
| Java | `PredLogRecoveryTask`、`EvalLogRecoveryTask` | `@Scheduled` + `@Async`，独立短事务批量更新 |
| Go | `internal/job/pred_log_recovery.go`、`eval_log_recovery.go` | cron 调度，单次批量 UPDATE |
| Python | `app/job/log_recovery.py` | APScheduler，asyncio.create_task |

**与 `sys_task` 僵尸任务修复的区别**：

| 项 | `sys_task` 僵尸修复 | `sys_pred_log`/`sys_eval_log` 僵尸修复 |
|----|---------------------|----------------------------------------|
| 触发场景 | MQ 消费者异常、节点宕机 | `@Async`/goroutine 进程重启 |
| 检测阈值 | 30 分钟 | 10 分钟（计算任务通常 10s~120s，10 分钟已足够判定异常） |
| 恢复方式 | RabbitMQ 重投消息 | 无法重投（异步任务已丢失），仅标记 failed |
| 是否可重试 | 是（MQ 重投） | 否（需用户重新提交） |
...
> 以下为三端任务管理模块在业务设计层面存在的**通用问题**（不区分语言）。

### 12.1 [P0] 取消任务接口三端不统一

三端取消任务接口的 HTTP 方法和路径不一致：
- Java/Go：`DELETE /api/v1/tasks/{taskId}`（DELETE 语义为删除资源）
- Python：`POST /api/v1/tasks/{taskId}/cancel`（POST + 动作子资源）

**统一方案**：三端统一为 `POST /api/v1/tasks/{taskId}/cancel`。取消是状态变更操作而非资源删除，使用 POST + 动作子资源更符合 RESTful 规范。SDK 同步修改。

### 12.2 [P0] 权限校验缺失（Java/Go）

Java 端 `getTaskStatus`、`getDownloadUrl`、`cancelTask` 方法缺少任务归属校验，任何登录用户可查询/下载/取消他人任务。Go 端 `DownloadExportFile`、`GetTaskStatus` 同样缺失。

**统一方案**：三端所有涉及任务操作的方法（查询状态、下载、取消、重试）均须校验 `task.createBy == currentUserId`，不匹配时返回 `B0302`（无权操作此任务）。Python 端已正确实现，Java/Go 对齐。

### 12.3 [P0] Go 端缺少下载接口

Go 端 Service 层有 `DownloadExportFile` 方法，但 API 层未注册 `GET /:id/download` 路由。

**统一方案**：Go 端补充下载路由和处理器，对齐 Java/Python。

### 12.4 [P1] Java 端 getTaskStatus 返回 null

Java 端 `getTaskStatus` 在任务不存在时返回 `null`，Controller 直接 `Result.success(null)`，前端收到 `data: null` 而非明确错误码。

**统一方案**：任务不存在时抛出 `BusinessException`，返回 `B0301`（任务不存在），对齐 Go/Python。

### 12.5 [P1] Java 端 cancelTask 无 CAS 更新

Java 端 `cancelTask` 使用 `updateById` 直接更新，无乐观锁/CAS，并发取消可能导致状态不一致。

**统一方案**：使用 CAS 更新 `WHERE status IN (PENDING, PROCESSING)`，更新失败说明状态已被并发改变，返回 `B0304`。

### 12.6 [P1] 异步任务可靠性不足（无重试/幂等/死信）

三端均缺乏任务幂等键，客户端重试产生重复任务。Java 端 MQ 消费者已落地（含幂等检查），Go 端 DLX 框架完整且 `handler.go` 已正确处理死信，Python 端 `aio-pika` 完整实现。

**改进方向**：任务创建引入客户端幂等键（`Idempotency-Key` 头），三端统一去重逻辑；失败任务提供重放入口与告警。

### 12.7 [P1] 任务进度实时推送机制不统一

Java/Go 端任务进度只能轮询 `getTaskStatus`，实时性差且增加 DB 负载。Python 端已有 WebSocket + Redis Pub/Sub 实现，Java/Go 未对齐。

**改进方向**：Java/Go 端对齐 Python 的 WebSocket + Redis Pub/Sub 方案；任务进度更新写 Redis 缓存（非每次写 DB），定期或完成时同步 DB；定义三端统一的 WebSocket 消息格式（进度百分比、状态变更、完成通知）。

### 12.8 [P1] 任务清理机制不一致

Java 端 `TaskCleanupJob` 已重构（抽取 `physicalDeleteTasks` / `markStuckTasksAsFailed` 模板方法）；Go 端 `CleanupJob` 已实现；Python 端已实现（过期清理/僵死回收/模型健康检查/孤儿文件/临时文件）。

**改进方向**：三端统一任务清理规则——7 天前已完成/取消任务删除、30 天前所有任务删除、24h 僵死任务标记 failed。

### 12.9 通用导入导出框架的三端对齐

通用导入导出框架同步修复以下三端差异，所有端点必须对齐：
...
| 重试时 retryCount | Java +1、Python 重置 0、Go 不变 | 统一为 +1（递增） |
| 任务类型枚举 | 三端枚举值不一致 | 统一扩展，包含所有新导入导出任务类型（见需求规格 §3.1.2） |
| TaskStrategy 接口 | `getTaskType()` 返回单个值 | 改为 `getTaskTypes()` 返回 `List<String>`，支持通用策略处理多个 taskType |
