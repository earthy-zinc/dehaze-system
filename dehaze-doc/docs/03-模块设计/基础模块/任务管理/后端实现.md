...
> 以下为三端任务管理模块在业务设计层面存在的**通用问题**（不区分语言），详细分析与三端现状对比见 [通用基础设施问题与改进](../../04-改造计划/通用基础设施问题与改进.md) §三。

### 12.1 [P0] 异步任务可靠性不足（无重试/幂等/死信）

三端均缺乏任务幂等键，客户端重试产生重复任务。Java 端 MQ 代码全部禁用但存在（双路径死代码），Go 端无 DLX 导致毒药消息无限重投。

**改进方向**：任务创建引入客户端幂等键（`Idempotency-Key` 头），三端统一去重逻辑；RabbitMQ 配置死信交换机 + 死信队列，消息 header 携带 `x-retry-count`，超阈值 reject 入死信；Java 端二选一删除 MQ 死代码或启用 MQ；失败任务提供重放入口与告警。
...
Java 端 `TaskCleanupJob` 因 Redis key 前缀错 + 状态大小写不匹配，清理逻辑是死代码；Go 端 `CleanupJob` 从未启动（`InitJobs` 未调用）且多个方法为 TODO 桩；Python 端已实现（过期清理/僵死回收/模型健康检查）。

**改进方向**：Java 端修复 `TaskCleanupJob` 常量引用与大小写问题；Go 端在 `app.Init()` 中调用 `job.InitJobs()` 并补全 TODO 桩；三端统一任务清理规则——7 天前已完成/取消任务删除、30 天前所有任务删除、24h 僵死任务标记 failed。
