...
本文档描述套餐管理模块的前端实现方案（dehaze-front-vue），聚焦组件架构、状态管理、路由设计与关键交互决策。套餐模块是会员商业化体系的商品中心，前端覆盖用户端套餐展示与购买入口，以及管理端套餐 CRUD、优惠券管理、促销活动管理与销售统计。
...
- packageStore 在商城页加载时通过 `PackageAPI.listOnSale` 获取在售套餐，权益对比表数据从列表派生（按权益项聚合）
- orderCreateStore 在用户点击购买时初始化，先调用 `calculate-price` 获取价格预览，再跳转订单创建流程；订单创建成功后跳转订单详情页
...
| 新增/编辑优惠券 | `package:coupon:add` / `package:coupon:edit` | 按钮隐藏 |
| 批量发放优惠券 | `package:coupon:distribute` | 按钮隐藏 |
| 删除优惠券 | `package:coupon:delete` | 按钮隐藏 |
| 创建/编辑促销活动 | `package:promotion:add` / `package:promotion:edit` | 按钮隐藏 |
...
| 决策点 | 实现方式 | 理由 |
|--------|---------|------|
| 套餐卡片展示 | 等级图标 + 促销价大字 + 原价删除线 + 日均价 + 权益列表平铺 | 价格信息层级清晰，权益直接展示免点击展开 |
| 购买按钮文案 | 按当前等级与套餐等级比较：同等级"续费"、更高等级"升级至此"、其余"立即开通" | 引导用户按需购买，避免重复购买同等级套餐 |
...
| 抽屉 | 销售统计 | 打开时按需加载统计数据 |
| 权益对比表 | 套餐商城 | 跨套餐权益聚合，当前等级列高亮，表头合并单元格 |
| 状态 switch | 套餐/活动上下架 | 直接调用接口切换状态，成功后刷新列表 |
