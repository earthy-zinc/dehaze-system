...
> **业务规则**：提交反馈时 `feedbackType` 枚举为 `suggestion/bug/experience/complaint`；`title` 长度 5-50、`content` 长度 10-1000；反馈类型不合规返回 `A0400`。匿名评价在用户端与后台列表均隐藏用户信息（userId/username/avatar 置空）。
