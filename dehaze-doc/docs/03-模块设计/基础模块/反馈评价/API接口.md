...
| 权限标识 | 功能 | 控制方式 |
|---------|------|---------|
| feedback:rating:edit | 评价管理（隐藏/标记） | 按钮显示 + 接口校验 |
...
| 状态值 | 显示 | 说明 |
|--------|------|------|
| pending | 待处理（橙色标签） | 用户已提交，等待管理员处理 |
| processing | 处理中（蓝色标签） | 已分配处理人，正在处理 |
| replied | 已回复（绿色标签） | 管理员已回复，等待用户确认 |
| closed | 已关闭（灰色标签） | 反馈已关闭 |
