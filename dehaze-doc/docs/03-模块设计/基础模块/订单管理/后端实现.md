...
flowchart TD
    ...
    E -->|存在| F[金额校验]
    F --> G{"回调金额 > 0 且 ≠ 应付金额?"}
    G -->|是| J["记录日志 + 抛 PAYMENT_AMOUNT_MISMATCH"]
...
2. 状态校验：订单状态 ∈ {paid(2), completed(3)}（否则 ORDER_STATUS_INVALID）
3. **退款时限校验**：`paid_time < NOW() - 7天` 抛 REFUND_TIME_EXCEEDED（三端统一）
4. **促销套餐校验**：套餐 `is_refundable=0` 抛 REFUND_NOT_SUPPORTED
5. **唯一性校验**：已存在退款记录抛 REFUND_ALREADY_EXISTS（+ 订单ID 唯一约束兜底）
6. **按天折算退款金额**：
...
| 金额校验 | 回调金额与订单应付金额一致，不一致抛 PAYMENT_AMOUNT_MISMATCH（三端统一） |
