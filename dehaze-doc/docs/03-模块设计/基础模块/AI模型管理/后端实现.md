...
- **取值规则**：`prefix + sha256(userId)` 十六进制，按 `max_len` 截断，**不含任何用户隐私信息**（合规要求）；未启用/无配置/无用户上下文/内置本地 provider（`provider_code=local`）均不注入
- **协议落位**：`openai_compat` 按配置的 `field` 注入请求体顶层，与 `extra_request_params` 同受核心键保护（字段已存在不覆盖，见 §2.5）；`anthropic` 按 `field` 嵌套路径注入（如 `metadata.user_id`，中间层不存在时初始化空对象），首段字段已存在（核心键）时不覆盖
...
排除 Redis 冷却标记的 Key（ai:provider_key:{key_id}:unavailable）
    ↓
按候选顺序原子预留额度（Lua：INCR 后判定，超限回滚；日/分钟各一键）
  - 日额度：ai:provider_key:{key_id}:daily:{date} vs daily_quota
  - 分钟频率：ai:provider_key:{key_id}:minute:{yyyyMMddHHmm} vs rpm_limit
  - 预留失败（并发抢占致超限）→ 换下一个候选；全部失败视为无可用 Key
    ↓
...
模型可用性测试结果存于 `last_test_status`（0 未测试/1 可用/2 不可用）、`last_test_at`、`last_test_error` 三列（§2.7.1），与 `status` 管理开关正交：测试不可用的模型不自动禁用，由管理员决策处置（修复供应商配置/禁用/下线）。
...
flowchart TD
    ...
    C -->|否| B2{"被其他启用模型的降级链引用（fallback_model_id）?"}
    B2 -->|是| D2["提示先调整引用方的降级链，避免其降级链静默断裂"]
    B2 -->|否| E["逻辑删除（deleted 写为行 id）"]
    E --> F["清除模型缓存"]
...
**管理端分页列表**（`list_models`）额外聚合两类可用性信息（需求规格 §2.8.4 生命周期"运行/可用性测试"行）：

| 字段 | 数据来源 | 说明 |
|------|---------|------|
| `last_test_status` / `last_test_at` / `last_test_error` | `sys_ai_model` 实体直出 | 最近一次可用性测试结果（§2.7.1） |
...
### 2.7.1 模型可用性测试（`model_test_service.test_model`）

管理员对单个模型发**最小真实推理请求**，验证模型在供应商侧真实开通——供应商连通性测试（§2.2）只验证供应商可访问与凭据有效，验证不了具体模型标识开通（模型 ID 不存在/无权限/未开通只有在真实推理时才暴露）。入口 `POST /api/v1/ai/models/{model_id}/test`（`ai:model:manage`）。
...
| embedding | `POST {base}/embeddings`，单词向量化 | cohere 特判 `/v1/embed`（body 加 `input_type`），同知识库 embedding_client 口径 |
...
- local 供应商（内置本地模型）复用同一探测逻辑（base_url 已配置为本地 OpenAI 兼容端点）；chat/embedding 探测前 `ensure_running` 拉起本地推理服务；local 无 rerank 模型注册
...
**启动播种（`model_seeder`，幂等）**：启动时确保 local provider、占位 Key、三个内置模型记录存在；**免费价格（全 0 价，input/cached/output × idle/peak）对每个内置模型独立幂等补齐**——价格补齐不依赖模型创建分支（存量环境模型已存在但缺价时同样补种），否则计费结算抛"未配置用户售价"、direct 轮次整条 trace 丢失（2026-09-13 修复）。`effective_from` 落地前截掉微秒：MySQL `DATETIME(0)` 对微秒四舍五入，xx.7s 进位成下一秒会使紧随其后的生效判定查不到刚插入的行，幂等检查落空触发重复插入。
