...
背景知识：
/Users/earthywu/dehaze-system/dehaze-python/new_dehaze.sql 是最新的图像去雾系统数据库表结构sql语句。目前已经存在代码设计文档，但是由于需求的修改，需要对代码设计文档进一步修改
dehaze-java/src/main/resources/db/migration/V2.0__dataset_module_extend.sql 是本次需求分析在修改之前的方案设计的数据库表结构修改语句。

你可以查阅数据库中图像数据集相关的表结构，以及dehaze-java中数据集相关的controller接口目前的修改情况
dehaze-java/src/main/java/com/pei/dehaze/controller/SysDatasetController.java
dehaze-java/src/main/java/com/pei/dehaze/controller/SysDatasetItemController.java
dehaze-java/src/main/java/com/pei/dehaze/controller/SysItemFileController.java
dehaze-java/src/main/java/com/pei/dehaze/controller/FileController.java
了解数据集、文件相关的接口和后端项目现状。
随后查看最新的需求分析文档：

demo/docs/01-产品概述和总体架构.md
demo/docs/07-数据集管理模块需求（01到04）

分析和改造/优化现有的代码设计文档 dehaze-java/doc/数据集管理模块-后端代码设计文档-01-数据集管理功能.md

重点考虑如何对后端进行改造来满足需求，请你在原有基础上进行优化和修改。以下是设计方案撰写要求：
...
请继续根据 dehaze-go/doc/数据集管理模块-后端代码设计文档-02-图片展示与浏览功能.md demo/docs/07-数据集管理模块需求-02-图片展示与浏览功能.md
  dehaze-go/test 为dehaze-go项目编写完善的测试代码

你是一位拥有15年经验、追求极致效率的资深软件架构师兼代码质量审计专家。现在需要为【模块名称】编写一份“代码设计文档”，输出要求如下：

1. **语言/框架无关**：禁止出现任何具体语言、框架、类名、方法名或真实代码。仅描述抽象架构、职责、数据流与业务规则。必要时可用通用伪代码表达复杂逻辑，但不得包含任何真实语法。
2. **遵循DRY**：先抽象通用逻辑（如CRUD、校验、异常、缓存、权限等），后续接口只描述与通用逻辑的差异。杜绝重复描述。
3. **结构化输出**（可按需调整）：
    - 模块概览与核心目标
    - 分层/组件架构图（Mermaid）
    - 核心实体关系或数据模型图（Mermaid ER）
    - 通用业务模式（含流程图/伪代码）
    - 各接口/功能的差异化逻辑
    - 数据流/时序图（Mermaid Sequence）
    - 关键业务规则与校验
    - 异常处理与事务策略
    - 缓存与性能优化建议
4. **可视化优先**：能用Mermaid表达的流程尽量用图示，复杂逻辑可配伪代码。简单逻辑直述即可，避免啰嗦。
5. **深度逻辑**：必须覆盖数据校验、权限、异常、事务、缓存、文件/任务处理、批处理、性能优化等关键点，逐层说明的数据流向与责任。
6. **禁止冗余**：只讲重点；若与通用逻辑一致则略写“同通用流程”，仅突出差异，简单一眼能懂的内容禁止写入文档！
7. **输出格式**：使用 Markdown，采用“##/###”标题，与 Mermaid/伪代码组合呈现；无需任何框架或实现细节。
8. **风格要求**：专业、精炼、逻辑清晰，确保结果可直接作为后端设计评审材料。

请在写作前根据现有需求与代码/接口梳理模块范围，确保输出完整、准确。

当前测试用例存在以下问题，请直接编辑代码并解决：

1. 测试注释过于冗长
   每个测试都有大段模板化注释，但实际价值有限。只保留必要注释，如必要的关键业务说明，删除冗余注释。测试方法名称能够自说明，能够清晰表达了测试意图。

2. 冗余的类型断言
   大量无意义的类型检查断言，这些应该由 TypeScript 类型系统保证，而非运行时测试。
   避免冗余的类型检查，只断言业务关键行为。
...
修改后需运行测试并保证测试通过无意外修改
`npx vitest run test/XXX/XXX.test.ts`
