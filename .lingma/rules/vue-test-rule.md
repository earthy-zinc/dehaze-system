...
description: 当你对 dehaze-front-vue 项目编写测试用例时，请务必参考该规则。
---

# dehaze-front-vue 项目测试规范

## 1. 测试架构概述

dehaze-front-vue 项目采用了完整的测试体系，包括单元测试和端到端(E2E)测试两个层面。
...
- **测试辅助工具**: 
  ...
  - [vitest-canvas-mock]() - Canvas 相关测试模拟
  - [jsdom]() - DOM 环境模拟
...
- 测试文件命名: `[组件名].spec.ts` 或 `[模块名].test.ts`
...
### 3.1 测试文件组织
- 端到端测试文件位于项目根目录的 [e2e](../../dehaze-front-vue/e2e/) 目录下
- 文件命名: `[功能].spec.ts`
...
对于特定组件的测试，可以使用以下命令：
```bash
npx vitest run src/components/Magnifier/__tests__/index.spec.ts --coverage
```
...
### 5.2 Mock策略
1. 外部API调用必须mock
2. 时间相关函数(Date.now, setTimeout等)需要mock
3. 随机值生成需要固定种子
4. 复杂的第三方库应适当mock
