 ...
 */
const CompareToolbar: React.FC<CompareToolbarProps> = ({ currentMode, resultUrl }) => {
  // 切换对比模式（统一使用 redirectTo 避免页面栈堆积）
  const handleSwitchMode = useCallback((mode: CompareMode, path: string) => {
  ...
  return (
    <View className="compare-toolbar">
      ...
      <View className="action-buttons">
        {actions.map((action) => (
          <View key={action.text} className="action-btn" onClick={action.onClick}>
