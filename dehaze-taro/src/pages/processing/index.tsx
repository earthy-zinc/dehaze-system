...
const PROCESS_STAGES = [
  { name: '图片预处理', range: [0, 10] },
  { name: '算法初始化', range: [10, 20] },
  { name: '去雾处理', range: [20, 90] },
  { name: '后处理优化', range: [90, 95] },
  { name: '结果保存', range: [95, 100] },
]
...
function estimateTime(algorithm: Algorithm | null): number {
  ...
  if (type.includes('cnn') || type.includes('gan') || type.includes('transformer') || type.includes('深度')) {
    return 6
  }
  if (type.includes('传统') || type.includes('dcp') || type.includes('retinex')) {
    return 2
  }
  return 4
}
...
const ProcessingPage: React.FC = () => {
  ...
  const startProgressSimulation = useCallback(() => {
    ...
    // 阶段推进：每 800ms 推进一个阶段（去雾处理阶段停留更久）
    const stageDurations = [800, 800, 2400, 600, 400]
    let currentStage = 0
    setStageIndex(0)

    const advanceStage = () => {
      currentStage += 1
      if (currentStage < PROCESS_STAGES.length) {
        setStageIndex(currentStage)
        const [start, end] = PROCESS_STAGES[currentStage].range
        // 在当前阶段范围内渐进
        const steps = 10
        const stepDuration = stageDurations[currentStage] / steps
        const increment = (end - start) / steps
        let stepCount = 0
        progressTimerRef.current = setInterval(() => {
          stepCount += 1
          setProgress(start + increment * stepCount)
          if (stepCount >= steps && progressTimerRef.current) {
            clearInterval(progressTimerRef.current)
            progressTimerRef.current = null
            if (currentStage < PROCESS_STAGES.length - 1) {
              stageTimerRef.current = setTimeout(advanceStage, 100) as unknown as ReturnType<typeof setInterval>
            }
          }
        }, stepDuration)
      }
    ...
    // 第一阶段立即开始
    const [start, end] = PROCESS_STAGES[0].range
    const steps = 10
    const stepDuration = stageDurations[0] / steps
    const increment = (end - start) / steps
    let stepCount = 0
    progressTimerRef.current = setInterval(() => {
      stepCount += 1
  ...
  }, [currentImage, selectedAlgorithm, params, startProgressSimulation, clearAllTimers])
      ...
      <View className="processing-content">
        ...
        {currentImage && (
          <View className="info-section">
            ...
            <View className="image-meta">
              ...
              <Text className="meta-item">{currentImage.width}×{currentImage.height}</Text>
          ...
          <View className="info-section">
            ...
            <View className="algorithm-info">
              ...
              {selectedAlgorithm.description && (
                <Text className="algo-desc">{selectedAlgorithm.description}</Text>
              )}
              <View className="algo-meta-row">
                {selectedAlgorithm.version && (
                  <Text className="algo-version">版本: {selectedAlgorithm.version}</Text>
                )}
                <Text className="algo-estimate">预估耗时: 约 {estimatedTime} 秒</Text>
              </View>
            </View>
          </View>
        )}
        ...
        {status === 'idle' && (
          <View className="info-section params-section">
            <View className="params-header" onClick={() => setShowParams(prev => !prev)}>
              <Text className="section-label" style={{ marginBottom: 0 }}>参数调节</Text>
              <Text className="toggle-text">{showParams ? '收起' : '展开'}</Text>
            </View>
              ...
              <View className="params-body">
                <View className="param-item">
                  <View className="param-row">
                    ...
                    <Text className="param-value">{params.intensity}</Text>
                  </View>
                  <Slider
                    min={0}
                    max={100}
                    value={params.intensity}
                    onChanging={(e) => setParams(prev => ({ ...prev, intensity: e.detail.value }))}
                  />
                </View>
                ...
                <View className="param-item">
                  ...
                  <Slider
                    ...
                    onChanging={(e) => setParams(prev => ({ ...prev, saturation: e.detail.value }))}
                  />
                </View>
                ...
                <View className="param-item">
                  ...
                  <Slider
                    ...
                    onChanging={(e) => setParams(prev => ({ ...prev, contrast: e.detail.value }))}
                  />
                </View>
                ...
                <View className="param-item">
                  ...
                  <Slider
                    ...
                    onChanging={(e) => setParams(prev => ({ ...prev, sharpen: e.detail.value }))}
                  />
                </View>
        ...
        {status === 'processing' && (
          <View className="status-section processing">
            ...
            <Text className="status-hint">
              {PROCESS_STAGES[stageIndex]?.name} · 已用 {formatTime(elapsedTime)}
            </Text>
            ...
            <View className="progress-wrapper">
              ...
              <Text className="progress-text">{Math.min(Math.floor(progress), 100)}%</Text>
            </View>

            {/* 阶段列表 */}
            <View className="stages-list">
              {PROCESS_STAGES.map((stage, idx) => (
                <View key={idx} className={`stage-item ${idx < stageIndex ? 'done' : ''} ${idx === stageIndex ? 'active' : ''}`}>
                  <View className="stage-dot">
                    {idx < stageIndex ? <Text>✓</Text> : <Text>{idx + 1}</Text>}
                  </View>
        ...
        {status === 'success' && result && (
          <View className="status-section success">
            <View className="success-icon"><Text>✓</Text></View>
            <Text className="status-text">处理完成</Text>
            <Text className="status-hint">
              耗时 {formatTime(result.time)}{result.fromCache ? ' · 缓存命中' : ''}
            </Text>
            ...
            <View className="action-buttons">
              <View className="btn btn-primary" onClick={() => Taro.navigateTo({ url: '/pages/side-by-side/index' })}>
                <Text>效果对比</Text>
              </View>
        ...
        {status === 'error' && (
          <View className="status-section error">
            <View className="error-icon"><Text>!</Text></View>
            ...
            <View className="action-buttons">
              ...
              <View className="btn btn-secondary" onClick={handleBackToAlgorithm}>
                <Text>更换算法</Text>
              </View>
            </View>
          </View>
