...
/** 任务状态标签映射 */
const TASK_STATUS_TAG: Record<string, { label: string; color: 'default' | 'primary' | 'success' | 'danger' }> = {
...
const WORKFLOW_ENTRIES = [
  { icon: '📷', title: '图像输入', desc: '上传或拍摄图片', route: '/pages/image-input/index' },
  { icon: '💡', title: '算法选择', desc: '选择去雾算法', route: '/pages/algorithm-select/index' },
  { icon: '⚙️', title: '去雾处理', desc: '执行去雾处理', route: '/pages/processing/index' },
  { icon: '📊', title: '效果对比', desc: '对比处理效果', route: '/pages/side-by-side/index' },
]
...
const Dashboard: React.FC = () => {
  ...
  /** 渲染统计卡片 */
  const renderStatCard = (label: string, value: number | null, color: string) => (
    <View className='stat-card' key={label}>
      <Text className='stat-value' style={{ color }}>{value === null ? '-' : String(value)}</Text>
      ...
      <View className='dashboard-page'>
        ...
        <View className='welcome-section'>
          <View className='user-avatar'>
            {user?.avatar ? (
              <Text className='avatar-text'>{user.nickname?.[0] || user.username?.[0] || 'U'}</Text>
            ) : (
              <Text className='avatar-text'>{user?.nickname?.[0] || user?.username?.[0] || 'U'}</Text>
            )}
          </View>
          <View className='user-info'>
            <Text className='user-name'>{user?.nickname || user?.username || '用户'}</Text>
        ...
        <View className='section'>
          <View className='section-header'>
            ...
            <Text className='section-more' onClick={() => handleNavigate('/pages/task/index')}>查看全部 ›</Text>
          </View>
          {loading ? (
            ...
            <View className='task-list'>
              {recentTasks.map(task => {
                const tagInfo = TASK_STATUS_TAG[task.status] || { label: task.status, color: 'default' as const }
                return (
                  <View
                    key={task.taskId}
                  ...
                  >
                    <View className='task-info'>
                      <Tag color={tagInfo.color} size='small'>{tagInfo.label}</Tag>
