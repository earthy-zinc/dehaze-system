        ...
        <View className='preview-info'>
          ...
          <View className='info-item'>
            ...
            <Text className='info-value'>
              {imageData.width > 0 ? `${imageData.width} × ${imageData.height}` : '-'}
            </Text>
          </View>
          {imageData.compressed && (
            <View className='compress-hint'>
              <Text>图片已自动压缩（原始: {formatFileSize(imageData.originalSize || 0)}）</Text>
            </View>
          )}
          {imageData.sampleInfo && (
            <View className='sample-info'>
              ...
              {imageData.sampleInfo.recommendAlgorithm && (
                <View className='sample-row'>
                  ...
                  <Text className='sample-value recommend'>{imageData.sampleInfo.recommendAlgorithm}</Text>
                </View>
              )}
            </View>
          )}
