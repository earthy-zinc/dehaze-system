...
import { Navbar, Loading, Empty, Tag, Button, Popup, Progress } from '@taroify/core'
...
/** 状态标签映射 */
const STATUS_TAG: Record<TaskStatus, { label: string; color: 'default' | 'primary' | 'success' | 'danger' }> = {
...
const TaskPage: React.FC = () => {
    ...
    Taro.showModal({
      ...
      success: async (res) => {
        ...
        try {
          ...
          const updated: TaskVO = { ...task, status: 'CANCELLED', completedAt: now }
          setTaskList(prev => prev.map(t => t.taskId === task.taskId ? updated : t))
  ...
  const renderTaskCard = (task: TaskVO) => {
    const tagInfo = STATUS_TAG[task.status] || { label: task.status, color: '#8c8c8c' }
    const isActive = POLLING_STATUSES.includes(task.status)
    const canDownload = task.status === 'COMPLETED'

    return (
      <View key={task.taskId} className='task-card' onClick={() => handleDetail(task)}>
        ...
        {detailTask && (
          <View className='detail-content'>
            <View className='detail-header'>
              ...
              <Text className='detail-close' onClick={handleDetailClose}>关闭</Text>
            </View>

            {renderDetailItem('任务ID', detailTask.taskId)}
            {renderDetailItem('任务类型', TASK_TYPE_LABEL[detailTask.taskType || ''] || detailTask.taskType || '-')}
            {renderDetailItem(
              '状态',
              <Tag color={STATUS_TAG[detailTask.status]?.color || 'default'} size='small'>
            ...
            {detailTask.totalFiles != null && renderDetailItem(
              '文件处理',
            ...
            {detailTask.expiresAt && renderDetailItem('过期时间', formatTime(detailTask.expiresAt))}
            ...
            <View className='detail-footer'>
              ...
              {TERMINAL_STATUSES.includes(detailTask.status) && detailTask.status !== 'COMPLETED' && (
                <Button block onClick={handleDetailClose}>关闭</Button>
              )}
            </View>
          </View>
        )}
