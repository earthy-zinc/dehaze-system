...
const DatasetFormDialog: React.FC<DatasetFormDialogProps> = ({
  ...
  const [selectedParent, setSelectedParent] = useState<DatasetOption | null>(null)
  ...
  useEffect(() => {
    if (visible) {
      if (mode === 'edit' && dataset) {
        ...
        setSelectedParent(options.find(o => o.value === dataset.parentId) || null)
      } else {
        // 新增模式
        setFormData({
        ...
        setSelectedParent(options.find(o => o.value === defaultParentId) || null)
      }
      setError('')
    }
    ...
    >
      <View className="dataset-form-dialog">
        ...
        <View className="dialog-header">
          <Text className="dialog-title">{mode === 'create' ? '新增数据集' : '修改数据集'}</Text>
        ...
        <View className="form-body">
          ...
          {/* 名称 */}
          <View className="form-item">
            <Text className="form-label">数据集名称 <Text className="required">*</Text></Text>
            <Input
          ...
          {error && <View className="form-error"><Text>{error}</Text></View>}
        </View>
          ...
          <View className="select-list">
            ...
            {options.map(opt => (
              <View
                ...
                onClick={() => {
                  setFormData(prev => ({ ...prev, parentId: opt.value as number }))
