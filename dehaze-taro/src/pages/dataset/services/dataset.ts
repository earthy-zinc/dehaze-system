...
export class DatasetService {
  ...
  // 获取数据集图片列表
  static async getDatasetImages(
    params: DatasetImageListParams
  ): Promise<ApiResponse<PaginatedResponse<DatasetImage>>> {
    await delay(400);
    ...
    if (params.image_type && params.image_type !== "all") {
      allImages = allImages.filter(
        (img) => img.image_type === params.image_type
      );
    }
