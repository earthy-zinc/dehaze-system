...
const TYPE_WEIGHT: Record<string, number> = {
  // 深度学习类
  cnn: 10, gan: 9, transformer: 10, '深度学习': 10, deeplab: 9,
  // 混合类
  '混合': 7, hybrid: 7,
  // 传统类
  '传统': 5, dcp: 5, retinex: 5, '暗通道': 5,
}
...
function getStatusInfo(status?: number) {
  switch (status) {
    case 0: return { label: '草稿', className: 'status-draft' }
    case 1: return { label: '测试中', className: 'status-testing' }
    case 2: return { label: '待审核', className: 'status-pending' }
    case 3: return { label: '已发布', className: 'status-published' }
    case 4: return { label: '已停用', className: 'status-disabled' }
    case 5: return { label: '已归档', className: 'status-archived' }
    default: return { label: '未知', className: 'status-unknown' }
  }
}
...
const AlgorithmSelectPage: React.FC = () => {
  ...
  // 所有叶子算法
  const allLeafAlgorithms = useMemo(() => collectLeafAlgorithms(algorithms), [algorithms])

  // 智能推荐：从已发布叶子算法中按类型权重排序取前3
  const recommendedAlgorithms = useMemo(() => {
    const published = allLeafAlgorithms.filter(a => a.status === PUBLISHED_STATUS)
    return published
  ...
  // 递归搜索过滤
  const filterTree = useCallback((nodes: Algorithm[], keyword: string): Algorithm[] => {
      ...
      >
        ...
        <View className="recommend-info">
          <View className="recommend-header">
            ...
            <View className={`recommend-tag type-${getTypeWeight(algorithm.type) >= 9 ? 'dl' : getTypeWeight(algorithm.type) >= 7 ? 'hybrid' : 'traditional'}`}>
        ...
        <View
          className="fav-btn"
          onClick={(e) => { e.stopPropagation(); toggleFavorite(algorithm) }}
        >
      ...
      <View key={node.id}>
        ...
        >
          ...
          {hasChildren ? (
            ...
            <View className="leaf-icon"><Text>⚡</Text></View>
          )}
          <View className="node-content">
            ...
            {isLeaf && (node.version || node.size || node.flops) && (
              <View className="node-meta">
                {node.version && <Text className="meta-text">v{node.version}</Text>}
          ...
          {isLeaf && (
            <View className="node-actions">
              ...
              <View
                className="action-icon"
                onClick={(e) => { e.stopPropagation(); setDetailAlgorithm(node) }}
              >
      ...
      <ScrollView className="main-scroll" scrollY>
        ...
        {!searchKeyword && recommendedAlgorithms.length > 0 && (
          <View className="recommend-section">
            ...
            <View className="recommend-list">
              {recommendedAlgorithms.map((algo, idx) => renderRecommendCard(algo, idx))}
            </View>
          </View>
        )}
        ...
        {!searchKeyword && favoriteAlgorithms.length > 0 && (
          <View className="favorite-section">
            <View className="section-header">
              ...
              <Text className="section-hint">{favoriteAlgorithms.length} 个算法</Text>
            </View>
        ...
        <View className="algorithm-tree-wrapper">
          {loading ? (
            ...
            <EmptyState type="search" title="未找到算法" description="请尝试其他关键词" />
          ) : (
        ...
        {detailAlgorithm && (
          <View className="detail-popup">
            <View className="detail-header">
              ...
              <View className="detail-close" onClick={() => setDetailAlgorithm(null)}>
                <Text>✕</Text>
              </View>
            </View>
            <ScrollView className="detail-body" scrollY>
              ...
              {detailAlgorithm.version && (
                <View className="detail-item">
                  ...
                  <Text className="detail-value">{detailAlgorithm.version}</Text>
                </View>
              )}
              ...
              <View className="detail-item">
                ...
                <View className={`status-tag ${getStatusInfo(detailAlgorithm.status).className}`}>
              ...
              {detailAlgorithm.description && (
                <View className="detail-item detail-desc-item">
                  ...
                  <Text className="detail-value detail-desc">{detailAlgorithm.description}</Text>
                </View>
              )}
              {detailAlgorithm.createTime && (
                <View className="detail-item">
                  <Text className="detail-label">创建时间</Text>
                  <Text className="detail-value">{detailAlgorithm.createTime}</Text>
                </View>
              )}
            </ScrollView>
