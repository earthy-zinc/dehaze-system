...
const RolePermissionPage: React.FC = () => {
  ...
  const { permissions, fetchPermissions, fetchRolePermissions, assignPermissions } = useRoleManagement();
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // 设置已选中的权限（转换为字符串数组）
      setCheckedPermissions((rolePermissionIds || []).map(id => String(id)));
  ...
  const isAllSelected = useMemo(() =>
    checkedPermissions.length === allPermissionIds.length && allPermissionIds.length > 0,
  ...
  const handleSave = useCallback(async () => {
    try {
      setSaving(true);
      // 转换为数字数组
      const permissionIds = checkedPermissions.map(id => Number(id));
      ...
      <View className="role-permission-page">
        ...
        <View className="permission-tree">
          {permissions.length === 0 ? (
            <View className="empty-permissions">
              暂无权限数据
            </View>
          ) : (
            <Checkbox.Group
