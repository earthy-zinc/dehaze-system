...
/** 状态信息映射 */
const STATUS_INFO: Record<number, { label: string; color: 'default' | 'primary' | 'success' | 'warning' | 'danger' }> = {
...
function filterTree(nodes: Algorithm[], keyword: string, statusFilter: number | ''): Algorithm[] {
  ...
  const match = (algo: Algorithm): boolean => {
    const nameMatch = !keyword || (algo.name || '').toLowerCase().includes(lowerKeyword)
    const typeMatch = !keyword || (algo.type || '').toLowerCase().includes(lowerKeyword)
...
const AlgorithmManagePage: React.FC = () => {
    ...
    Taro.showModal({
      ...
      success: async (res) => {
        ...
        try {
          ...
          // 本地更新
          setAlgorithms(prev => updateAlgorithmInTree(prev, algo.id, { status: newStatus }))
          setDetailAlgo(prev => prev?.id === algo.id ? { ...prev, status: newStatus } : prev)
          Taro.showToast({ title: `${actionText}成功`, icon: 'success' })
        } catch (err: any) {
          Taro.showToast({ title: err?.message || `${actionText}失败`, icon: 'none' })
        } finally {
  ...
  const handleAuditSubmit = useCallback(async () => {
    ...
    try {
      const form: AlgorithmAuditForm = { approved: auditApproved, remark: auditRemark.trim() || undefined }
      await AlgorithmAPI.auditAlgorithm(auditAlgo.id, form)
      const newStatus = auditApproved ? 3 : 1
      setAlgorithms(prev => updateAlgorithmInTree(prev, auditAlgo.id, { status: newStatus }))
      setDetailAlgo(prev => prev?.id === auditAlgo.id ? { ...prev, status: newStatus } : prev)
      setAuditVisible(false)
      Taro.showToast({ title: auditApproved ? '审核通过' : '已驳回', icon: 'success' })
    } catch (err: any) {
  ...
  function removeAlgorithmFromTree(nodes: Algorithm[], id: number): Algorithm[] {
    return nodes
      ...
      .map(node => node.children ? { ...node, children: removeAlgorithmFromTree(node.children, id) } : node)
  }
      ...
      >
        <View className='node-main'>
          ...
          <View className='node-meta'>
            ...
            {algo.version && <Text className='node-version'>v{algo.version}</Text>}
          </View>
        </View>
        ...
        {!hasChildren && (
          <View className='node-actions' onClick={(e) => e.stopPropagation()}>
            {isPending && (
              <>
                <Button size='mini' color='success' onClick={() => handleOpenAudit(algo, true)}>通过</Button>
                <Button size='mini' color='danger' onClick={() => handleOpenAudit(algo, false)}>驳回</Button>
              </>
            )}
    ...
    <View className='algo-manage-page'>
        ...
        {detailAlgo && (
          <View className='detail-content'>
            <View className='detail-header'>
              ...
              <Text className='detail-close' onClick={() => setDetailVisible(false)}>关闭</Text>
            </View>
            ...
            <View className='detail-section'>
              ...
              {renderDetailItem(
                '状态',
                <Tag color={STATUS_INFO[detailAlgo.status ?? 0]?.color || 'default'} size='small'>
              ...
              {detailAlgo.status === 2 && (
                <>
                  <Button
                    block
                    color='success'
                    onClick={() => { setDetailVisible(false); handleOpenAudit(detailAlgo, true) }}
                  >
                    审核通过
                  </Button>
                  <Button
                    block
                    color='danger'
                    onClick={() => { setDetailVisible(false); handleOpenAudit(detailAlgo, false) }}
                  >
                    审核驳回
                  </Button>
                </>
              ...
              {detailAlgo.status === 4 && (
                <>
                  ...
                  <Button
                    ...
                    color='danger'
                    loading={actionLoadingId === detailAlgo.id}
                    onClick={() => { setDetailVisible(false); handleDelete(detailAlgo) }}
                  >
                    删除算法
                  </Button>
                </>
      ...
      >
        <View className='audit-content'>
          ...
          {auditAlgo && <Text className='audit-name'>算法：{auditAlgo.name}</Text>}
          {!auditApproved && (
          ...
          <View className='audit-footer'>
            <Button block onClick={() => setAuditVisible(false)}>取消</Button>
            <Button
              block
