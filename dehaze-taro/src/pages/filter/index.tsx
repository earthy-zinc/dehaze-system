...
// 滤镜配置
const FILTER_CONFIGS: { key: keyof FilterParams; label: string; min: number; max: number }[] = [
...
const FilterPage: React.FC = () => {
  ...
  const [customPresets, setCustomPresets] = useState<{ name: string; params: FilterParams }[]>([])
  ...
  const filterStyle = useMemo(() => {
    ...
    const hueRotate = params.temperature > 0 ? params.temperature * 0.5 : params.temperature * 0.5
              ...
              {FILTER_CONFIGS.map((config) => (
                <View key={config.key} className="slider-item">
                  ...
                  <Slider
                    ...
                    step={1}
                    activeColor="#1890ff"
                    onChanging={(e: any) => handleParamChange(config.key, e.detail.value)}
                    onChange={(e: any) => handleParamChange(config.key, e.detail.value)}
                  />
                </View>
              ))}
            </View>
