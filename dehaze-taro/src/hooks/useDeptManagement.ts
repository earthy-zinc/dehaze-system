...
export const useDeptManagement = () => {
  ...
  // 获取部门树形列表
  const fetchDeptList = useCallback(
    async (params?: Partial<DeptQuery>) => {
      setLoading(true);
      try {
        ...
        Taro.showToast({ title: "获取部门列表失败", icon: "none" });
        throw error;
      } finally {
        setLoading(false);
      }
  ...
  const fetchDeptOptions = useCallback(async () => {
    try {
      ...
      Taro.showToast({ title: "获取部门选项失败", icon: "none" });
      throw error;
    }
  }, []);
  ...
  const fetchDeptForm = useCallback(async (id: number) => {
    try {
      ...
      Taro.showToast({ title: "获取部门表单数据失败", icon: "none" });
      throw error;
    }
  }, []);
  ...
  const createDept = useCallback(
    async (data: DeptForm) => {
      try {
        ...
        await fetchDeptList();
        Taro.showToast({ title: "新增部门成功", icon: "none" });
        return true;
      } catch (error) {
        console.error("新增部门失败:", error);
        Taro.showToast({ title: "新增部门失败", icon: "none" });
        throw error;
      }
    },
    [fetchDeptList]
  ...
  const updateDept = useCallback(
    async (id: number, data: DeptForm) => {
      try {
        const { DeptAPI } = await import("dehaze-sdk-js");
        await DeptAPI.update(id, data);
        await fetchDeptList();
        Taro.showToast({ title: "修改部门成功", icon: "none" });
        return true;
      } catch (error) {
        console.error("修改部门失败:", error);
        Taro.showToast({ title: "修改部门失败", icon: "none" });
        throw error;
      }
    },
    [fetchDeptList]
  ...
  const deleteDept = useCallback(
    async (id: number) => {
      try {
        ...
        await fetchDeptList();
        Taro.showToast({ title: "删除部门成功", icon: "none" });
        return true;
      } catch (error) {
        console.error("删除部门失败:", error);
        Taro.showToast({ title: "删除部门失败", icon: "none" });
        throw error;
      }
    },
    [fetchDeptList]
