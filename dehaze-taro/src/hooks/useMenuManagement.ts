...
export const useMenuManagement = () => {
  ...
  // 获取菜单树形列表
  const fetchMenus = useCallback(async (params?: Partial<MenuQuery>) => {
    setLoading(true);
    try {
      ...
      Toast.open({ message: '获取菜单列表失败', position: 'top' });
      throw error;
    } finally {
      setLoading(false);
    }
  ...
  const fetchMenuOptions = useCallback(async () => {
    try {
      ...
      Toast.open({ message: '获取菜单下拉选项失败', position: 'top' });
      throw error;
    }
  }, []);
  ...
  const fetchMenuForm = useCallback(async (id: number) => {
    try {
      ...
      Toast.open({ message: '获取菜单表单数据失败', position: 'top' });
      throw error;
    }
  }, []);

  // 新增菜单
  const createMenu = useCallback(async (data: MenuForm) => {
    try {
      const { MenuAPI } = await import('dehaze-sdk-js');
      await MenuAPI.add(data);
      await fetchMenus();
      Toast.open({ message: '新增菜单成功', position: 'top' });
      return true;
    } catch (error) {
      console.error('新增菜单失败:', error);
      Toast.open({ message: '新增菜单失败', position: 'top' });
      throw error;
    }
  }, [fetchMenus]);

  // 修改菜单
  const updateMenu = useCallback(async (id: string, data: MenuForm) => {
    try {
      const { MenuAPI } = await import('dehaze-sdk-js');
      await MenuAPI.update(id, data);
      await fetchMenus();
      Toast.open({ message: '修改菜单成功', position: 'top' });
      return true;
    } catch (error) {
      console.error('修改菜单失败:', error);
      Toast.open({ message: '修改菜单失败', position: 'top' });
      throw error;
    }
  }, [fetchMenus]);
  ...
  const deleteMenu = useCallback(async (id: number) => {
    try {
      ...
      await fetchMenus();
      Toast.open({ message: '删除菜单成功', position: 'top' });
      return true;
    } catch (error) {
      console.error('删除菜单失败:', error);
      Toast.open({ message: '删除菜单失败', position: 'top' });
      throw error;
    }
  }, [fetchMenus]);

  // 搜索菜单
  const searchMenus = useCallback(async (keywords: string) => {
