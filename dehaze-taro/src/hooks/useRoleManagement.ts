...
import type { RoleQuery, RolePageVO, RoleForm, OptionType } from 'dehaze-sdk-js';
...
export const useRoleManagement = () => {
  ...
  // 获取角色列表
  const fetchRoles = useCallback(async (params?: Partial<RoleQuery>) => {
    setLoading(true);
    try {
      ...
      Toast.open({ message: '获取角色列表失败', position: 'top' });
      throw error;
    } finally {
      setLoading(false);
    }
  ...
  const fetchPermissions = useCallback(async () => {
    try {
      ...
      Toast.open({ message: '获取权限列表失败', position: 'top' });
      throw error;
    }
  }, []);
  ...
  const fetchRolePermissions = useCallback(async (roleId: number) => {
    try {
      ...
      Toast.open({ message: '获取角色权限失败', position: 'top' });
      throw error;
    }
  }, []);

  // 创建角色
  const createRole = useCallback(async (roleData: RoleForm) => {
    try {
      const { RoleAPI } = await import('dehaze-sdk-js');
      await RoleAPI.add(roleData);
      await fetchRoles();
      Toast.open({ message: '创建角色成功', position: 'top' });
      return true;
    } catch (error) {
      console.error('创建角色失败:', error);
      Toast.open({ message: '创建角色失败', position: 'top' });
      throw error;
    }
  }, [fetchRoles]);

  // 更新角色
  const updateRole = useCallback(async (id: number, roleData: RoleForm) => {
    try {
      const { RoleAPI } = await import('dehaze-sdk-js');
      await RoleAPI.update(id, roleData);
      await fetchRoles();
      Toast.open({ message: '更新角色成功', position: 'top' });
      return true;
    } catch (error) {
      console.error('更新角色失败:', error);
      Toast.open({ message: '更新角色失败', position: 'top' });
      throw error;
    }
  }, [fetchRoles]);

  // 删除角色
  const deleteRole = useCallback(async (ids: string | number) => {
    try {
      const { RoleAPI } = await import('dehaze-sdk-js');
      await RoleAPI.deleteByIds(String(ids));
      await fetchRoles();
      Toast.open({ message: '删除角色成功', position: 'top' });
      return true;
    } catch (error) {
      console.error('删除角色失败:', error);
      Toast.open({ message: '删除角色失败', position: 'top' });
      throw error;
    }
  }, [fetchRoles]);
  ...
  const getRoleOptions = useCallback(async () => {
    try {
      ...
      Toast.open({ message: '获取角色选项失败', position: 'top' });
      throw error;
    }
  }, []);

  // 搜索角色
  const searchRoles = useCallback(async (keywords: string) => {
