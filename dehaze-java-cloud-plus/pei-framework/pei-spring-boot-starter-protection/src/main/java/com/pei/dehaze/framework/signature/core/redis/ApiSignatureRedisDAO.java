package com.pei.dehaze.framework.signature.core.redis;
...
public class ApiSignatureRedisDAO {
     ...
     * 验签随机数
     * <p>
     * KEY 格式：signature_nonce:%s // 参数为 随机数 VALUE 格式：String 过期时间：不固定
     */
    private static final String SIGNATURE_NONCE = "api_signature_nonce:%s:%s";
    /**
     * 签名密钥
     * <p>
     * HASH 结构 KEY 格式：%s // 参数为 appid VALUE 格式：String 过期时间：永不过期（预加载到 Redis）
     */
