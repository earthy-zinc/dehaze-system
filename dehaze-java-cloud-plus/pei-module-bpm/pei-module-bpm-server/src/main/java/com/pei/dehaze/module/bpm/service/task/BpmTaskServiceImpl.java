package com.pei.dehaze.module.bpm.service.task;
...
import com.pei.dehaze.module.bpm.controller.admin.definition.vo.model.BpmModelMetaInfoVO;
import com.pei.dehaze.module.bpm.controller.admin.task.vo.task.*;
import com.pei.dehaze.module.bpm.convert.task.BpmTaskConvert;
...
import com.pei.dehaze.module.bpm.enums.task.BpmCommentTypeEnum;
import com.pei.dehaze.module.bpm.enums.task.BpmReasonEnum;
import com.pei.dehaze.module.bpm.enums.task.BpmTaskSignTypeEnum;
...
import com.pei.dehaze.module.bpm.framework.flowable.core.enums.BpmnVariableConstants;
import com.pei.dehaze.module.bpm.framework.flowable.core.util.BpmHttpRequestUtils;
...
import com.pei.dehaze.module.bpm.service.definition.BpmFormService;
import com.pei.dehaze.module.bpm.service.definition.BpmModelService;
import com.pei.dehaze.module.bpm.service.definition.BpmProcessDefinitionService;
import com.pei.dehaze.module.bpm.service.message.BpmMessageService;
...
import static com.pei.dehaze.module.bpm.enums.ErrorCodeConstants.*;
import static com.pei.dehaze.module.bpm.framework.flowable.core.enums.BpmnModelConstants.START_USER_NODE_ID;
import static com.pei.dehaze.module.bpm.framework.flowable.core.enums.BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_RETURN_FLAG;
import static com.pei.dehaze.module.bpm.framework.flowable.core.enums.BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_SKIP_START_USER_NODE;
import static com.pei.dehaze.module.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
public class BpmTaskServiceImpl implements BpmTaskService {
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 pei-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
             ...
             * 参见 <a href="https://gitee.com/zhijiantianya/pei-cloud/issues/IB7V7Q">issue</a> 反馈
             */
            @Override
     ...
     * <p>
     * 1. 或签场景 + 审批通过：一个或签有多个审批时，如果 A 审批通过，其它或签 B、C 等任务会被 Flowable 自动删除，此时需要通过该方法更新状态为已取消 2. 审批不通过：在
     * {@link #rejectTask(Long, BpmTaskRejectReqVO)} 不通过时，对于加签的任务，不会被 Flowable 删除，此时需要通过该方法更新状态为已取消
     */
    @Override
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
            ...
            public void afterCompletion(int transactionStatus) {
                ...
                if (userTaskElement.getId().equals(START_USER_NODE_ID)
                        ...
                        || skipStartUserNodeFlag) // 目的：一般是“子流程”，发起人节点，按配置自动通过审核
     ...
     * <p>
     * 1. 如果存在“向前加签”的任务，则不能“向后加签” 2. 如果存在“向后加签”的任务，则不能“向前加签”
     *
     ...
     * 校验选择的下一个节点的审批人，是否合法
     * <p>
     * 1. 是否有漏选：没有选择审批人 2. 是否有多选：非下一个节点
     *
     ...
     * 审批通过存在“后加签”的任务。
     * <p>
     * 注意：该任务不能马上完成，需要一个中间状态（APPROVING），并激活剩余所有子任务（PROCESS）为可审批处理 如果马上完成，则会触发下一个任务，甚至如果没有下一个任务则流程实例就直接结束了！
     *
     * @param task  当前任务
     * @param reqVO 前端请求参数
     */
     ...
     * <p>
     * 1. 如果是【向前】加签，则需要重新激活父任务，让它可以被审批 2. 如果是【向后】加签，则需要完成父任务，让它完成审批
     *
