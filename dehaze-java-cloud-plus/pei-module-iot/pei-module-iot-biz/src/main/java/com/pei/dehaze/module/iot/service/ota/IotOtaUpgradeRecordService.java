...
/**
 * IotOtaUpgradeRecordService 接口定义了与物联网设备OTA升级记录相关的操作。 该接口提供了创建、更新、查询、统计和重试升级记录的功能。
 */
public interface IotOtaUpgradeRecordService {

    /**
     * 批量创建 OTA 升级记录 该函数用于为指定的设备列表、固件ID和升级任务ID创建OTA升级记录。
     *
     ...
     * 根据任务 ID 取消升级记录
     * <p>
     * 该函数用于根据给定的任务ID，取消与该任务相关的升级记录。通常用于在任务执行失败或用户手动取消时， 清理或标记相关的升级记录为取消状态。
     *
