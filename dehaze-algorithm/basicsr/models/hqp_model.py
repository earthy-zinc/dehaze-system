...
class HQPModel(BaseModel):
    def __init__(self, opt):
        ...
        if load_path is not None:
            logger.info(f'从 {load_path} 中加载生成网络（network generator）')
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output, l_codebook, l_semantic, feat_to_quant, quant_gt, gt_indices = self.net_g(self.gt)
        ...
        if self.use_dis and current_iter > train_opt['net_d_init_iters']:
            fake_g_pred = self.net_d(quant_gt)
    ...
    def vis_single_code(self, up_factor=2):
        net_g = self.get_bare_model(self.net_g)
        codebook_emb_num = self.opt['network_g']['codebook_emb_num']
        with torch.no_grad():
            code_idx = torch.arange(codebook_emb_num).reshape(codebook_emb_num, 1, 1, 1)
    ...
    def get_current_visuals(self):
        ...
        out_dict['gt_rec'] = self.output.detach().cpu()[:vis_samples]
