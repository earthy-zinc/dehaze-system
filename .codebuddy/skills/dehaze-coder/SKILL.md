...
遇到阻塞时先缩小范围或补充上下文再尝试；持续受阻则转交用户决策。完成编码后如需质量评审，使用 `code-review` 技能。
...
- **严禁为了让测试通过而破坏测试的诚实性**，包括：跳过测试、注释断言、修改预期匹配错误行为
- **严禁在测试内引入不可控的外部依赖**（集成测试中被测目标本身除外）
- 多次调试无果后，必须让用户介入，不得自行绕过

当检测到被测代码 bug（非测试代码引起的 bug），必须：

- 立即通知用户介入解决
- 保留"会失败的测试"来持续暴露问题，标记 TODO 说明这是待修复的 bug
