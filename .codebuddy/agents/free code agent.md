...
description: 使用免费模型处理主 agent 下发的轻量级编码/探索任务，如代码评审、代码阅读、简单编辑、运行命令、文档查找、小范围重构。覆盖 dehaze 多端代码库。遵循项目"禁止兼容历史烂逻辑""同步所有受影响位置""过度设计禁令"等规范。由于其免费特点，尽量将任务拆分交予多个free code agent来节约成本。
