...
-- 配置类表，使用逻辑删除；唯一键含 deleted，软删后可重建同 (model_id, provider_id)。
-- ------------------------------------------------------------
...
(
    ...
    `last_test_status`     tinyint                                                         NOT NULL DEFAULT 0 COMMENT '最近可用性测试状态(0:未测试;1:可用;2:不可用)',
    `last_test_at`         datetime                                                        NULL DEFAULT NULL COMMENT '最近可用性测试时间',
    `last_test_error`      varchar(500) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci  NULL DEFAULT NULL COMMENT '最近可用性测试错误信息(含HTTP状态与延迟)',
    ...
    UNIQUE INDEX `uk_model_provider` (`model_id`, `provider_id`, `deleted`) USING BTREE,
