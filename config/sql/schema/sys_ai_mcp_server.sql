...
-- ============================================================
-- 设计思路:
-- 外部 MCP Server 的注册中心。任何符合 MCP 规范的外部服务（streamable-http/sse，
-- stdio 无网络端点、运行时不支持，已禁用注册）均可注册接入，工具按命名空间分组供
-- Agent 关联（最小权限）。
-- credentials 存储 AES 加密后的密文（JSON，仅录入/更新，不回显明文、不暴露给 LLM）。
-- health 记录最近一次健康探测结果（online/offline），探测失败不阻断管理流程；
-- last_check_time 记录探测时间（手动探测与后台巡检共用）。
...
(
    ...
    `protocol_type` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci  NOT NULL DEFAULT 'streamable-http' COMMENT '传输协议(streamable-http;sse)',
    `endpoint`    varchar(512) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci   NULL DEFAULT NULL COMMENT '端点URL',
    ...
    `last_check_time` datetime                                                     NULL DEFAULT NULL COMMENT '最近一次健康探测时间',
