...
func RegisterAlgorithmRoutes(rg *gin.RouterGroup, algorithmApi *api.AlgorithmApi) {
...

	{
		...
		algorithmRouterGroup.PUT("/:id/audit", middleware.Permission("sys:algorithm:audit"), algorithmApi.Audit)                                                   // 审核算法（通过→已发布/驳回→测试中）
		algorithmRouterGroup.POST("/:id/version", middleware.Permission("sys:algorithm:version"), algorithmApi.CreateVersion)                                      // 新增版本
		algorithmRouterGroup.POST("/:id/rollback", middleware.Permission("sys:algorithm:version"), algorithmApi.RollbackVersion)                                   // 版本回滚
