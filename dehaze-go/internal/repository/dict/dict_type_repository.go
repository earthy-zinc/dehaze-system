...
// FindPage 分页查询字典类型
func (r *DictTypeRepository) FindPage(ctx context.Context, q *query.DictTypePageQuery) (*read.PageResult[read.DictTypePage], error) {
	pageNum := q.PageNum
	pageSize := q.PageSize
	if pageNum <= 0 {
		pageNum = 1
	...
	var dictTypePages []read.DictTypePage
	for _, dictType := range dictTypeList {
		dictTypePage := read.DictTypePage{
			ID:         dictType.ID,
			Name:       dictType.Name,
			Code:       dictType.Code,
			Status:     dictType.Status,
			Remark:     dictType.Remark,
			CreateTime: dictType.CreatedAt,
		}
		dictTypePages = append(dictTypePages, dictTypePage)
	}

	return &read.PageResult[read.DictTypePage]{
		List:     dictTypePages,
		Total:    total,
		PageNum:  pageNum,
		PageSize: pageSize,
...
func (r *DictTypeRepository) FindCodesByIDs(ctx context.Context, ids []int64) ([]string, error) {
	if len(ids) == 0 {
		return nil, nil
	}
	var codes []string
...
// Transaction 执行事务
func (r *DictTypeRepository) Transaction(ctx context.Context, fn func(repo IDictTypeRepository) error) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		return fn(&DictTypeRepository{db: tx})
	})
}

// WithDB 返回使用指定 DB 的新实例（用于跨 Repository 事务）
func (r *DictTypeRepository) WithDB(db *gorm.DB) IDictTypeRepository {
	return &DictTypeRepository{db: db}
}
