...
func (r *itemFileRepository) FindByItemIDs(ctx context.Context, itemIDs []int64) ([]model.SysItemFile, error) {
	if len(itemIDs) == 0 {
		return nil, nil
	}
	var itemFiles []model.SysItemFile
	...
	return itemFiles, err
}

// DeleteByItemIDs 根据数据项 ID 列表删除所有项文件
func (r *itemFileRepository) DeleteByItemIDs(ctx context.Context, itemIDs []int64) error {
	if len(itemIDs) == 0 {
		return nil
	}
	return r.db.WithContext(ctx).
		Where("item_id IN ?", itemIDs).
		Delete(&model.SysItemFile{}).Error
}
