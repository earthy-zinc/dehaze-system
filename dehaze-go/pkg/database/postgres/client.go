...
func init() {
	// 注册PostgreSQL工厂
	database.RegisterFactory("postgres", &postgresFactory{})
}

// postgresFactory PostgreSQL工厂实现
type postgresFactory struct{}

// Create 创建PostgreSQL数据库实例
func (f *postgresFactory) Create(config *database.Config) (database.DBer, error) {
	return NewClient(config)
}

// Client PostgreSQL客户端实现
type Client struct {
	master *gorm.DB   // 主库实例
...
func NewClient(config *database.Config) (*Client, error) {
	if config == nil {
		return nil, fmt.Errorf("postgres: 配置为空")
	}
	...
	if err := client.initMaster(); err != nil {
		return nil, fmt.Errorf("postgres: 初始化主库失败: %w", err)
	}

	// 初始化从库（可选）
	if err := client.initSlaves(); err != nil {
		// 从库初始化失败不影响主库使用，仅记录日志
		logger.Warn("postgres: 从库初始化失败，将使用主库进行读操作", zap.Error(err))
	}
...
func (c *Client) initMaster() error {
	masterConfig := c.config.Postgres.Master

	// 构建DSN
	dsn := database.BuildPostgresDSN(masterConfig, c.config.Postgres.SSLMode)

	// PostgreSQL驱动配置
	postgresConfig := postgres.Config{
		DSN:                  dsn,
		PreferSimpleProtocol: c.config.Postgres.PreferSimpleProtocol,
	}

	// 创建Gorm实例
	db, err := gorm.Open(postgres.New(postgresConfig), database.GetGormConfig(c.config))
	if err != nil {
...
func (c *Client) initSlaves() error {
	if len(c.config.Postgres.Slaves) == 0 {
		return nil
	}

	for i, slaveConfig := range c.config.Postgres.Slaves {
		// 构建DSN
		dsn := database.BuildPostgresDSN(slaveConfig, c.config.Postgres.SSLMode)
...
func (c *Client) Close() error {
	...
	for i, slave := range c.slaves {
		if err := database.CloseDB(slave); err != nil {
			...
			logger.Info("postgres: 从库连接已关闭", zap.Int("index", i))
		}
	}

	if len(errs) > 0 {
		return fmt.Errorf("postgres: 关闭错误: %v", errs)
	}
...
func (c *Client) GetRawDB() (interface{}, error) {
	if c.master == nil {
		return nil, fmt.Errorf("postgres: 主库为空")
	}
	return c.master.DB()
}
